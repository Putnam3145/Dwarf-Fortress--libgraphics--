...
void musicsoundst::play_ambience(int32_t new_ambience) {
  ...
  bool active_is_playing =
      internal->is_ambience_playing(ambience_active_channel);

  if ((new_ambience != ambience_active || !active_is_playing) &&
      !ambience_crossfading) {
    internal->set_init_volumes(init.media.volume_master, init.media.volume_sfx,
                               init.media.volume_ambience,
                               init.media.volume_music);
  ...
  if (new_song != song || !is_playing) {
    if (!is_playing) {
      ...
      if (song != new_song && song >= 0 && song < SONGNUM)
        internal->stop_song();
      internal->set_init_volumes(
          init.media.volume_master, init.media.volume_sfx,
          init.media.volume_ambience, init.media.volume_music);
      song = new_song;
      if (queued_song == song)
        queued_song = -1;
...
void musicsoundst::stop_song() { internal->stop_song(); }

bool musicsoundst::card_is_playing() { return internal->is_card_playing(); }

void musicsoundst::stop_card() { internal->stop_card(); }
...
float musicsoundst::get_song_volume() { return internal->get_song_volume(); }

float musicsoundst::get_card_volume() { return internal->get_card_volume(); }
...
void musicsoundst::set_custom_song(string& token, string& file, string& title,
                                   string& author, bool loops) {
  if (loaded_music.count(token))
    return;

  int id = next_song_id;
  next_song_id++;

  loading_files.push_back(std::async([this, token, file, title, author, id,
                                      loops]() mutable -> loading_music_filest {
...
void musicsoundst::set_custom_sound(string& token, string& file) {
  if (loaded_sounds.count(token))
    return;

  int id = next_sound_id;
  next_sound_id++;

  loading_files.push_back(
      std::async([this, token, file, id]() mutable -> loading_music_filest {
    ...
    for (f = 0; f < processfilename.size(); f++) {
      ...
      for (int t = 1; t < lines.text.str.size(); t++) {
        ...
        for (int pos = 0; pos < curstr.length(); pos++) {
          if (curstr[pos] == '[') {
            string token;
            if (!grab_token_string(token, curstr, pos))
              continue;
            if (token == "MUSIC_FILE") {
              if (!grab_token_string(token, curstr, pos))
                continue;
      ...
      for (auto& mus : files) {
        set_custom_song(mus.token, mus.filename, mus.title, mus.author,
                        mus.loops);
      }
...
musicsound_info::musicsound_info(void* handle) {
  if (handle != NULL) {
    is_ambience_playing_ptr =
        (bool (*)(int32_t))(dlsym(handle, "is_ambience_playing"));
    ...
    play_ambience_ptr =
        (void (*)(int32_t, int32_t))(dlsym(handle, "play_ambience"));
    play_sound_ptr =
        (void (*)(int, int32_t, bool))(dlsym(handle, "play_sound"));
    set_song_ptr =
        (bool (*)(const std::string&, int, bool))(dlsym(handle, "set_song"));
    set_sound_ptr =
        (bool (*)(const std::string&, int))(dlsym(handle, "set_sound"));
    set_init_volumes_ptr =
        (void (*)(int, int, int, int))(dlsym(handle, "set_init_volumes"));
    set_song_volume_ptr = (void (*)(float))(dlsym(handle, "set_song_volume"));
    set_card_volume_ptr = (void (*)(float))(dlsym(handle, "set_card_volume"));
    set_ambience_volume_ptr =
        (void (*)(int32_t, float))(dlsym(handle, "set_ambience_volume"));
    get_song_volume_ptr = (float (*)())(dlsym(handle, "get_song_volume"));
    get_card_volume_ptr = (float (*)())(dlsym(handle, "get_card_volume"));
    get_ambience_volume_ptr =
        (float (*)(int))(dlsym(handle, "get_ambience_volume"));
...
bool musicsound_info::is_ambience_playing(int32_t ambience_active_channel) {
  ...
  if (ambience_channel[ambience_active_channel] != NULL) {
    result = ambience_channel[ambience_active_channel]->isPlaying(
        &active_is_playing);
  }
...
void musicsound_info::play_ambience(int32_t new_ambience,
                                    int32_t ambience_active_channel) {
  result = fmod_system->playSound(stream[new_ambience], NULL, true,
                                  &ambience_channel[ambience_active_channel]);
...
void musicsound_info::play_sound(int s, int32_t vol,
                                 bool use_media_sound_volume) {
  int32_t i;
  for (i = 0; i < 10; ++i) {
    if (sound_channel[i] == NULL) {
      ...
      sound_channel[i]->setVolume(
          int_vol_to_float(vol) * use_media_sound_volume
              ? int_vol_to_float(init.media.volume_sfx)
              : 1.0f * int_vol_to_float(init.media.volume_master));
  ...
  for (i = 0; i < 10; ++i) {
    if (sound_channel[i] != NULL) {
      ...
      if (!is_playing) {
        ...
        sound_channel[i]->setPosition(0, FMOD_TIMEUNIT_MS);
        sound_channel[i]->setVolume(int_vol_to_float(init.media.volume_sfx) *
                                    int_vol_to_float(init.media.volume_master));
        sound_channel[i]->setPaused(false);
        return;
      }
    }
...
bool musicsound_info::set_song(const string& filename, int slot, bool loops) {
  ...
  result = fmod_system->createStream(filename.c_str(),
                                     (loops ? FMOD_LOOP_NORMAL : 0) | FMOD_2D,
                                     0, &stream[slot]);
  switch (result) {
  case FMOD_OK:
    return true;
  case FMOD_ERR_FILE_BAD:
    errorlog_string("Could not load file " + filename +
                    ": error loading file.");
    return false;
  ...
  case FMOD_ERR_FILE_EOF:
    errorlog_string("Could not load file " + filename +
                    ": end of file unexpectedly reached while trying to read "
                    "essential data.");
    return false;
...
bool musicsound_info::set_sound(const string& filename, int slot) {
  ...
  result =
      fmod_system->createSound(filename.c_str(), FMOD_LOOP_OFF, 0, &samp[slot]);
  switch (result) {
  case FMOD_OK:
    return true;
  case FMOD_ERR_FILE_BAD:
...
void musicsound_info::set_init_volumes(int master_volume, int snd_volume,
                                       int ambience_volume, int music_volume) {
} // technically only here to keep the API clean elsewhere
...
void musicsound_info::deinit_sound() {
  ...
  if (song_channel)
    song_channel->stop();
  if (ambience_channel[0] != NULL)
    ambience_channel[0]->stop();
  if (ambience_channel[1] != NULL)
    ambience_channel[1]->stop();
  if (card_channel != NULL)
    card_channel->stop();
  for (auto& m : mod) {
    if (m)
