...
void play_sound(int s, int32_t vol, bool use_media_sound_volume) {
  ...
  for (i = 0; i < 10; ++i) {
    if (sound_channel[i] == NULL) {
      ...
      if (s == SOUND_ALERT)
        sound_channel[i]->setPan(-0.75f);
      else
        sound_channel[i]->setPan(0.0f);
      ...
      if (use_media_sound_volume) {
        if (vol == 255)
          ...
          sound_channel[i]->setVolume(((float)vol / 255.0f) * volume_sfx *
                                      volume_master);
      } else {
        if (vol == 255)
          sound_channel[i]->setVolume(volume_master);
        else
          sound_channel[i]->setVolume(((float)vol / 255.0f) * volume_master);
      }
...
void set_init_volumes(int master_volume, int snd_volume, int ambience_volume,
                      int music_volume) {
