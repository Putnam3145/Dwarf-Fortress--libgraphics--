...
template <class T> void zero_vector(svector<T>& vc) {
  // NOTE: vector MEMORY IS GUARANTEED TO BE CONTIGUOUS, AND THIS IS FASTER THAN
  // GOING THROUGH ONE BY ONE apparently this gives linux a headache though, so
  // back to the slower way
  // int32_t sz=vc.size();
  // if(sz==0)return;
...
void add_dual_unique_to_vectors(T nl, T2 nl2, svector<T>& vc,
                                svector<T2>& vc2) {
  auto ii_s = vc.begin(), ii_e = vc.end();
  auto ii_s2 = vc2.begin(), ii_e2 = vc2.end();
  for (; ii_s < ii_e; ++ii_s, ++ii_s2) {
    if ((*ii_s) == nl && (*ii_s2) == nl2)
      return;
  }
  vc.push_back(nl);
  vc2.push_back(nl2);
...
void add_quad_unique_to_vectors(T nl, T2 nl2, T3 nl3, T4 nl4, svector<T>& vc,
                                svector<T2>& vc2, svector<T3>& vc3,
                                svector<T4>& vc4) {
  auto ii_s = vc.begin(), ii_e = vc.end();
  auto ii_s2 = vc2.begin(), ii_e2 = vc2.end();
  ...
  for (; ii_s < ii_e; ++ii_s, ++ii_s2, ++ii_s3, ++ii_s4) {
    if ((*ii_s) == nl && (*ii_s2) == nl2 && (*ii_s3) == nl3 && (*ii_s4) == nl4)
      return;
  }
  vc.push_back(nl);
  vc2.push_back(nl2);
...
template <class T, class T2>
void remove_all_from_dual_vectors(T nl, T2 nl2, svector<T>& vc,
                                  svector<T2>& vc2) {
  int32_t i;
  for (i = (int32_t)vc.size() - 1; i >= 0; i--) {
    if (vc[i] == nl && vc2[i] == nl2) {
      vc.erase(i);
      vc2.erase(i);
    }
  }
...
template <class T, class T2>
int32_t get_dual_vector_index(T a1, T2 a2, svector<T>& vc, svector<T2>& vc2) {
  auto ii_s = vc.begin(), ii_e = vc.end();
  auto ii_s2 = vc2.begin(), ii_e2 = vc2.end();
  auto ii_b = ii_s;
  for (; ii_s < ii_e; ++ii_s, ++ii_s2) {
...
template <class T, class T2, class T3, class T4>
int32_t get_quad_vector_index(T a1, T2 a2, T3 a3, T4 a4, svector<T>& vc,
                              svector<T2>& vc2, svector<T3>& vc3,
                              svector<T4>& vc4) {
  auto ii_s = vc.begin(), ii_e = vc.end();
  auto ii_s2 = vc2.begin(), ii_e2 = vc2.end();
  ...
  auto ii_b = ii_s;
  for (; ii_s < ii_e; ++ii_s, ++ii_s2, ++ii_s3, ++ii_s4) {
    if ((*ii_s) == a1 && (*ii_s2) == a2 && (*ii_s3) == a3 && (*ii_s4) == a4)
      return (int32_t)(ii_s - ii_b);
  }
  return -1;
}
...
template <class T>
int32_t get_common_element_vector_index(T& master, T& merger) {
  auto ii_s = merger.begin(), ii_e = merger.end();
  auto ii_s2 = master.begin(), ii_e2 = master.end();
  auto ii_b2 = ii_s2;
...
template <class T, class T2>
void merge_dual_vectors(T& master, T2& master2, T& merger, T2& merger2) {
  auto ii_s = merger.begin(), ii_e = merger.end();
...
template <class T, class T2, class T3, class T4>
void merge_quad_vectors(T& master, T2& master2, T3& master3, T4& master4,
...
template <class T>
void remove_from_global_id_vector_by_id(int32_t id, svector<T>& vect) {
  int32_t start = 0;
  int32_t end = (int32_t)vect.size() - 1;
...
template <class T>
int32_t get_index_from_global_id_vector(int32_t id, svector<T>& vect) {
  int32_t start = 0;
  int32_t end = (int32_t)vect.size() - 1;
...
template <class T>
VIndex add_possible_duplicate_to_binary_vector(T ptr, svector<T>& vect) {
  ...
  while (start <= end) {
    ...
    if (cptr == ptr) {
      vect.insert(mid,
                  ptr); // INSERT A COPY HERE SINCE THIS IS A NON-UNIQUE VECTOR
      return mid;
    }
    if (start == end) {
      if (cptr < ptr) {
...
template <class T>
VIndex add_unique_to_binary_vector_always_index(T ptr, svector<T>& vect,
                                                bool& was_present) {
  was_present = false;
...
template <class T>
int32_t get_index_from_binary_vector(T id, svector<T>& vect) {
  int32_t size = (int32_t)vect.size();
  if (size == 0 || id == -1)
    return -1;
...
template <class T>
int32_t get_floor_index_from_binary_vector(T ptr, svector<T>& vect) {
  int32_t size = (int32_t)vect.size();
  if (size == 0)
    return -1;
  if (vect[size - 1] < ptr)
...
template <class T>
void fixed_array_push_back(T ptr, T* vect, int32_t& size, int32_t max) {
  if (size >= max)
    return;
  vect[size] = ptr;
  size++;
...
template <class T>
void fixed_array_insert(int32_t index, T ptr, T* vect, int32_t& size,
                        int32_t max) {
  if (size >= max)
    return;
  if (size > 0) {
...
template <class T>
void add_to_fixed_binary_array(T ptr, T* vect, int32_t& size, int32_t max) {
  if (size == 0) {
  ...
  while (start <= end) {
    ...
    cptr = vect[mid];
    if (cptr == ptr) {
      fixed_array_insert(mid, ptr, vect, size,
                         max); // INSERT A COPY SINCE THIS IS NON-UNIQUE CASE
      return;
    }
    if (start == end) {
      if (cptr < ptr) {
        if (start + 1 >= size)
          ...
          fixed_array_insert(start + 1, ptr, vect, size, max);
      } else
        fixed_array_insert(start, ptr, vect, size, max);
      return;
    }
  ...
  if (end < 0) {
    ...
    if (cptr < ptr) {
      if (end + 1 >= size)
        ...
        fixed_array_insert(end + 1, ptr, vect, size, max);
    } else
      fixed_array_insert(end, ptr, vect, size, max);
  } else
    fixed_array_push_back(ptr, vect, size, max);
}

template <class T>
void add_unique_to_fixed_binary_array(T ptr, T* vect, int32_t& size,
                                      int32_t max) {
  if (size == 0) {
    fixed_array_push_back(ptr, vect, size, max);
    return;
  ...
  // NOTE: NO else CASE HERE BECAUSE IN THE NON-UNIQUE VERSION IT JUST PUSHED
  // BACK AND PUSHING BACK IS HANDLED IN THE OPENING FUNCTION
}
    ...
    if (start == end) {
      if (cptr->local_id < ptr->local_id) {
        if (start + 1 >= size)
          ...
          vect.insert(start + 1, ptr);
      } else if (cptr->local_id > ptr->local_id)
        vect.insert(start, ptr);
      return;
    }
...
template <class T>
int32_t get_index_from_local_id_vector(int32_t id, svector<T>& vect) {
  int32_t start = 0;
  int32_t end = (int32_t)vect.size() - 1;
    ...
    if (start == end) {
      if (cptr->short_id < ptr->short_id) {
        if (start + 1 >= size)
          ...
          vect.insert(start + 1, ptr);
      } else if (cptr->short_id > ptr->short_id)
        vect.insert(start, ptr);
      return;
    }
...
template <class T>
int16_t get_index_from_short_id_vector(int16_t id, svector<T>& vect) {
  int16_t start = 0;
  int16_t end = (int16_t)vect.size() - 1;
  int16_t mid;
  T cptr;
...
template <class T>
void remove_from_local_id_64_vector(T ptr, svector<T>& vect) {
  int64_t start = 0;
...
template <class T>
int64_t get_index_from_local_id_64_vector(int64_t id, svector<T>& vect) {
  int64_t start = 0;
  int64_t end = (int64_t)vect.size() - 1;
