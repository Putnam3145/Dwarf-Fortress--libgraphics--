...
// opening/closing these too often seems to cause crashes on
// linux--std::osyncstream makes sure they can be used between threads, too
...
void errorlog_string(const std::string& str) {
  ...
  if (!error_file.is_open())
    error_file.open("errorlog.txt", std::ios::out | std::ios::app);
  // SAVE AN ERROR TO THE LOG FILE
...
void gamelog_string(const std::string& str) {
  ...
  if (!gamelog_file.is_open())
    gamelog_file.open("gamelog.txt", std::ios::out | std::ios::app);
