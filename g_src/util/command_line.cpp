...
void command_linest::init(const std::string& str) {
  ...
  // HANDLE EACH TOKEN
  for (long l = 0; l < arg_vect.str.size(); l++) {
    handle_arg(arg_vect.str[l]->dat);
  }
}

char command_linest::grab_arg(std::string& source, long& pos) {
  std::string dest;
...
void command_linest::handle_arg(std::string& arg) {
  long pos = 0;
  std::string dest;
  ...
  if (dest == "gen") {
    ...
    while (pos + 1 < arg.length()) {
      ...
      if (!dest.empty()) {
        if (arg_pos == 0)
          gen_id = convert_string_to_long(dest);
