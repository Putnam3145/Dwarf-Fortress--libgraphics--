...
protected:
  ...
  bool init_video(int w, int h) {
    ...
    SDL_GL_SetAttribute(SDL_GL_SWAP_CONTROL,
                        init.window.flag.has_flag(INIT_WINDOW_FLAG_VSYNC_ON));
    SDL_GL_SetAttribute(
        SDL_GL_DOUBLEBUFFER,
    ...
    if (test != ((init.display.flag.has_flag(INIT_DISPLAY_FLAG_SINGLE_BUFFER))
                     ? 0
                     : 1)) {
      if (enabler.is_fullscreen())
        ;
  ...
  virtual void allocate(int tiles) {
    vertexes = static_cast<GLfloat*>(
        realloc(vertexes, sizeof(GLfloat) * tiles * 2 * 6));
    assert(vertexes);
  ...
  virtual void init_opengl() { enabler.textures.upload_textures(); }

  virtual void uninit_opengl() { enabler.textures.remove_uploaded_textures(); }
  ...
  void write_tile_arrays(int x, int y, GLfloat* fg, GLfloat* bg, GLfloat* tex) {
    ...
    if (id.isL) { // An ordinary tile
      ...
      // TODO: Only bother to set the one that's actually read in flat-shading
      // mode And set flat-shading mode.
      for (int i = 0; i < 6; i++) {
        *(fg++) = id.left.r;
        *(fg++) = id.left.g;
        *(fg++) = id.left.b;
...
public:
  ...
  renderer_opengl() {
    ...
    if (modes == NULL)
      ...
      int32_t i = 0;
      while (modes[i]) {
        if (modes[i]->w >= MINIMUM_WINDOW_WIDTH &&
            modes[i]->h >= MINIMUM_WINDOW_HEIGHT) {
  ...
  pair<int, int> compute_zoom(bool clamp = false) {
    const int dispx = enabler.is_fullscreen() ? init.font.large_font_dispx
                                              : init.font.small_font_dispx;
    const int dispy = enabler.is_fullscreen() ? init.font.large_font_dispy
  ...
  void resize(int w, int h) {
    ...
    dispx = enabler.is_fullscreen() ? init.font.large_font_dispx
                                    : init.font.small_font_dispx;
    dispy = enabler.is_fullscreen() ? init.font.large_font_dispy
                                    : init.font.small_font_dispy;
  ...
  void grid_resize(int w, int h) { reshape(make_pair(w, h)); }
...
public:
  void set_fullscreen() {
    if (enabler.is_fullscreen()) {
      ...
      resize(init.display.actual_windowed_width,
             init.display.actual_windowed_height);
    }
  }
};
...
protected:
  void update_tile(int x, int y) {
    ...
    write_tile_arrays(x, y, fg + tile_count * 6 * 4, bg + tile_count * 6 * 4,
                      tex + tile_count * 6 * 2);
    tile_count++;
  }
...
class renderer_partial : public renderer_opengl {
  ...
  void update_tile(int x, int y) {
    ...
    write_tile_arrays(x, y, fg + head * 6 * 4, bg + head * 6 * 4,
                      tex + head * 6 * 2);
    ...
    if (head == tail) {
      ...
      tail += buffersz;
      memmove(vertexes + tail * 6 * 2, vertexes + head * 6 * 2,
              sizeof(GLfloat) * 6 * 2 * (buffersz - head));
      memmove(fg + tail * 6 * 4, fg + head * 6 * 4,
              sizeof(GLfloat) * 6 * 4 * (buffersz - head));
      memmove(bg + tail * 6 * 4, fg + head * 6 * 4,
              sizeof(GLfloat) * 6 * 4 * (buffersz - head));
      memmove(tex + tail * 6 * 2, fg + head * 6 * 2,
              sizeof(GLfloat) * 6 * 2 * (buffersz - head));
      // And finish.
      buffersz *= 2;
    }
  ...
  void allocate(int tile_count) { assert(false); }
  ...
  void draw_arrays(GLfloat* vertexes, GLfloat* fg, GLfloat* bg, GLfloat* tex,
                   int tile_count) {
    // Set vertex pointer
    glVertexPointer(2, GL_FLOAT, 0, vertexes);
    // Render the background colors
  ...
  void draw(int dummy) {
    if (tail > head) {
      ...
      draw_arrays(vertexes + tail * 6 * 2, fg + tail * 6 * 4, bg + tail * 6 * 4,
                  tex + tail * 6 * 2, buffersz - tail);
      draw_arrays(vertexes, fg, bg, tex, head - 1);
    } else {
      draw_arrays(vertexes + tail * 6 * 2, fg + tail * 6 * 4, bg + tail * 6 * 4,
                  tex + tail * 6 * 2, sum_erasz);
    }
...
class renderer_framebuffer : public renderer_once {
  ...
  void init_opengl() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, screen->w, screen->h, 0, GL_RGBA,
...
class renderer_vbo : public renderer_opengl {
  ...
  void init_opengl() {
    ...
    glBufferDataARB(GL_ARRAY_BUFFER_ARB,
                    gps.dimx * gps.dimy * 6 * 2 * sizeof(GLfloat), vertexes,
                    GL_STATIC_DRAW_ARB);
