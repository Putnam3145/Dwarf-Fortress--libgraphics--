...
struct palettest {
  ...
  palettest() {
    int32_t c;
    for (c = 0; c < PALETTE_COLORNUM * 3; ++c)
      color[c] = 128;
  }
 ...
 *
 *
 * X*Y tiles of 4 bytes each in column-major order (FIXME: This is inefficient!
 * It should be row-major!) For each tile, byte 0 is the character, 1 is
 * foreground color, 2 is bacground, and 3 denotes bold.
 *
 * As there are only 8 different colors and bold is a boolean, this leaves a lot
 * of free space. Therefore, without involving the graphics arrays, out-of-gamut
 * values can be used for designating TTF objects.
 *
 * This means setting the bold byte to all 1s (0xff), and then using the other
 * three bytes as a designator.
 *
 * Time will tell whether this was a good idea or not.
 */

// So yeah, we store "type" in the previously bold byte. This means we have
// quite a lot of free types yet.
#define GRAPHICSTYPE_TTF 0xff
// This type denotes a tile that is covered by truetype, but is not the tile it
// starts on.
...
struct graphic_map_portst {
  ...
  //*************************** VIEWPORT OFFSETS
  // view port needs to understand an offset from 0 to tiledimx-1 for shifting
  // purposes and it also has an underlying grid then that is updated according
  // to some coordinate system the map rectangle on the outside handles those
  // offsets and that kind of information, as well as understanding what the
  // offsetx/y camera means
  ...
  ~graphic_map_portst() { clean_arrays(); }
  ...
  void set_clipping(int32_t min_x, int32_t max_x, int32_t min_y,
                    int32_t max_y) {
    clipx[0] = min_x;
    clipx[1] = max_x;
    clipy[0] = min_y;
  ...
  void clean_arrays() {
    if (screentexpos_base != NULL)
      delete[] screentexpos_base;
    int32_t ei;
    for (ei = 0; ei < 8; ++ei) {
      if (screentexpos_edge[ei] != NULL)
        delete[] screentexpos_edge[ei];
      if (screentexpos_edge2[ei] != NULL)
        delete[] screentexpos_edge2[ei];
    }
    if (screentexpos_detail != NULL)
      delete[] screentexpos_detail;
    if (screentexpos_tunnel != NULL)
      delete[] screentexpos_tunnel;
    if (screentexpos_river != NULL)
      delete[] screentexpos_river;
    if (screentexpos_road != NULL)
      delete[] screentexpos_road;
    if (screentexpos_site != NULL)
      delete[] screentexpos_site;
    if (screentexpos_interface != NULL)
      delete[] screentexpos_interface;
    if (screentexpos_detail_to_n != NULL)
      delete[] screentexpos_detail_to_n;
    if (screentexpos_detail_to_s != NULL)
      delete[] screentexpos_detail_to_s;
    if (screentexpos_detail_to_w != NULL)
      delete[] screentexpos_detail_to_w;
    if (screentexpos_detail_to_e != NULL)
      delete[] screentexpos_detail_to_e;
    if (screentexpos_detail_to_nw != NULL)
      delete[] screentexpos_detail_to_nw;
    if (screentexpos_detail_to_ne != NULL)
      delete[] screentexpos_detail_to_ne;
    if (screentexpos_detail_to_sw != NULL)
      delete[] screentexpos_detail_to_sw;
    if (screentexpos_detail_to_se != NULL)
      delete[] screentexpos_detail_to_se;

    if (screentexpos_base_old != NULL)
      delete[] screentexpos_base_old;
    for (ei = 0; ei < 8; ++ei) {
      if (screentexpos_edge_old[ei] != NULL)
        delete[] screentexpos_edge_old[ei];
      if (screentexpos_edge2_old[ei] != NULL)
        delete[] screentexpos_edge2_old[ei];
    }
    if (screentexpos_detail_old != NULL)
      delete[] screentexpos_detail_old;
    if (screentexpos_tunnel_old != NULL)
      delete[] screentexpos_tunnel_old;
    if (screentexpos_river_old != NULL)
      delete[] screentexpos_river_old;
    if (screentexpos_road_old != NULL)
      delete[] screentexpos_road_old;
    if (screentexpos_site_old != NULL)
      delete[] screentexpos_site_old;
    if (screentexpos_interface_old != NULL)
      delete[] screentexpos_interface_old;
    if (screentexpos_detail_to_n_old != NULL)
      delete[] screentexpos_detail_to_n_old;
    if (screentexpos_detail_to_s_old != NULL)
      delete[] screentexpos_detail_to_s_old;
    if (screentexpos_detail_to_w_old != NULL)
      delete[] screentexpos_detail_to_w_old;
    if (screentexpos_detail_to_e_old != NULL)
      delete[] screentexpos_detail_to_e_old;
    if (screentexpos_detail_to_nw_old != NULL)
      delete[] screentexpos_detail_to_nw_old;
    if (screentexpos_detail_to_ne_old != NULL)
      delete[] screentexpos_detail_to_ne_old;
    if (screentexpos_detail_to_sw_old != NULL)
      delete[] screentexpos_detail_to_sw_old;
    if (screentexpos_detail_to_se_old != NULL)
      delete[] screentexpos_detail_to_se_old;
    ...
    if (edge_biome_data != NULL)
      delete[] edge_biome_data;
    if (edge_type_n != NULL)
      delete[] edge_type_n;
    if (edge_type_s != NULL)
      delete[] edge_type_s;
    ...
    if (edge_type_e != NULL)
      delete[] edge_type_e;
    if (edge_type_nw != NULL)
      delete[] edge_type_nw;
    if (edge_type_ne != NULL)
      delete[] edge_type_ne;
    if (edge_type_sw != NULL)
      delete[] edge_type_sw;
    if (edge_type_se != NULL)
      delete[] edge_type_se;
    if (edge_biome_n != NULL)
      delete[] edge_biome_n;
    if (edge_biome_s != NULL)
      delete[] edge_biome_s;
    if (edge_biome_w != NULL)
      delete[] edge_biome_w;
    if (edge_biome_e != NULL)
      delete[] edge_biome_e;
    if (edge_biome_nw != NULL)
      delete[] edge_biome_nw;
    if (edge_biome_ne != NULL)
      delete[] edge_biome_ne;
    if (edge_biome_sw != NULL)
      delete[] edge_biome_sw;
    if (edge_biome_se != NULL)
      delete[] edge_biome_se;
...
#define VIEWPORT_WALL_FLAG_TYPE_BITS                                           \
  (BIT9 | BIT10 | BIT11 | BIT12 | BIT13 | BIT14 | BIT15 | BIT16 | BIT17 |      \
   BIT18 | BIT19 | BIT20)
...
#define VIEWPORT_WALL_FLAG_TYPE_STONE_SMOOTH                                   \
  (8 << VIEWPORT_WALL_FLAG_TYPE_SHIFT)
#define VIEWPORT_WALL_FLAG_TYPE_STONE_DETAILED                                 \
  (9 << VIEWPORT_WALL_FLAG_TYPE_SHIFT)
...
#define VIEWPORT_WALL_FLAG_TYPE_WORN_STONE_1                                   \
  (22 << VIEWPORT_WALL_FLAG_TYPE_SHIFT)
#define VIEWPORT_WALL_FLAG_TYPE_WORN_STONE_2                                   \
  (23 << VIEWPORT_WALL_FLAG_TYPE_SHIFT)
#define VIEWPORT_WALL_FLAG_TYPE_WORN_STONE_3                                   \
  (24 << VIEWPORT_WALL_FLAG_TYPE_SHIFT)
...
#define VIEWPORT_WALL_FLAG_FORE_COLOR_INDEX_BITS                               \
  (BIT23 | BIT24 | BIT25 | BIT26 | BIT27 | BIT28 | BIT29 | BIT30)
#define VIEWPORT_WALL_FLAG_FORE_COLOR_INDEX_SHIFT 22
#define VIEWPORT_WALL_FLAG_BACK_COLOR_INDEX_BITS                               \
  (BIT31 | BIT32 | BIT33 | BIT34 | BIT35 | BIT36 | BIT37 | BIT38)
#define VIEWPORT_WALL_FLAG_BACK_COLOR_INDEX_SHIFT 30

enum GraphicsRampType {
  GRAPHICS_RAMP_NOTHING, // must put 0 here so ramp_flag==0 does cause trouble
                         // in derive
  GRAPHICS_RAMP_GRASS,
  GRAPHICS_RAMP_SOIL,
  GRAPHICS_RAMP_STONE,
...
#define VIEWPORT_RAMP_FLAG_TYPE                                                \
  (BIT1 | BIT2 | BIT3 | BIT4 | BIT5 | BIT6 | BIT7 | BIT8)
...
#define VIEWPORT_RAMP_FLAG_COLOR_INDEX_BITS                                    \
  (BIT27 | BIT28 | BIT29 | BIT30 | BIT31 | BIT32 | BIT33 | BIT34)
...
struct graphic_viewportst {
  ...
  ~graphic_viewportst() { clean_arrays(); }
  ...
  void clean_arrays() {
    if (screentexpos != NULL)
      delete[] screentexpos;
    if (screentexpos_background != NULL)
      delete[] screentexpos_background;
    if (screentexpos_background_two != NULL)
      delete[] screentexpos_background_two;
    if (screentexpos_floor_flag != NULL)
      delete[] screentexpos_floor_flag;
    if (screentexpos_liquid_flag != NULL)
      delete[] screentexpos_liquid_flag;
    if (screentexpos_spatter_flag != NULL)
      delete[] screentexpos_spatter_flag;
    if (screentexpos_spatter != NULL)
      delete[] screentexpos_spatter;
    if (screentexpos_ramp_flag != NULL)
      delete[] screentexpos_ramp_flag;
    if (screentexpos_shadow_flag != NULL)
      delete[] screentexpos_shadow_flag;
    if (screentexpos_building_one != NULL)
      delete[] screentexpos_building_one;
    if (screentexpos_building_two != NULL)
      delete[] screentexpos_building_two;
    if (screentexpos_top_shadow != NULL)
      delete[] screentexpos_top_shadow;
    if (screentexpos_item != NULL)
      delete[] screentexpos_item;
    if (screentexpos_vehicle != NULL)
      delete[] screentexpos_vehicle;
    if (screentexpos_projectile != NULL)
      delete[] screentexpos_projectile;
    if (screentexpos_high_flow != NULL)
      delete[] screentexpos_high_flow;
    if (screentexpos_vermin != NULL)
      delete[] screentexpos_vermin;
    if (screentexpos_signpost != NULL)
      delete[] screentexpos_signpost;
    if (screentexpos_left_creature != NULL)
      delete[] screentexpos_left_creature;
    if (screentexpos_right_creature != NULL)
      delete[] screentexpos_right_creature;
    if (screentexpos_upleft_creature != NULL)
      delete[] screentexpos_upleft_creature;
    if (screentexpos_up_creature != NULL)
      delete[] screentexpos_up_creature;
    if (screentexpos_upright_creature != NULL)
      delete[] screentexpos_upright_creature;
    if (screentexpos_designation != NULL)
      delete[] screentexpos_designation;
    if (screentexpos_interface != NULL)
      delete[] screentexpos_interface;

    if (screentexpos_old != NULL)
      delete[] screentexpos_old;
    if (screentexpos_background_old != NULL)
      delete[] screentexpos_background_old;
    if (screentexpos_background_two_old != NULL)
      delete[] screentexpos_background_two_old;
    if (screentexpos_floor_flag_old != NULL)
      delete[] screentexpos_floor_flag_old;
    if (screentexpos_liquid_flag_old != NULL)
      delete[] screentexpos_liquid_flag_old;
    if (screentexpos_spatter_flag_old != NULL)
      delete[] screentexpos_spatter_flag_old;
    if (screentexpos_spatter_old != NULL)
      delete[] screentexpos_spatter_old;
    if (screentexpos_ramp_flag_old != NULL)
      delete[] screentexpos_ramp_flag_old;
    if (screentexpos_shadow_flag_old != NULL)
      delete[] screentexpos_shadow_flag_old;
    if (screentexpos_building_one_old != NULL)
      delete[] screentexpos_building_one_old;
    if (screentexpos_building_two_old != NULL)
      delete[] screentexpos_building_two_old;
    if (screentexpos_top_shadow_old != NULL)
      delete[] screentexpos_top_shadow_old;
    if (screentexpos_item_old != NULL)
      delete[] screentexpos_item_old;
    if (screentexpos_vehicle_old != NULL)
      delete[] screentexpos_vehicle_old;
    if (screentexpos_projectile_old != NULL)
      delete[] screentexpos_projectile_old;
    if (screentexpos_high_flow_old != NULL)
      delete[] screentexpos_high_flow_old;
    if (screentexpos_vermin_old != NULL)
      delete[] screentexpos_vermin_old;
    if (screentexpos_signpost_old != NULL)
      delete[] screentexpos_signpost_old;
    if (screentexpos_left_creature_old != NULL)
      delete[] screentexpos_left_creature_old;
    if (screentexpos_right_creature_old != NULL)
      delete[] screentexpos_right_creature_old;
    if (screentexpos_upleft_creature_old != NULL)
      delete[] screentexpos_upleft_creature_old;
    if (screentexpos_up_creature_old != NULL)
      delete[] screentexpos_up_creature_old;
    if (screentexpos_upright_creature_old != NULL)
      delete[] screentexpos_upright_creature_old;
    if (screentexpos_designation_old != NULL)
      delete[] screentexpos_designation_old;
    if (screentexpos_interface_old != NULL)
      delete[] screentexpos_interface_old;

    if (core_tree_species_plus_one != NULL)
      delete[] core_tree_species_plus_one;
    if (shadow_tree_species_plus_one != NULL)
      delete[] shadow_tree_species_plus_one;
  ...
  void shape_viewport_according_by_pixel_size(int32_t px, int32_t py,
                                              int32_t zoom_factor);
...
//************************ GLIQUID more types
// can't yet do different blood colors, etc., but adding a second 32 bit liquid
// will allow another bit or two for types
...
#define SPATTER_SHAPE_FULL_ISOLATED                                            \
  1 // all fulls must be between SPATTER_SHAPE_FULL_ISOLATED and
    // SPATTER_SHAPE_FULL_E
...
#define SPATTER_SHAPE_FULL_E                                                   \
  20 // all fulls must be between SPATTER_SHAPE_FULL_ISOLATED and
     // SPATTER_SHAPE_FULL_E
...
#define VIEWPORT_SPATTER_FLAG_COLOR_INDEX_BITS                                 \
  (BIT12 | BIT13 | BIT14 | BIT15 | BIT16 | BIT17 | BIT18 | BIT19)
#define VIEWPORT_SPATTER_FLAG_COLOR_INDEX_SHIFT 11
/////////// BIT20 - BIT28 - add to derived_bits above if they are used in the
/// derivation function
...
struct interface_setst {
  ...
  int32_t texpos_button_building_sheet[INTERFACE_BUTTON_BUILDING_SHEETNUM][4]
                                      [3];
  int32_t texpos_button_unit_sheet[INTERFACE_BUTTON_UNIT_SHEETNUM][3][3];
  int32_t texpos_button_large_unit_sheet[INTERFACE_BUTTON_LARGE_UNIT_SHEETNUM]
                                        [4][3];
  int32_t texpos_button_pets_livestock[INTERFACE_BUTTON_PETS_LIVESTOCKNUM][3]
                                      [3];
  ...
  void init() {
    ...
    memset(texpos_button_picture_box_sel_highlighted, 0,
           sizeof(int32_t) * 3 * 3);
    ...
    memset(texpos_button_category_rectangle_selected, 0,
           sizeof(int32_t) * 3 * 3);
    ...
    memset(texpos_scrollbar_offcenter_scroller_hover, 0,
           sizeof(int32_t) * 2 * 2);
    ...
    memset(texpos_button_horizontal_option_inactive, 0,
           sizeof(int32_t) * 3 * 3);
...
public:
  ...
  float ccolor[16]
              [3]; // The curses-RGB mapping used for non-curses display modes
  ...
  // we assume top is not in use unless a flag is set, and reprint the screen
  // when it goes away, to avoid cell by cell checks
  bool top_in_use;
  ...
  bool main_thread_requesting_reshape; // set to true by main thread, set to
                                       // false by graphics thread
  bool main_thread_requesting_reshape_activate_map_port; // set to true by main
                                                         // thread, set to false
                                                         // by graphics thread
  ...
  int32_t workshop_carpenter_texpos[4][3][4]; // many of these use upper row
                                              // for stuff that sticks up
  ...
  */
  ...
  int32_t texpos_debris[16][2]; // this is an annoying one based on EGA colors
                                // w/ two variants
  ...
  int32_t texpos_forest_broadleaf_trop_moist_evilsav_c[REGION_SQUARE_FOREST_NUM]
                                                      [9];
  int32_t texpos_forest_broadleaf_trop_moist_evil_c[REGION_SQUARE_FOREST_NUM]
                                                   [9];
  int32_t texpos_forest_broadleaf_trop_moist_goodsav_c[REGION_SQUARE_FOREST_NUM]
                                                      [9];
  int32_t texpos_forest_broadleaf_trop_moist_good_c[REGION_SQUARE_FOREST_NUM]
                                                   [9];
  int32_t texpos_forest_broadleaf_trop_dry_c[REGION_SQUARE_FOREST_NUM][9];
  int32_t texpos_forest_broadleaf_trop_dry_evilsav_c[REGION_SQUARE_FOREST_NUM]
                                                    [9];
  int32_t texpos_forest_broadleaf_trop_dry_evil_c[REGION_SQUARE_FOREST_NUM][9];
  int32_t texpos_forest_broadleaf_trop_dry_goodsav_c[REGION_SQUARE_FOREST_NUM]
                                                    [9];
  ...
  int32_t texpos_world_edge_shape_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_world_edge_shape_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_world_edge_shape_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_world_edge_shape_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_world_edge_shape_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_world_edge_shape_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_world_edge_shape_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_world_edge_shape_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_grass_temp_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_grass_temp_evil_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_temp_evil_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_temp_evil_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_temp_evil_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_grass_temp_evil_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_evil_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_evil_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_evil_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_evil_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_evil_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_evil_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_evil_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_grass_temp_good_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_temp_good_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_temp_good_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_temp_good_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_grass_temp_good_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_good_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_good_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_good_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_good_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_good_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_good_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_temp_good_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_grass_trop_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_grass_trop_evil_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_trop_evil_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_trop_evil_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_trop_evil_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_grass_trop_evil_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_evil_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_evil_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_evil_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_evil_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_evil_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_evil_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_evil_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_grass_trop_good_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_trop_good_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_trop_good_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_grass_trop_good_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_grass_trop_good_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_good_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_good_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_good_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_good_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_good_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_good_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_grass_trop_good_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_wetland_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_wetland_evil_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_evil_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_evil_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_evil_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_evil_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_evil_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_evil_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_evil_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_wetland_good_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_good_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_good_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_good_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_good_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_good_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_good_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_wetland_good_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_beach_thin_corner_transition_nw_n[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_beach_thin_corner_transition_nw_w[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_beach_thin_corner_transition_ne_n[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_beach_thin_corner_transition_ne_e[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_beach_thin_corner_transition_sw_s[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_beach_thin_corner_transition_sw_w[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_beach_thin_corner_transition_se_s[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_beach_thin_corner_transition_se_e[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_beach_evil_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_evil_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_evil_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_evil_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_evil_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_evil_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_evil_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_evil_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_beach_good_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_good_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_good_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_good_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_good_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_good_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_good_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_good_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_desert_thin_corner_transition_nw_n[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_desert_thin_corner_transition_nw_w[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_desert_thin_corner_transition_ne_n[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_desert_thin_corner_transition_ne_e[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_desert_thin_corner_transition_sw_s[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_desert_thin_corner_transition_sw_w[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_desert_thin_corner_transition_se_s[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_desert_thin_corner_transition_se_e[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_desert_evil_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_evil_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_evil_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_evil_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_evil_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_evil_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_evil_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_evil_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_desert_good_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_good_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_good_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_good_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_good_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_good_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_good_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_desert_good_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_sand_desert_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_sand_desert_yellow_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thin_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thin_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thin_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thin_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_sand_desert_yellow_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_yellow_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_yellow_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_yellow_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_yellow_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_yellow_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_yellow_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_yellow_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_tiny_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_tiny_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_tiny_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_tiny_corner_se[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thinning_n_w[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thinning_n_e[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thinning_s_w[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thinning_s_e[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thinning_w_n[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thinning_w_s[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thinning_e_n[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_yellow_thinning_e_s[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_sand_desert_white_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_thin_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_thin_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_thin_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_thin_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_sand_desert_white_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_white_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_white_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_white_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_white_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_white_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_white_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_white_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_tiny_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_tiny_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_tiny_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_white_tiny_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_sand_desert_black_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_thin_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_thin_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_thin_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_thin_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_sand_desert_black_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_black_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_black_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_black_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_black_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_black_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_black_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_black_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_tiny_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_tiny_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_tiny_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_black_tiny_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_sand_desert_red_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_red_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_red_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_red_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_sand_desert_red_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_red_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_red_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_red_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_red_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_red_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_red_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_red_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_sand_desert_evil_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_thin_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_thin_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_thin_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_thin_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_sand_desert_evil_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_evil_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_evil_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_evil_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_evil_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_evil_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_evil_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_evil_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_tiny_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_tiny_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_tiny_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_evil_tiny_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_sand_desert_good_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_thin_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_thin_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_thin_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_thin_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_sand_desert_good_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_good_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_good_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_good_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_good_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_good_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_good_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_sand_desert_good_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_tiny_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_tiny_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_tiny_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_sand_desert_good_tiny_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_tundra_thin_corner_transition_nw_n[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_tundra_thin_corner_transition_nw_w[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_tundra_thin_corner_transition_ne_n[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_tundra_thin_corner_transition_ne_e[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_tundra_thin_corner_transition_sw_s[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_tundra_thin_corner_transition_sw_w[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_tundra_thin_corner_transition_se_s[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_tundra_thin_corner_transition_se_e[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_tundra_evil_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_evil_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_evil_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_evil_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_evil_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_evil_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_evil_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_evil_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_tundra_good_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_good_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_good_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_good_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_good_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_good_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_good_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_tundra_good_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_glacier_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_glacier_evil_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evil_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evil_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evil_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evil_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evil_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evil_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evil_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_glacier_good_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_good_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_good_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_good_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_good_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_good_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_good_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_good_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_glacier_evilsav_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_glacier_evilsav_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_glacier_evilsav_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_glacier_evilsav_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_glacier_evilsav_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evilsav_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evilsav_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evilsav_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evilsav_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evilsav_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evilsav_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_evilsav_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_glacier_goodsav_thick_corner_nw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_glacier_goodsav_thick_corner_ne[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_glacier_goodsav_thick_corner_sw[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_glacier_goodsav_thick_corner_se[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_glacier_goodsav_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_goodsav_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_goodsav_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_goodsav_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_goodsav_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_goodsav_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_goodsav_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_glacier_goodsav_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t
      texpos_edge_water_thin_corner_transition_nw_n[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_water_thin_corner_transition_nw_w[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_water_thin_corner_transition_ne_n[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_water_thin_corner_transition_ne_e[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_water_thin_corner_transition_sw_s[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_water_thin_corner_transition_sw_w[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_water_thin_corner_transition_se_s[REGION_SQUARE_VARIANT_NUM];
  int32_t
      texpos_edge_water_thin_corner_transition_se_e[REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_water_evil_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_evil_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_evil_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_evil_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_evil_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_evil_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_evil_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_evil_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_water_good_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_good_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_good_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_good_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_good_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_good_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_good_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_water_good_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  int32_t texpos_edge_beach_line_thin_corner_transition_nw_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_line_thin_corner_transition_nw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_line_thin_corner_transition_ne_n
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_line_thin_corner_transition_ne_e
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_line_thin_corner_transition_sw_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_line_thin_corner_transition_sw_w
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_line_thin_corner_transition_se_s
      [REGION_SQUARE_VARIANT_NUM];
  int32_t texpos_edge_beach_line_thin_corner_transition_se_e
      [REGION_SQUARE_VARIANT_NUM];
  ...
  //*********************** INDIV TREE definers
  // need the data structure for special floor and top shadow textures here,
  // stores texpos and also the definer definer is each 8 directions and the
  // species of tree index
  ...
  graphicst() {
    ...
    int32_t tt;
    for (tt = 0; tt < LOWER_VIEWPORT_MAX; ++tt)
      multilevel_depth_texpos[tt] = 0;
    print_index = 0;
    display_frames = 0;
    rect_id = -1;
    ...
    main_viewport = NULL;
    int32_t lvp;
    for (lvp = 0; lvp < LOWER_VIEWPORT_MAX; ++lvp)
      lower_viewport[lvp] = NULL;
  ...
  // Calling this is not enough in itself. You also need to call
  // swap_front/back.
  ...
  void addchar(unsigned int x, unsigned int y, unsigned char c, unsigned char f,
  ...
  void addst(const string& str, justification just = justify_left,
             int space = 0);
  void addst_flag(const string& str_orig, justification just, int space = 0,
                  uint32_t sflag = 0);
  void top_addst(const string& str, justification just = justify_left,
                 int space = 0);
  void top_addst_flag(const string& str_orig, justification just, int space = 0,
                      uint32_t sflag = 0);
  ...
  void add_anchored_tile(long texp, long offset_x, long offset_y, long tdx,
                         long tdy, bool use_color);

  void add_top_tile(long texp, char addcolor);
  void add_top_lower_tile(long texp);
  void add_top_anchored_tile(long texp, long offset_x, long offset_y, long tdx,
                             long tdy, bool use_color);

  void prepare_graphics(const string& src_dir);
  void create_derived_textures();
  void copy_transformed_pixel_data(SDL_Surface* src, SDL_Surface* dst,
                                   int32_t cw_rotate, bool flip_vert);
  void copy_pixel_data_with_alpha_mask(SDL_Surface* src, SDL_Surface* alpha_src,
                                       SDL_Surface* dst);
  ...
  void color_square(long x, long y, unsigned char f, unsigned char b,
                    unsigned char br);
  ...
  void delete_all_viewports() {
    ...
    main_viewport = NULL;
    int32_t v;
    for (v = 0; v < LOWER_VIEWPORT_MAX; ++v)
      lower_viewport[v] = NULL;
