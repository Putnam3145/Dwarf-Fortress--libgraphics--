...
SDL_Texture* cached_texturest::get_texture() {
  if (std::holds_alternative<SDL_Surface*>(tex)) {
    if (enabler.main_renderer()) {
      tex = SDL_CreateTextureFromSurface(enabler.main_renderer(),
                                         std::get<SDL_Surface*>(tex));
    } else {
      return NULL;
    }
  }
...
SDL_Surface* textures::get_texture_data(long pos) {
  if (raws.size() > pos) {
    ...
    SDL_Surface *surf = SDL_CreateRGBSurface(SDL_SWSURFACE, 8, 8, 32, 0, 0, 0,
    0); SDL_FillRect(surf, NULL, SDL_MapRGB(surf->format, 255, 0, 255));
    raws.resize(pos+1);
    raws[pos] = surf;
    return raws[pos];
  */
...
long textures::create_texture(long dimx, long dimy) {
  ...
  SDL_Surface* tile = SDL_CreateRGBSurface(SDL_SWSURFACE, dimx, dimy, 32, rmask,
                                           gmask, bmask, amask);
...
long textures::clone_texture(long src) {
  ...
  if (raws.size() > src && raws[src]) {
    SDL_Surface* dst =
        SDL_ConvertSurface(raws[src], raws[src]->format, SDL_SWSURFACE);
    tx = add_texture(dst);
  } else {
...
void textures::grayscale_texture(long pos) {
  ...
  for (int i = 0; i < s->w * s->h; i++) { // For each pixel
    ...
    int luminosity =
        (int)((float)r * 0.30f + (float)g * 0.59f + (float)b * 0.11f);
    if (luminosity < 0)
      luminosity = 0;
    if (luminosity > 255)
      luminosity = 255;
    pixels[i] = (luminosity << f->Rshift) | (luminosity << f->Gshift) |
                (luminosity << f->Bshift) | (alpha << f->Ashift);
  }
...
void textures::load_multi_pdim(const string& filename, long* tex_pos, long dimx,
  ...
  if (!raw) {
    MessageBox(NULL, ("Not found: " + filename).c_str(), "Tileset not found",
               MB_OK);
    exit(1);
  }
  ...
  for (int y = 0; y < dimy; y++) {
    for (int x = 0; x < dimx; x++, ++idx) {
      SDL_Surface* tile = SDL_CreateRGBSurface(
          SDL_SWSURFACE, *disp_x, *disp_y, 32, src->format->Rmask,
...
void textures::load_multi_pdim(const string& filename, svector<long>& tex_pos,
...
void textures::refresh_multi_pdim(const string& filename,
                                  svector<long>& tex_pos, long dimx, long dimy,
...
cached_texturest textures::load(const string& filename, bool convert_magenta) {
  SDL_Surface* raw = IMG_Load(filename.c_str());
  if (!raw) {
    MessageBox(NULL, ("Not found: " + filename).c_str(), "Image not found",
               MB_OK);
    exit(1);
  }
