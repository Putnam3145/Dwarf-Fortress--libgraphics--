...
void KeybindingScreen::feed(set<InterfaceKey>& input) {
  ...
  if (input.count(INTERFACEKEY_KEYBINDING_COMPLETE)) {
    ...
    if (keys.size() == 0) {
      ...
      keyRegister.clear();
      list<RegisteredKey> keys = enabler.getRegisteredKey();
      for (list<RegisteredKey>::iterator it = keys.begin(); it != keys.end();
           ++it) {
        string display;
        switch (it->type) {
        case type_button:
  ...
  } else if (mode == mode_keyL &&
             input.count(INTERFACEKEY_STANDARDSCROLL_RIGHT))
    mode = mode_keyR;
    ...
    mode = mode_keyL;
  else if ((mode == mode_keyL || mode == mode_macro) &&
           input.count(INTERFACEKEY_STANDARDSCROLL_LEFT))
    mode = mode_main;
  else if (input.count(
               INTERFACEKEY_STRING_A000)) { // Backspace: Delete something.
    switch (mode) {
    case mode_macro:
    ...
    case mode_register:
      enabler.bindRegisteredKey(keyRegister.get_selection(),
                                keyL.get_selection());
      mode = mode_keyR;
      reset_keyR();
      break;
             ...
             input.count(INTERFACEKEY_OPTIONS)) {
...
void KeybindingScreen::reset_keyR() {
  ...
  int i = 6;
  for (list<EventMatch>::iterator it = matchers.begin(); it != matchers.end();
       ++it, ++i) {
    ostringstream desc;
    switch (it->type) {
    case type_unicode:
      ...
      break;
    case type_key:
      desc << "By position: " << translate_mod(it->mod)
           << sdlNames.left[it->key];
      break;
    case type_button:
...
void KeybindingScreen::render_macro() {
  ...
  gps.addst("Select a macro, then press " +
            enabler.GetKeyDisplay(INTERFACEKEY_STRING_A000) + " to delete.");
...
void KeybindingScreen::render_key() {
  ...
  gps.locate(3, 6);
  gps.changecolor(4, 0, 1);
  gps.addst("Select a binding, then press " +
            enabler.GetKeyDisplay(INTERFACEKEY_STRING_A000) + " to delete.");
  ...
  if (mode == mode_keyL || mode == mode_register)
    ...
    keyR.bleach(false);
  keyR.render(init.display.grid_x / 2 + 1, init.display.grid_x - 2, 5,
              init.display.grid_y - 2);
}

void KeybindingScreen::render_register() {
  int x1 = init.display.grid_x / 2 - 20, x2 = init.display.grid_x / 2 + 20,
      y1 = init.display.grid_y / 2 - 1, y2 = init.display.grid_y / 2 + 1;
  if (!enabler.is_registering()) {
  ...
  if (enabler.is_registering()) {
    ...
    gps.addst("Select binding, or press " +
              enabler.GetKeyDisplay(INTERFACEKEY_LEAVESCREEN) + " to abort");
  }
}
...
void MacroScreenSave::render() {
  if (parent)
    parent->render();
  const int x1 = 5, x2 = init.display.grid_x - 4,
            y1 = init.display.grid_y / 2 - 1, y2 = init.display.grid_y / 2 + 2;
