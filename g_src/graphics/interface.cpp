...
inline void CHECK_ERR(int err, const char* msg) {
  if (err != Z_OK) {
    MessageBox(NULL,
               "One of the compressed files on disk has errors in it.  Restore "
               "from backup if you are able.",
               0, 0);
    exit(1);
  }
}
...
viewscreen_movieplayerst* viewscreen_movieplayerst::create(char pushtype,
                                                           viewscreenst* scr) {
...
void viewscreen_movieplayerst::logic() {
  ...
  if (!force_file.empty() && !is_playing && !quit_if_no_play &&
      is_forced_play) {
    is_playing = 1;
    quit_if_no_play = 1;
  ...
  if (is_playing) {
    if (gview.currentblocksize > 0) {
      ...
      for (x2 = 0; x2 < init.display.grid_x; x2++) {
        for (y2 = 0; y2 < init.display.grid_y; y2++, ++curp) {
          ...
          gps.changecolor(
              (gview.supermoviebuffer[curp + half_frame_size] & 7),
    ...
    if (editing_menu) {
      ...
      for (ty = 0; ty < 16; ty++) {
        for (tx = 0; tx < 16; tx++) {
          gps.locate(ty, tx);
          gps.changecolor(editing_screenf, editing_screenb,
      ...
      gps.locate(20, 0);
      string str;
      gps.addst("1/100 sec per frame: ");
      convert_long_to_string(gview.supermovie_delayrate, str);
      gps.addst(str);

      int scroll = (editing_selected_sound / 25) * 25;
      int e;
      for (e = scroll; e < scroll + 25 && e < gview.supermovie_sound.str.size();
           e++) {
        if (e == editing_selected_sound)
          gps.changecolor(7, 0, 1);
        else
          gps.changecolor(7, 0, 0);
      ...
      for (e = 0; e < SOUND_CHANNELNUM; e++) {
        ...
        if (sd >= 0 && sd < gview.supermovie_sound.str.size())
          gps.addst(gview.supermovie_sound.str[sd]->dat);
        else {
          gps.addst("-----------------");
        }
    ...
    gps.addst(
        ": Start recording (active record is erased, stops when you return "
        "here)");
    gps.locate(3, 2);
...
void viewscreen_movieplayerst::feed(std::set<InterfaceKey>& events) {
  if (events.count(INTERFACEKEY_LEAVESCREEN)) {
    ...
    standardstringentry(savename, 39,
                        STRINGENTRY_LETTERS | STRINGENTRY_SPACE |
                            STRINGENTRY_NUMBERS | STRINGENTRY_SYMBOLS,
                        events);
    ...
    if (editing_menu) {
      if (enabler.mouse_lbut && enabler.tracking_on) {
        ...
        if (sx >= 0 && sx < 80 && sy >= 0 && sy < 25) {
          ...
          if (sx == (init.display.grid_x - 1) && sy >= 0 &&
              sy < SOUND_CHANNELNUM) {
      ...
      if (editing_selected_sound >= 0 &&
          editing_selected_sound < gview.supermovie_sound.str.size()) {
        if (gview.c == '%') {
          delete gview.supermovie_sound.str[editing_selected_sound];
          gview.supermovie_sound.str.erase(editing_selected_sound);
        } else {
          standardstringentry(
              events, gview.supermovie_sound.str[editing_selected_sound]->dat,
              26,
              STRINGENTRY_LETTERS | STRINGENTRY_SPACE | STRINGENTRY_NUMBERS |
                  STRINGENTRY_SYMBOLS);
          entering = 1;
        }
      } else {
        if (gview.c == '#')
          gview.supermovie_sound.add_string("new_sound");
        if (gview.c == '+')
          gview.supermovie_delayrate++;
        if (gview.c == '-')
          gview.supermovie_delayrate--;
        if (gview.c == 'T')
          text_mode = 1 - text_mode;
      }
      if (gview.supermovie_delayrate < 0)
        gview.supermovie_delayrate = 0;
      if (gview.supermovie_delayrate > 10)
        gview.supermovie_delayrate = 10;
    } else {
      if (text_mode) {
        if (gview.c != 0) {
          int sx, sy;
          ...
          if (sx >= 0 && sx < 80 && sy >= 0 && sy < 25) {
            ...
            gview.supermoviebuffer[curpos + 2000] =
                (editing_screenf & 7) + ((editing_screenb & 7) << 3);
            if (editing_screenbright)
              gview.supermoviebuffer[curpos + 2000] += 64;
          }
        }
      } else {
        if (gview.c == 'a') {
          ...
          for (x2 = 0; x2 < 80; x2++) {
            for (y2 = 0; y2 < 25; y2++) {
              if (x2 > 0) {
                gview.supermoviebuffer[gview.supermovie_pos + y2 +
                                       (x2 - 1) * 25] =
                    gview.supermoviebuffer[gview.supermovie_pos + y2 + x2 * 25];
                gview.supermoviebuffer[gview.supermovie_pos + y2 +
                                       (x2 - 1) * 25 + 2000] =
                    gview.supermoviebuffer[gview.supermovie_pos + y2 + x2 * 25 +
                                           2000];
              ...
              if (x2 == (init.display.grid_x - 1))
                gview.supermoviebuffer[gview.supermovie_pos + y2 + x2 * 25] = 0;
            }
          }
        }
        if (gview.c == 'd') {
        ...
        if (gview.c == 'E')
          end_frame_pos = gview.supermovie_pos;
        if (gview.c == 'c')
          editing_copy_from = gview.supermovie_pos;
        if (gview.c == 'p') {
          int i;
          for (i = 0; i < 4000; i++) {
            gview.supermoviebuffer[gview.supermovie_pos + i] =
                gview.supermoviebuffer[editing_copy_from + i];
          }
        }
        if (gview.c == '+')
          gview.supermovie_pos += 4000;
        if (gview.c == '-')
          gview.supermovie_pos -= 4000;
        if (gview.c == '/')
          gview.supermovie_pos -= 40000;
        if (gview.c == '*')
          gview.supermovie_pos += 40000;
        if (gview.supermovie_pos < 0)
          gview.supermovie_pos = 0;
        if (gview.supermovie_pos >= MOVIEBUFFSIZE)
          gview.supermovie_pos = MOVIEBUFFSIZE - 4000;
      }
...
void interfacest::addscreen(viewscreenst* scr, char pushtype,
                            viewscreenst* relate) {
...
void interfacest::insertscreen_as_parent(viewscreenst* scr,
                                         viewscreenst* child) {
  if (child == NULL) {
    insertscreen_at_back(scr);
    return;
...
void interfacest::insertscreen_as_child(viewscreenst* scr,
                                        viewscreenst* parent) {
  if (parent == NULL) {
    insertscreen_at_back(scr);
    return;
...
char interfacest::loop() {
  ...
  case INTERFACE_BREAKDOWN_NONE: {
    ...
    // Process as much input as possible. Some screens can't handle multiple
    // input events per logic call (retain_nonzero_input, and any alteration
    // to the window setup requires us to stop until the next logic call.
    for (;;) {
      if (currentscreen->child ||
          currentscreen->breakdownlevel != INTERFACE_BREAKDOWN_NONE)
        break; // Some previous input or logic had the effect of switching
               // screens
      ...
      if (flag & INTERFACEFLAG_RETAIN_NONZERO_INPUT) {
        ...
        if (era.size() == 0) {
          if (enabler.mouse_lbut || enabler.mouse_rbut || enabler.mouse_mbut ||
              enabler.get_text_input()[0] != '\0')
        ...
        if (era.count(INTERFACEKEY_MOVIES) &&
            !currentscreen->key conflict(INTERFACEKEY_MOVIES))
          if (currentscreen->movies_okay())
            use_movie_input();
#endif
        // Prefix commands
        // Most prefix commands we don't want to touch game management
        // commands, i.e. what's in here. Macro playback is a notable
        // exception.
        if (era.count(INTERFACEKEY_PREFIX))
          enabler.prefix_toggle();
        int repeats = 1; // If this input ends a prefix command, we'll want
                         // to repeat it.
        if (enabler.prefix_building()) {
          // TODO: OMGWTFBBQ
          char c = 0;
          if (era.count(INTERFACEKEY_STRING_A048))
            c = '0';
          else if (era.count(INTERFACEKEY_STRING_A049))
            c = '1';
          else if (era.count(INTERFACEKEY_STRING_A050))
            c = '2';
          else if (era.count(INTERFACEKEY_STRING_A051))
            c = '3';
          else if (era.count(INTERFACEKEY_STRING_A052))
            c = '4';
          else if (era.count(INTERFACEKEY_STRING_A053))
            c = '5';
          else if (era.count(INTERFACEKEY_STRING_A054))
            c = '6';
          else if (era.count(INTERFACEKEY_STRING_A055))
            c = '7';
          else if (era.count(INTERFACEKEY_STRING_A056))
            c = '8';
          else if (era.count(INTERFACEKEY_STRING_A057))
            c = '9';
...
void interfacest::removescreen(viewscreenst* scr) {
  // THE MINIMAP IS EXPENSIVE, SO WE REFRESH IT WHENEVER INTERFACE GETS IN THE
  // WAY
  if (gamemode == GAMEMODE_DWARF)
    dwarf_remove_screen();
...
int interfacest::write_movie_chunk() {
  ...
  while (c_stream.total_in != inputsize &&
         c_stream.total_out < COMPMOVIEBUFFSIZE) {
  ...
  if (f.is_open()) {
    ...
    if (gview.nextfilepos < file_size) {
      ...
      if (!fail) {
        ...
        while (d_stream.total_out < MOVIEBUFFSIZE &&
               d_stream.total_in < gview.currentblocksize) {
...
void interfacest::print_interface_token(InterfaceKey key, justification just) {
  short o_screenf = gps.screenf, o_screenb = gps.screenb,
        o_screenbright = gps.screenbright;
  gps.changecolor(2, 0, 1);
...
char standardstringentry(string& str, int maxlen, unsigned int flag,
                         std::set<InterfaceKey>& events) {
  char r =
      standardstringentry(str, maxlen, flag, events, enabler.get_text_input());
  if (r)
...
char standardstringentry(string& str, int maxlen, unsigned int flag,
  ...
  if (events.count(INTERFACEKEY_STRING_A000) && str.length() > 0) {
    ...
    if (events.count(INTERFACEKEY_SELECT) ||
        events.count(INTERFACEKEY_LEAVESCREEN) || enabler.mouse_rbut) {
    ...
    for (size_t i = 0; i < 32; i++) {
      ...
      if (entry == '\0')
        break;
      if (str.length() < maxlen && (entry == 0xA) ||
          (!(flag & STRINGENTRY_FILENAME) ||
           invalid_filename_chars.count(entry) == 0) ||
          (flag & STRINGENTRY_SYMBOLS) ||
          ((flag & STRINGENTRY_LETTERS) && (entry >= 'a' && entry <= 'z') ||
           (entry >= 'A' && entry <= 'Z')) ||
        ...
        any_valid = true;
        str.push_back(entry);
        if (str.length() >= maxlen || entry == '\0' || entry == 0xA)
          break;
      }
    }
    return any_valid;
