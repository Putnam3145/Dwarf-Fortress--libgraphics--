...
// some of this stuff is based on public domain code from nehe or opengl books
// over the years additions and modifications Copyright (c) 2008, Tarn Adams All
// rights reserved.  See game.cpp or license.txt for more information.
...
public:
  ...
  bool has_flag(long checkflag) {
    ...
    return (slot >= 0 && slot < slotnum &&
            ((array[slot] & (1 << (checkflag & 7))) != 0));
  }
 ...
 public:
  ...
  GLuint upload(GLenum type) {
    ...
    if (status == GL_FALSE) { // ..no. Check the compilation log.
      ...
      //errorlog << filename << " shader compilation log (" << log_size << "):"
<< std::endl; std::cerr << filename << " shader compilation log (" << log_size
<< "):" << std::endl; char *buf = new char[log_size]; glGetShaderInfoLog(shader,
log_size, NULL, buf);
      //errorlog << buf << std::endl;
      std::cerr << buf << std::endl;
      //errorlog.flush();
      delete[] buf;
      MessageBox(NULL, "Shader compilation failed; details in errorlog.txt",
"Critical error", MB_OK); abort();
    }
    printGLError();
    return shader;
  }
...
public:
  ...
  void write_file(file_compressorst& filecomp) { text.write_file(filecomp); }

  void clean() { text.clean(); }
};
...
template <> struct hash<ttf_id> {
  size_t operator()(ttf_id val) const {
    // Not the ideal hash function, but it'll do. And it's better than GCC's.
    // id? Seriously?
...
protected:
  ...
  void gps_allocate(int x, int y, int screen_x, int screen_y, int tile_dim_x,
                    int tile_dim_y);
  Either<texture_fullid, int32_t /*texture_ttfid*/> screen_to_texid(int x,
                                                                    int y);
  Either<texture_fullid, int32_t /*texture_ttfid*/> screen_top_to_texid(int x,
                                                                        int y);
...
public:
  ...
  virtual void update_viewport_tile(graphic_viewportst* vp, int32_t x,
                                    int32_t y) = 0;
  virtual void update_map_port_tile(graphic_map_portst* vp, int32_t x,
                                    int32_t y) = 0;
  ...
  virtual void clean_cached_tile(int32_t texpos, float r, float g, float b,
  ...
  virtual ~renderer() { cleanup_arrays(); }

  virtual bool get_precise_mouse_coords(int& px, int& py, int& x, int& y) = 0;
  virtual void get_current_interface_tile_dims(int32_t& cur_tx,
                                               int32_t& cur_ty) = 0;

  virtual bool uses_opengl() { return false; };
};

enum FullscreenState : uint8_t { FULLSCREEN = BIT1, EXCLUSIVE = BIT2 };
...
class enablerst : public enabler_inputst {
  ...
  queue<int> frame_timings,
      gframe_timings;          // Milisecond lengths of the last few frames
...
private:
  ...
  struct async_cmd {
    ...
    int val; // If async_inc, number of extra frames to run. If set_fps,
             // current value of fps.
  ...
  unsigned int
      async_frames; // Number of frames the async thread has been asked to run
  bool async_paused;
  Chan<async_cmd> async_tobox;    // Messages to the simulation thread
  Chan<async_msg> async_frombox;  // Messages from the simulation thread, and
                                  // acknowledgements of those to
  Chan<zoom_commands> async_zoom; // Zoom commands (from the simulation thread)
  std::binary_semaphore async_fromcomplete; // Barrier for async_msg requests
...
public:
  ...
  char mouse_lbut, mouse_rbut, mouse_lbut_down, mouse_rbut_down,
      mouse_lbut_lift, mouse_rbut_lift;
  ...
  Uint32 clock; // An *approximation* of the current time for use in garbage
                // collection thingies, updated every frame or so.
  bool mouse_focus;
