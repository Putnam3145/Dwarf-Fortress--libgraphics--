...
int32_t convert_raw_to_ascii_texpos(uint8_t tile, uint8_t color_f,
                                    uint8_t color_b, uint8_t color_br);
...
void initst::begin() {
  ...
  for (fl = 0; fl < 2; ++fl) {
    std::ifstream fseed;
    if (fl == 0)
      fseed.open("data/init/init_default.txt");
    else
      fseed.open("prefs/init.txt");
    if (fseed.is_open()) {
      string str;
      ...
      while (std::getline(fseed, str)) {
        if (str.length() > 1) {
          ...
          if (!token.compare("WINDOWEDX")) {
            ...
            if (display.desired_windowed_width < MINIMUM_WINDOW_WIDTH)
              display.desired_windowed_width = MINIMUM_WINDOW_WIDTH;
          }
          if (!token.compare("WINDOWEDY")) {
            display.desired_windowed_height = convert_string_to_long(token2);
            if (display.desired_windowed_height < MINIMUM_WINDOW_HEIGHT)
              display.desired_windowed_height = MINIMUM_WINDOW_HEIGHT;
          }
          if (!token.compare("RESIZABLE")) {
            if (token2 == "NO")
              display.flag.add_flag(INIT_DISPLAY_FLAG_NOT_RESIZABLE);
            else
              display.flag.remove_flag(INIT_DISPLAY_FLAG_NOT_RESIZABLE);
          }
          ...
          if (!token.compare("MAXIMUM_INTERFACE_PERCENTAGE")) {
            ...
            if (display.max_interface_percentage > 100)
              display.max_interface_percentage = 100;
            if (display.max_interface_percentage < 1)
              display.max_interface_percentage = 1;
          }
          ...
          if (!token.compare("INTERFACE_SCALING_TO_DESIRED_GRID")) {
            if (token2 == "YES") {
              display.flag.add_flag(
                  INIT_DISPLAY_FLAG_INTERFACE_SCALING_TO_DESIRED_HEIGHT_WIDTH);
            } else
              display.flag.remove_flag(
                  INIT_DISPLAY_FLAG_INTERFACE_SCALING_TO_DESIRED_HEIGHT_WIDTH);
          }
          if (!token.compare("INTERFACE_SCALING_DESIRED_GRID_WIDTH")) {
            display.interface_scaling_desired_width =
                convert_string_to_long(token2);
            if (display.interface_scaling_desired_width < MIN_GRID_X)
              display.interface_scaling_desired_width = MIN_GRID_X;
            if (display.interface_scaling_desired_width > MAX_GRID_X)
              display.interface_scaling_desired_width = MAX_GRID_X;
          }
          if (!token.compare("INTERFACE_SCALING_DESIRED_GRID_HEIGHT")) {
            display.interface_scaling_desired_height =
                convert_string_to_long(token2);
            if (display.interface_scaling_desired_height < MIN_GRID_Y)
              display.interface_scaling_desired_height = MIN_GRID_Y;
            if (display.interface_scaling_desired_height > MAX_GRID_Y)
              display.interface_scaling_desired_height = MAX_GRID_Y;
          }
          if (!token.compare("INTERFACE_SCALING_PERCENTAGE")) {
            display.interface_scaling_percentage =
                convert_string_to_long(token2);
            if (display.interface_scaling_percentage < 100)
              display.interface_scaling_percentage = 100;
            if (display.interface_scaling_percentage > 400)
              display.interface_scaling_percentage = 400;
          }
          if (token == "WINDOWED") {
            if (token2 == "YES") {
          ...
          if (!token.compare("SOUND")) {
            if (token2 != "YES") {
              ...
              media.flag.remove_flag(INIT_MEDIA_FLAG_SOUND_OFF);
          }
          ...
          if (!token.compare("AVERAGE_TIME_BETWEEN_SONGS")) {
            ...
            if (media.time_between_songs < 10)
              media.time_between_songs = 10;
            if (media.time_between_songs > 600)
              media.time_between_songs = 600;
          }
          ...
          if (token == "COMPRESSED_SAVES") {
            if (token2 == "YES") {
              ...
              media.flag.remove_flag(INIT_MEDIA_FLAG_COMPRESS_SAVES);
          }
  ...
  if (!display.flag.has_flag(INIT_DISPLAY_FLAG_TEXT)) {
    if (enabler.command_line.empty()) {
      if (display.windowed == INIT_DISPLAY_WINDOW_TRUE) {
        ...
        if (MessageBox(
                NULL,
                "Run in Fullscreen Mode?  You can set your preferences in "
                "data/init/init.txt.\rUnless you've changed your bindings, you "
                "can press F11 to toggle this setting any time.",
  ...
  enabler.textures.load_multi_pdim(basic_font, font.basic_font_texpos, 16, 16,
                                   true, &font.basic_font_dispx,
                                   &font.basic_font_dispy);
  enabler.textures.load_multi_pdim(small_font, font.small_font_texpos, 16, 16,
                                   true, &font.small_font_dispx,
                                   &font.small_font_dispy);
  enabler.textures.load_multi_pdim(large_font, font.large_font_texpos, 16, 16,
                                   true, &font.large_font_dispx,
                                   &font.large_font_dispy);
  ...
  gps.tex[TEXTURE_PUBLISHER] =
      enabler.textures.load("data/art/pixel_kf.png", true);
  gps.tex[TEXTURE_PUBLISHER_SMALL] =
      enabler.textures.load("data/art/pixel_kf_small.png", true);
  gps.tex[TEXTURE_PUBLISHER_TINY] =
      enabler.textures.load("data/art/pixel_kf_tiny.png", true);
  gps.tex[TEXTURE_TITLE] = enabler.textures.load("data/art/df_logo.png", true);
  gps.tex[TEXTURE_TITLE_BACKGROUND] =
      enabler.textures.load("data/art/title_background.png", true);
  gps.tex[TEXTURE_DEVELOPER] =
      enabler.textures.load("data/art/bay12.png", true);
  gps.tex[TEXTURE_DEVELOPER_SMALL] =
      enabler.textures.load("data/art/bay12_small.png", true);
  gps.tex[TEXTURE_DEVELOPER_TINY] =
      enabler.textures.load("data/art/bay12_tiny.png", true);
  gps.tex[TEXTURE_SOUND_SYSTEM] =
      enabler.textures.load("data/art/fmod.png", true);
#endif

  long d3, d4;
  enabler.textures.load_multi_pdim("data/art/load_bar.png", load_bar_texpos, 6,
                                   1, true, &d3, &d4);
  int32_t intro_button_total = (INIT_INTRO_BUTTON_TEXTURENUM / 9) * 3;
  enabler.textures.load_multi_pdim("data/art/intro_button.png",
                                   intro_button_texpos, intro_button_total, 3,
                                   true, &d3, &d4);
  int32_t dx, dy;
  for (dy = 0; dy < 3; ++dy) {
    for (dx = 0; dx < 3; ++dx) {
      texpos_neutral_intro_button[dx + dy * 3] =
          intro_button_texpos[dx + dy * intro_button_total];
      texpos_confirm_intro_button[dx + dy * 3] =
          intro_button_texpos[dx + 1 * 3 + dy * intro_button_total];
      texpos_cancel_intro_button[dx + dy * 3] =
          intro_button_texpos[dx + 2 * 3 + dy * intro_button_total];
      texpos_selected_intro_button[dx + dy * 3] =
          intro_button_texpos[dx + 3 * 3 + dy * intro_button_total];
      texpos_unselected_intro_button[dx + dy * 3] =
          intro_button_texpos[dx + 4 * 3 + dy * intro_button_total];
      texpos_open_list_button[dx + dy * 3] =
          intro_button_texpos[dx + 5 * 3 + dy * intro_button_total];
      texpos_increase_button[dx + dy * 3] =
          intro_button_texpos[dx + 6 * 3 + dy * intro_button_total];
      texpos_decrease_button[dx + dy * 3] =
          intro_button_texpos[dx + 7 * 3 + dy * intro_button_total];
      texpos_nullify_button[dx + dy * 3] =
          intro_button_texpos[dx + 8 * 3 + dy * intro_button_total];
      texpos_left_arrow_button[dx + dy * 3] =
          intro_button_texpos[dx + 9 * 3 + dy * intro_button_total];
      texpos_right_arrow_button[dx + dy * 3] =
          intro_button_texpos[dx + 10 * 3 + dy * intro_button_total];
      texpos_up_arrow_button[dx + dy * 3] =
          intro_button_texpos[dx + 11 * 3 + dy * intro_button_total];
      texpos_down_arrow_button[dx + dy * 3] =
          intro_button_texpos[dx + 12 * 3 + dy * intro_button_total];
    }
  }
  enabler.textures.load_multi_pdim("data/art/border.png", border_texpos, 7, 3,
                                   true, &d3, &d4);
  ...
  enabler.textures.load_multi_pdim("data/art/scrollbar.png", scrollbar_texpos,
                                   12, 4, true, &d3, &d4);
  ...
  texpos_scrollbar_offcenter_scroller_hover[0][0] =
      scrollbar_texpos[10 + 2 * 12];
  texpos_scrollbar_offcenter_scroller_hover[1][0] =
      scrollbar_texpos[11 + 2 * 12];
  texpos_scrollbar_offcenter_scroller_hover[0][1] =
      scrollbar_texpos[10 + 3 * 12];
  texpos_scrollbar_offcenter_scroller_hover[1][1] =
      scrollbar_texpos[11 + 3 * 12];
  enabler.textures.load_multi_pdim("data/art/filter.png", filter_texpos, 10, 3,
                                   true, &d3, &d4);
  ...
  enabler.textures.load_multi_pdim("data/art/tabs.png", tabs_texpos, 10, 2,
                                   true, &d3, &d4);
  ...
  classic_load_bar_texpos[INIT_LOAD_BAR_TEXTURE_LEFT_FULL] =
      convert_raw_to_ascii_texpos('[', 7, 0, 1);
  classic_load_bar_texpos[INIT_LOAD_BAR_TEXTURE_MID_FULL] =
      convert_raw_to_ascii_texpos(254, 7, 0, 1);
  classic_load_bar_texpos[INIT_LOAD_BAR_TEXTURE_RIGHT_FULL] =
      convert_raw_to_ascii_texpos(']', 7, 0, 1);
  classic_load_bar_texpos[INIT_LOAD_BAR_TEXTURE_LEFT_EMPTY] =
      convert_raw_to_ascii_texpos('[', 7, 0, 0);
  classic_load_bar_texpos[INIT_LOAD_BAR_TEXTURE_MID_EMPTY] =
      convert_raw_to_ascii_texpos(250, 7, 0, 0);
  classic_load_bar_texpos[INIT_LOAD_BAR_TEXTURE_RIGHT_EMPTY] =
      convert_raw_to_ascii_texpos(']', 7, 0, 0);

  classic_texpos_neutral_intro_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 7, 0, 0);
  classic_texpos_neutral_intro_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 0);
  classic_texpos_neutral_intro_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 7, 0, 0);
  classic_texpos_neutral_intro_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_neutral_intro_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos(' ', 7, 0, 0);
  classic_texpos_neutral_intro_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_neutral_intro_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 7, 0, 0);
  classic_texpos_neutral_intro_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 0);
  classic_texpos_neutral_intro_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 7, 0, 0);

  classic_texpos_confirm_intro_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 2, 0, 1);
  classic_texpos_confirm_intro_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 2, 0, 1);
  classic_texpos_confirm_intro_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 2, 0, 1);
  classic_texpos_confirm_intro_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 2, 0, 1);
  classic_texpos_confirm_intro_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos(' ', 2, 0, 1);
  classic_texpos_confirm_intro_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 2, 0, 1);
  classic_texpos_confirm_intro_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 2, 0, 1);
  classic_texpos_confirm_intro_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 2, 0, 1);
  classic_texpos_confirm_intro_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 2, 0, 1);

  classic_texpos_cancel_intro_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 4, 0, 1);
  classic_texpos_cancel_intro_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 4, 0, 1);
  classic_texpos_cancel_intro_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 4, 0, 1);
  classic_texpos_cancel_intro_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 4, 0, 1);
  classic_texpos_cancel_intro_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos(' ', 4, 0, 1);
  classic_texpos_cancel_intro_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 4, 0, 1);
  classic_texpos_cancel_intro_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 4, 0, 1);
  classic_texpos_cancel_intro_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 4, 0, 1);
  classic_texpos_cancel_intro_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 4, 0, 1);

  classic_texpos_selected_intro_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 3, 0, 1);
  classic_texpos_selected_intro_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 3, 0, 1);
  classic_texpos_selected_intro_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 3, 0, 1);
  classic_texpos_selected_intro_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 3, 0, 1);
  classic_texpos_selected_intro_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos(' ', 3, 0, 1);
  classic_texpos_selected_intro_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 3, 0, 1);
  classic_texpos_selected_intro_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 3, 0, 1);
  classic_texpos_selected_intro_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 3, 0, 1);
  classic_texpos_selected_intro_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 3, 0, 1);

  classic_texpos_unselected_intro_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 3, 0, 0);
  classic_texpos_unselected_intro_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 3, 0, 0);
  classic_texpos_unselected_intro_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 3, 0, 0);
  classic_texpos_unselected_intro_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 3, 0, 0);
  classic_texpos_unselected_intro_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos(' ', 3, 0, 0);
  classic_texpos_unselected_intro_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 3, 0, 0);
  classic_texpos_unselected_intro_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 3, 0, 0);
  classic_texpos_unselected_intro_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 3, 0, 0);
  classic_texpos_unselected_intro_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 3, 0, 0);

  classic_texpos_open_list_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 7, 0, 1);
  classic_texpos_open_list_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_open_list_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 7, 0, 1);
  classic_texpos_open_list_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_open_list_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos('V', 7, 0, 1);
  classic_texpos_open_list_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_open_list_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 7, 0, 1);
  classic_texpos_open_list_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_open_list_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 7, 0, 1);

  classic_texpos_increase_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 7, 0, 1);
  classic_texpos_increase_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_increase_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 7, 0, 1);
  classic_texpos_increase_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_increase_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos('+', 7, 0, 1);
  classic_texpos_increase_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_increase_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 7, 0, 1);
  classic_texpos_increase_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_increase_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 7, 0, 1);

  classic_texpos_decrease_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 7, 0, 1);
  classic_texpos_decrease_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_decrease_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 7, 0, 1);
  classic_texpos_decrease_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_decrease_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos('-', 7, 0, 1);
  classic_texpos_decrease_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_decrease_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 7, 0, 1);
  classic_texpos_decrease_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_decrease_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 7, 0, 1);

  classic_texpos_nullify_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 4, 0, 1);
  classic_texpos_nullify_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 4, 0, 1);
  classic_texpos_nullify_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 4, 0, 1);
  classic_texpos_nullify_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 4, 0, 1);
  classic_texpos_nullify_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos('X', 4, 0, 1);
  classic_texpos_nullify_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 4, 0, 1);
  classic_texpos_nullify_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 4, 0, 1);
  classic_texpos_nullify_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 4, 0, 1);
  classic_texpos_nullify_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 4, 0, 1);

  classic_texpos_left_arrow_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 7, 0, 1);
  classic_texpos_left_arrow_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_left_arrow_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 7, 0, 1);
  classic_texpos_left_arrow_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_left_arrow_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos(27, 7, 0, 1);
  classic_texpos_left_arrow_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_left_arrow_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 7, 0, 1);
  classic_texpos_left_arrow_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_left_arrow_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 7, 0, 1);

  classic_texpos_right_arrow_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 7, 0, 1);
  classic_texpos_right_arrow_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_right_arrow_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 7, 0, 1);
  classic_texpos_right_arrow_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_right_arrow_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos(26, 7, 0, 1);
  classic_texpos_right_arrow_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_right_arrow_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 7, 0, 1);
  classic_texpos_right_arrow_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_right_arrow_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 7, 0, 1);

  classic_texpos_up_arrow_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 7, 0, 1);
  classic_texpos_up_arrow_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_up_arrow_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 7, 0, 1);
  classic_texpos_up_arrow_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_up_arrow_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos(24, 7, 0, 1);
  classic_texpos_up_arrow_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_up_arrow_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 7, 0, 1);
  classic_texpos_up_arrow_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_up_arrow_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 7, 0, 1);

  classic_texpos_down_arrow_button[0 + 0 * 3] =
      convert_raw_to_ascii_texpos(218, 7, 0, 1);
  classic_texpos_down_arrow_button[0 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_down_arrow_button[0 + 2 * 3] =
      convert_raw_to_ascii_texpos(192, 7, 0, 1);
  classic_texpos_down_arrow_button[1 + 0 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_down_arrow_button[1 + 1 * 3] =
      convert_raw_to_ascii_texpos(25, 7, 0, 1);
  classic_texpos_down_arrow_button[1 + 2 * 3] =
      convert_raw_to_ascii_texpos(196, 7, 0, 1);
  classic_texpos_down_arrow_button[2 + 0 * 3] =
      convert_raw_to_ascii_texpos(191, 7, 0, 1);
  classic_texpos_down_arrow_button[2 + 1 * 3] =
      convert_raw_to_ascii_texpos(179, 7, 0, 1);
  classic_texpos_down_arrow_button[2 + 2 * 3] =
      convert_raw_to_ascii_texpos(217, 7, 0, 1);
  ...
  classic_texpos_scrollbar_up_hover[0] =
      convert_raw_to_ascii_texpos('/', 3, 0, 1);
  classic_texpos_scrollbar_up_hover[1] =
      convert_raw_to_ascii_texpos('\\', 3, 0, 1);
  classic_texpos_scrollbar_up_pressed[0] =
      convert_raw_to_ascii_texpos('/', 3, 0, 1);
  classic_texpos_scrollbar_up_pressed[1] =
      convert_raw_to_ascii_texpos('\\', 3, 0, 1);
  classic_texpos_scrollbar_down_hover[0] =
      convert_raw_to_ascii_texpos('\\', 3, 0, 1);
  classic_texpos_scrollbar_down_hover[1] =
      convert_raw_to_ascii_texpos('/', 3, 0, 1);
  classic_texpos_scrollbar_down_pressed[0] =
      convert_raw_to_ascii_texpos('\\', 3, 0, 1);
  classic_texpos_scrollbar_down_pressed[1] =
      convert_raw_to_ascii_texpos('/', 3, 0, 1);
  classic_texpos_scrollbar_small_scroller[0][0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_small_scroller[0][1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_small_scroller[1][0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_small_scroller[1][1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_small_scroller_hover[0][0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_small_scroller_hover[0][1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_small_scroller_hover[1][0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_small_scroller_hover[1][1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_top_scroller[0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_top_scroller[1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_top_scroller_hover[0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_top_scroller_hover[1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_bottom_scroller[0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_bottom_scroller[1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_bottom_scroller_hover[0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_bottom_scroller_hover[1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_blank_scroller[0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_blank_scroller[1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_blank_scroller_hover[0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_blank_scroller_hover[1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_center_scroller[0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_center_scroller[1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_center_scroller_hover[0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_center_scroller_hover[1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_offcenter_scroller[0][0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_offcenter_scroller[0][1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_offcenter_scroller[1][0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_offcenter_scroller[1][1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 0);
  classic_texpos_scrollbar_offcenter_scroller_hover[0][0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_offcenter_scroller_hover[0][1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_offcenter_scroller_hover[1][0] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);
  classic_texpos_scrollbar_offcenter_scroller_hover[1][1] =
      convert_raw_to_ascii_texpos(219, 3, 0, 1);

  classic_texpos_button_filter[0][0] =
      convert_raw_to_ascii_texpos(218, 7, 0, 0);
  classic_texpos_button_filter[0][1] =
      convert_raw_to_ascii_texpos(179, 7, 0, 0);
  classic_texpos_button_filter[0][2] =
      convert_raw_to_ascii_texpos(192, 7, 0, 0);
  classic_texpos_button_filter[1][0] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_button_filter[1][1] =
      convert_raw_to_ascii_texpos(' ', 7, 0, 0);
  classic_texpos_button_filter[1][2] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_button_filter[2][0] =
      convert_raw_to_ascii_texpos(194, 7, 0, 0);
  classic_texpos_button_filter[2][1] =
      convert_raw_to_ascii_texpos(179, 7, 0, 0);
  classic_texpos_button_filter[2][2] =
      convert_raw_to_ascii_texpos(193, 7, 0, 0);
  classic_texpos_button_filter[3][0] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_button_filter[3][1] =
      convert_raw_to_ascii_texpos('-', 7, 0, 1);
  classic_texpos_button_filter[3][2] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_button_filter[4][0] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_button_filter[4][1] =
      convert_raw_to_ascii_texpos('O', 7, 0, 1);
  classic_texpos_button_filter[4][2] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_button_filter[5][0] =
      convert_raw_to_ascii_texpos(191, 7, 0, 0);
  classic_texpos_button_filter[5][1] =
      convert_raw_to_ascii_texpos(179, 7, 0, 0);
  classic_texpos_button_filter[5][2] =
      convert_raw_to_ascii_texpos(217, 7, 0, 0);

  classic_texpos_button_filter_name[0][0] =
      convert_raw_to_ascii_texpos(218, 7, 0, 0);
  classic_texpos_button_filter_name[0][1] =
      convert_raw_to_ascii_texpos(179, 7, 0, 0);
  classic_texpos_button_filter_name[0][2] =
      convert_raw_to_ascii_texpos(192, 7, 0, 0);
  classic_texpos_button_filter_name[1][0] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_button_filter_name[1][1] =
      convert_raw_to_ascii_texpos('_', 0, 0, 1);
  classic_texpos_button_filter_name[1][2] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_button_filter_name[2][0] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_button_filter_name[2][1] =
      convert_raw_to_ascii_texpos(13, 7, 0, 1);
  classic_texpos_button_filter_name[2][2] =
      convert_raw_to_ascii_texpos(196, 7, 0, 0);
  classic_texpos_button_filter_name[3][0] =
      convert_raw_to_ascii_texpos(191, 7, 0, 0);
  classic_texpos_button_filter_name[3][1] =
      convert_raw_to_ascii_texpos(179, 7, 0, 0);
  classic_texpos_button_filter_name[3][2] =
      convert_raw_to_ascii_texpos(217, 7, 0, 0);

  classic_texpos_tab_unselected[0][0] =
      convert_raw_to_ascii_texpos(' ', 6, 0, 0);
  classic_texpos_tab_unselected[0][1] =
      convert_raw_to_ascii_texpos('/', 6, 0, 0);
  classic_texpos_tab_unselected[1][0] =
      convert_raw_to_ascii_texpos('/', 6, 0, 0);
  classic_texpos_tab_unselected[1][1] =
      convert_raw_to_ascii_texpos('-', 6, 0, 0);
  classic_texpos_tab_unselected[2][0] =
      convert_raw_to_ascii_texpos('-', 6, 0, 0);
  classic_texpos_tab_unselected[2][1] =
      convert_raw_to_ascii_texpos('-', 6, 0, 0);
  classic_texpos_tab_unselected[3][0] =
      convert_raw_to_ascii_texpos('\\', 6, 0, 0);
  classic_texpos_tab_unselected[3][1] =
      convert_raw_to_ascii_texpos('-', 6, 0, 0);
  classic_texpos_tab_unselected[4][0] =
      convert_raw_to_ascii_texpos(' ', 6, 0, 0);
  classic_texpos_tab_unselected[4][1] =
      convert_raw_to_ascii_texpos('\\', 6, 0, 0);
  ...
  classic_texpos_tab_selected[3][0] =
      convert_raw_to_ascii_texpos('\\', 6, 0, 1);
  ...
  classic_texpos_tab_selected[4][1] =
      convert_raw_to_ascii_texpos('\\', 6, 0, 1);
  ...
  enabler.textures.load_multi_pdim(black_background_filename,
                                   gps.black_background_texpos, 1, 1, false,
                                   &dummy_dispx, &dummy_dispy);
  ...
  // got rid of this - this is computing the window size if they provide a
  // desired dimension in characters, like 80x25
  /*
      if (display.desired_windowed_width < MAX_GRID_X &&
     display.desired_windowed_height < MAX_GRID_Y) { int dimx =
     MAX(display.desired_windowed_width,80); int dimy =
     MAX(display.desired_windowed_height,25); display.desired_windowed_width =
     font.basic_font_dispx * dimx; display.desired_windowed_height =
     font.basic_font_dispy * dimy;
      }
  */
}
  ...
  for (f = 0; f < 3; ++f) {
    for (g = 0; g < 256; ++g) {
      ...
      *dst_bot = enabler.textures.create_texture(src->w, src->h);
      ...
      memset(dst_top_px->pixels, 0,
             sizeof(Uint8) * (dst_top_px->h) * dst_top_px->pitch);
      memset(dst_bot_px->pixels, 0,
             sizeof(Uint8) * (dst_bot_px->h) * dst_bot_px->pitch);
      ...
      for (y = 0; y < src->h; y++) {
        ...
        if (y < src->h / 2) {
          pixel_dst = ((Uint8*)dst_top_px->pixels) +
                      ((y + (src->h + 1) / 2) * dst_top_px->pitch);
          ...
          pixel_dst = ((Uint8*)dst_bot_px->pixels) +
                      ((y - src->h / 2) * dst_bot_px->pitch);
          for (x = 0; x < src->w; ++x, pixel_src += 4, pixel_dst += 4) {
            if (pixel_src[3] != 0) {
              pixel_dst[0] = pixel_src[0];
              pixel_dst[1] = pixel_src[1];
...
void initst::swap_basic_sets() {
  int32_t l;
  for (l = 0; l < INIT_LOAD_BAR_TEXTURENUM; ++l)
    swap_long(load_bar_texpos[l], classic_load_bar_texpos[l]);
  for (l = 0; l < 9; ++l) {
    swap_int32_t(texpos_neutral_intro_button[l],
                 classic_texpos_neutral_intro_button[l]);
    swap_int32_t(texpos_confirm_intro_button[l],
                 classic_texpos_confirm_intro_button[l]);
    swap_int32_t(texpos_cancel_intro_button[l],
                 classic_texpos_cancel_intro_button[l]);
    swap_int32_t(texpos_selected_intro_button[l],
                 classic_texpos_selected_intro_button[l]);
    swap_int32_t(texpos_unselected_intro_button[l],
                 classic_texpos_unselected_intro_button[l]);
    swap_int32_t(texpos_open_list_button[l],
                 classic_texpos_open_list_button[l]);
    ...
    swap_int32_t(texpos_left_arrow_button[l],
                 classic_texpos_left_arrow_button[l]);
    swap_int32_t(texpos_right_arrow_button[l],
                 classic_texpos_right_arrow_button[l]);
    swap_int32_t(texpos_up_arrow_button[l], classic_texpos_up_arrow_button[l]);
    swap_int32_t(texpos_down_arrow_button[l],
                 classic_texpos_down_arrow_button[l]);
  }
  ...
  for (l = 0; l < 2; ++l) {
    for (k = 0; k < 3; ++k)
      swap_int32_t(texpos_scrollbar[l][k], classic_texpos_scrollbar[l][k]);
  }
  for (l = 0; l < 2; ++l) {
    swap_int32_t(texpos_scrollbar_up_hover[l],
                 classic_texpos_scrollbar_up_hover[l]);
    swap_int32_t(texpos_scrollbar_up_pressed[l],
                 classic_texpos_scrollbar_up_pressed[l]);
    swap_int32_t(texpos_scrollbar_down_hover[l],
                 classic_texpos_scrollbar_down_hover[l]);
    swap_int32_t(texpos_scrollbar_down_pressed[l],
                 classic_texpos_scrollbar_down_pressed[l]);
    swap_int32_t(texpos_scrollbar_top_scroller[l],
                 classic_texpos_scrollbar_top_scroller[l]);
    swap_int32_t(texpos_scrollbar_top_scroller_hover[l],
                 classic_texpos_scrollbar_top_scroller_hover[l]);
    swap_int32_t(texpos_scrollbar_bottom_scroller[l],
                 classic_texpos_scrollbar_bottom_scroller[l]);
    swap_int32_t(texpos_scrollbar_bottom_scroller_hover[l],
                 classic_texpos_scrollbar_bottom_scroller_hover[l]);
    swap_int32_t(texpos_scrollbar_blank_scroller[l],
                 classic_texpos_scrollbar_blank_scroller[l]);
    swap_int32_t(texpos_scrollbar_blank_scroller_hover[l],
                 classic_texpos_scrollbar_blank_scroller_hover[l]);
    swap_int32_t(texpos_scrollbar_center_scroller[l],
                 classic_texpos_scrollbar_center_scroller[l]);
    swap_int32_t(texpos_scrollbar_center_scroller_hover[l],
                 classic_texpos_scrollbar_center_scroller_hover[l]);
  }
  for (l = 0; l < 2; ++l) {
    for (k = 0; k < 2; ++k) {
      swap_int32_t(texpos_scrollbar_small_scroller[l][k],
                   classic_texpos_scrollbar_small_scroller[l][k]);
      swap_int32_t(texpos_scrollbar_small_scroller_hover[l][k],
                   classic_texpos_scrollbar_small_scroller_hover[l][k]);
      swap_int32_t(texpos_scrollbar_offcenter_scroller[l][k],
                   classic_texpos_scrollbar_offcenter_scroller[l][k]);
      swap_int32_t(texpos_scrollbar_offcenter_scroller_hover[l][k],
                   classic_texpos_scrollbar_offcenter_scroller_hover[l][k]);
    }
  }
  for (l = 0; l < 6; ++l) {
    for (k = 0; k < 3; ++k)
      swap_int32_t(texpos_button_filter[l][k],
                   classic_texpos_button_filter[l][k]);
  }
  for (l = 0; l < 4; ++l) {
    for (k = 0; k < 3; ++k)
      swap_int32_t(texpos_button_filter_name[l][k],
                   classic_texpos_button_filter_name[l][k]);
  }
  for (l = 0; l < 5; ++l) {
    for (k = 0; k < 2; ++k) {
      swap_int32_t(texpos_tab_unselected[l][k],
                   classic_texpos_tab_unselected[l][k]);
      swap_int32_t(texpos_tab_selected[l][k],
                   classic_texpos_tab_selected[l][k]);
    }
  }
}