...
bool scrollbarst::handle_events(std::set<InterfaceKey>& events,
                                int32_t& scroll_position, bool& scrolling) {
...
void scrollbarst::handle_initial_click(int32_t mouse_y,
...
void scrollbarst::render(int32_t mouse_x, int32_t mouse_y, int32_t bx,
                         bool active, uint32_t display_flag) {
  if (display_flag & SCROLLBAR_DISPLAY_FLAG_BASIC_SET) {
    int32_t px, py;
    for (px = bx; px <= bx + 1; ++px) {
      for (py = print_sy - 1; py <= print_ey + 1;
           ++py) //+/-1 for arrows at top bottom
      {
        gps.locate(py, px);
        //************************** PRESSED SCROLL BUTTONS
        // we have texpos_scrollbar_up/down_pressed but they aren't used yet
        // hard to maintain variable since the scrollbarst doesn't persist
        // anywhere
        if (py == print_sy - 1) {
          if (mouse_y == py && mouse_x >= bx && mouse_x <= bx + 1)
            gps.add_lower_tile(::init.texpos_scrollbar_up_hover[px - bx]);
          else
            gps.add_lower_tile(::init.texpos_scrollbar[px - bx][0]);
        } else if (py == print_ey + 1) {
          if (mouse_y == py && mouse_x >= bx && mouse_x <= bx + 1)
            gps.add_lower_tile(::init.texpos_scrollbar_down_hover[px - bx]);
          else
            gps.add_lower_tile(::init.texpos_scrollbar[px - bx][2]);
        } else
          gps.add_lower_tile(::init.texpos_scrollbar[px - bx][1]);
    ...
    bool hover = ((mouse_x >= bx && mouse_x <= bx + 1 && mouse_y >= print_sy &&
                   mouse_y <= print_ey) ||
                  active);
    ...
    if (scroller_ey - 1 <= scroller_sy) {
      ...
      for (py = scroller_sy; py <= scroller_ey; ++py) {
        if (py == mid) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_center_scroller_hover[0], 0);
          else
            gps.add_tile(::init.texpos_scrollbar_center_scroller[0], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_center_scroller_hover[1], 0);
          else
            gps.add_tile(::init.texpos_scrollbar_center_scroller[1], 0);
        } else if (py == scroller_sy) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_top_scroller_hover[0], 0);
          else
            gps.add_tile(::init.texpos_scrollbar_top_scroller[0], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_top_scroller_hover[1], 0);
          else
            gps.add_tile(::init.texpos_scrollbar_top_scroller[1], 0);
        } else if (py == scroller_ey) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_bottom_scroller_hover[0], 0);
          else
            gps.add_tile(::init.texpos_scrollbar_bottom_scroller[0], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_bottom_scroller_hover[1], 0);
          else
            gps.add_tile(::init.texpos_scrollbar_bottom_scroller[1], 0);
        } else {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_blank_scroller_hover[0], 0);
          else
            gps.add_tile(::init.texpos_scrollbar_blank_scroller[0], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_blank_scroller_hover[1], 0);
          else
            gps.add_tile(::init.texpos_scrollbar_blank_scroller[1], 0);
        }
      }
    } else {
      int32_t mid = (scroller_sy + scroller_ey) / 2;
      ...
      for (py = scroller_sy; py <= scroller_ey; ++py) {
        if (py == mid) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_offcenter_scroller_hover[0][0],
                         0);
          else
            gps.add_tile(::init.texpos_scrollbar_offcenter_scroller[0][0], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_offcenter_scroller_hover[1][0],
                         0);
          else
            gps.add_tile(::init.texpos_scrollbar_offcenter_scroller[1][0], 0);
        } else if (py == mid + 1) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_offcenter_scroller_hover[0][1],
                         0);
          else
            gps.add_tile(::init.texpos_scrollbar_offcenter_scroller[0][1], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_offcenter_scroller_hover[1][1],
                         0);
          else
            gps.add_tile(::init.texpos_scrollbar_offcenter_scroller[1][1], 0);
        } else if (py == scroller_sy) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(::init.texpos_scrollbar_top_scroller_hover[0], 0);
    ...
    for (px = bx; px <= bx + 1; ++px) {
           ...
           ++py) //+/-1 for arrows at top bottom
      {
        ...
        if (py == print_sy - 1) {
          if (mouse_y == py && mouse_x >= bx && mouse_x <= bx + 1)
            gps.add_lower_tile(gps.texpos_scrollbar_up_hover[px - bx]);
          else
            gps.add_lower_tile(gps.texpos_scrollbar[px - bx][0]);
        } else if (py == print_ey + 1) {
          if (mouse_y == py && mouse_x >= bx && mouse_x <= bx + 1)
            gps.add_lower_tile(gps.texpos_scrollbar_down_hover[px - bx]);
          else
            gps.add_lower_tile(gps.texpos_scrollbar[px - bx][2]);
        } else
          gps.add_lower_tile(gps.texpos_scrollbar[px - bx][1]);
    ...
    if (scroller_ey - 1 <= scroller_sy) {
      ...
      for (py = scroller_sy; py <= scroller_ey; ++py) {
        if (py == mid) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_center_scroller_hover[0], 0);
          else
            gps.add_tile(gps.texpos_scrollbar_center_scroller[0], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_center_scroller_hover[1], 0);
          else
            gps.add_tile(gps.texpos_scrollbar_center_scroller[1], 0);
        } else if (py == scroller_sy) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_top_scroller_hover[0], 0);
          else
            gps.add_tile(gps.texpos_scrollbar_top_scroller[0], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_top_scroller_hover[1], 0);
          else
            gps.add_tile(gps.texpos_scrollbar_top_scroller[1], 0);
        } else if (py == scroller_ey) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_bottom_scroller_hover[0], 0);
          else
            gps.add_tile(gps.texpos_scrollbar_bottom_scroller[0], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_bottom_scroller_hover[1], 0);
          else
            gps.add_tile(gps.texpos_scrollbar_bottom_scroller[1], 0);
        } else {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_blank_scroller_hover[0], 0);
          else
            gps.add_tile(gps.texpos_scrollbar_blank_scroller[0], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_blank_scroller_hover[1], 0);
          else
            gps.add_tile(gps.texpos_scrollbar_blank_scroller[1], 0);
        }
      }
    } else {
      int32_t mid = (scroller_sy + scroller_ey) / 2;
      ...
      for (py = scroller_sy; py <= scroller_ey; ++py) {
        if (py == mid) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_offcenter_scroller_hover[0][0],
                         0);
          else
            gps.add_tile(gps.texpos_scrollbar_offcenter_scroller[0][0], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_offcenter_scroller_hover[1][0],
                         0);
          else
            gps.add_tile(gps.texpos_scrollbar_offcenter_scroller[1][0], 0);
        } else if (py == mid + 1) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_offcenter_scroller_hover[0][1],
                         0);
          else
            gps.add_tile(gps.texpos_scrollbar_offcenter_scroller[0][1], 0);
          gps.locate(py, bx + 1);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_offcenter_scroller_hover[1][1],
                         0);
          else
            gps.add_tile(gps.texpos_scrollbar_offcenter_scroller[1][1], 0);
        } else if (py == scroller_sy) {
          gps.locate(py, bx);
          if (hover)
            gps.add_tile(gps.texpos_scrollbar_top_scroller_hover[0], 0);
...
void draw_nineslice(int32_t* texpos, int sy, int sx, int ey, int ex,
                    bool override) {
  // corners
  gps.locate(sy, sx);
...
void scroll_rows::feed(set<InterfaceKey>& events) {
  ...
  scrollbar.init(scroll, 0, (int32_t)children.size() - 1, num_visible, rect.y,
                 rect.y + rect.h);
...
void scroll_rows::render() {
  ...
  scrollbar.init(scroll, 0, (int32_t)children.size() - 1, num_visible, rect.y,
                 rect.y + rect.h);
  scrollbar.render(tmx, tmy, rect.x + rect.w, scroll,
                   SCROLLBAR_DISPLAY_FLAG_BASIC_SET);
}

void scroll_rows::arrange() {
  std::for_each(children.begin(), children.end(),
                [](auto& child) { child->visible = true; });
  int32_t sy = 0;
  num_visible = 0;
...
void tabs::add_tab_label(const char* s) { tab_labels.push_back(string(s)); }

void tabs::add_tab_label(string& s) { tab_labels.push_back(s); }

bool tabs::switch_tab(size_t which) {
  if (which == cur_idx || which < 0)
    return false;
  cur_idx = which;
  arrange();
  return true;
...
void tabs::render() {
  ...
  for (auto i = 0; i < tab_labels.size(); i++) {
    ...
    auto texpos =
        cur_idx == i ? init.texpos_tab_selected : init.texpos_tab_unselected;
    // left border
    ...
    if (!(::init.display.flag.has_flag(INIT_DISPLAY_FLAG_USE_GRAPHICS))) {
      ...
      gps.addst_flag(tab_labels[i], justify_left, 0,
                     SCREENTEXPOS_FLAG_TOP_OF_TEXT);
      gps.locate(rect.y + 1, tab_sx + 2);
      gps.addst_flag(tab_labels[i], justify_left, 0,
                     SCREENTEXPOS_FLAG_BOTTOM_OF_TEXT);
    }
...
button::button(std::function<void()> c) : widget() { callback = c; }
...
// If you use this constructor with any bool that doesn't have a static lifetime
...
better_button::better_button(bool* b) : widget() {
  ...
  texpos = []() { return (int32_t*)&(init.texpos_confirm_intro_button); };
  callback = [b]() { *b = !*b; };
  check_truth = [b]() { return *b; };
}
...
void better_button::render() {
  ...
  if (tex)
    draw_nineslice(tex, sy, sx, ey, ex);
  if (s.size() > 0) {
...
void textbox::render() {
  ...
  for (auto by = sy; by <= ey; ++by) {
    ...
    for (auto bx = sx + 1; bx <= ex - 4; ++bx) {
      ...
      if (init.display.flag.has_flag(INIT_DISPLAY_FLAG_USE_GRAPHICS))
        gps.addchar(0); // clear out drawborder
    }
  }
  ...
  if (input && GetTickCount() % 1000 < 500)
    display_str += "_";
...
void dropdown::render() {
  ...
  if (open) {
    ...
    for (int i = 0; i < options.size(); i++) {
      if (i == cur_selected) {
        draw_nineslice(init.texpos_selected_intro_button, by, sx + 1, by + 2,
                       ex);
      } else {
        draw_nineslice(init.texpos_unselected_intro_button, by, sx + 1, by + 2,
                       ex);
      }
