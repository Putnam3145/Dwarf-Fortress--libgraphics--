...
class renderer_curses : public renderer {
  ...
  static int ncurses_map_color(int color) {
    ...
    case 7:
      return 7;
    default:
      return ncurses_map_color(color - 7);
    }
  }
  ...
  int lookup_pair(pair<int, int> color) {
    ...
    if (color.first < 0 || color.first > 7 || color.second < 0 ||
        color.second > 7)
      return 0;
    ...
    if (color_pairs.size() < COLOR_PAIRS - 1) {
      ...
      init_pair(pair, ncurses_map_color(color.first),
                ncurses_map_color(color.second));
      color_pairs[color] = pair;
      return pair;
    }
    // We don't have it, and there's no space for more. Find the closest
    // equivalent.
...
public:
  ...
  void grid_resize(int w, int h) { gps_allocate(w, h, w, h, 8, 12); }

  renderer_curses() { init_curses(); }
  ...
  // The following four are, I *think*, actually required for text mode to work,
  // at least in a way that squares with the main game, and thus I'm commenting
  // them out for now so trying to compile with them just crashes. One day. One
  // day. Hopefully soon!
  /*
  void update_full_map_port(graphic_map_portst *port) {
  report_error("PRINT_MODE","TEXT not supported yet"); abort(); } void
  update_full_viewport(graphic_viewportst *port) {
  report_error("PRINT_MODE","TEXT not supported yet"); abort(); } void
  update_map_port_tile(graphic_map_portst *port, int32_t x,int32_t y) {
  report_error("PRINT_MODE","TEXT not supported yet"); abort(); } void
  update_viewport_tile(graphic_viewportst *port,int32_t x,int32_t y) {
  report_error("PRINT_MODE","TEXT not supported yet"); abort(); }
  */
};
...
void init_curses() {
  ...
  opened:
    ...
    _waddnstr =
        (int (*)(WINDOW* w, const char* s, int n))dlsym_orexit("waddnstr");
    ...
    _init_pair =
        (int (*)(short p, short fg, short bg))dlsym_orexit("init_pair");
    _getmouse = (int (*)(MEVENT* m))dlsym_orexit("getmouse");
    _waddnwstr =
        (int (*)(WINDOW* w, const wchar_t* s, int i))dlsym_orexit("waddnwstr");
  }
