...
static inline void Resample(SDL_Surface* src, SDL_Surface* dst, int filter) {
  ...
  std::vector<double> contribution_x(
      std::min((size_t)src->w, 5 + (size_t)(2 * support)));
  ...
  for (int x = 0; x < dst->w; ++x) {
    ...
    for (int y = 0; y < src->h; y++) {
      for (size_t n = 0; n < nmax; ++n) {
        if (y ==
            0) { // only come up with the contribution list once per column.
  ...
  std::vector<double> contribution_y(
      std::min((size_t)src->h, 5 + (size_t)(2 * support)));
...
SDL_Surface* SDL_Resize(SDL_Surface* src, int new_w, int new_h, bool free,
                        int filter) {
  //*********************** ALPHA CHECK
