...
protected:
  ...
  // We may shrink or enlarge dispx/dispy in response to zoom requests.
  // dispx/y_z are the size we actually display tiles at.
  int dispx_z, dispy_z;
  // Viewport origin
    ...
    if (it != tile_cache.end()) {
      ...
      /*
        color = SDL_CreateRGBSurface(SDL_SWSURFACE,
                                     ...
                                     0);
                     */
      SDL_Surface* color = SDL_CreateRGBSurface(
          0, surf->w, surf->h, 32, surf->format->Rmask, surf->format->Gmask,
          surf->format->Bmask, surf->format->Amask);
      if (!color) {
        MessageBox(NULL, "Unable to create texture!", "Fatal error",
      ...
      SDL_SetSurfaceColorMod(surf, (Uint8)(id.r * 255), (Uint8)(id.g * 255),
                             (Uint8)(id.b * 255));
      if (id.flag & TEXTURE_FULLID_FLAG_TRANSPARENT_BACKGROUND)
        ...
        SDL_FillRect(
            color, NULL,
            SDL_MapRGB(color->format, id.br * 255, id.bg * 255, id.bb * 255));
      ...
      for (int x = 0; x < tex->w; x++, pixel_src+=4, pixel_dst+=4) {
        ...
        if(id.flag & TEXTURE_FULLID_FLAG_DO_RECOLOR)
        {
        float fg = color_fg[c] * (1 / 255.0f), bg = color_bg[c] * (1 / 255.0f),
      tex = pixel_src[c] * (1 / 255.0f); pixel_dst[c] = (Uint8)(((alpha * (tex *
      fg)) + ((1 - alpha) * bg)) * 255);
        }
        else if(norecolor)
        {
      ...
      // only including the comment here because if g_src gets released again
      this won't be in the git history, no good
        */
      ...
      if (use_viewport_zoom) {
        ...
        if (should_do_soft_resize) {
          SDL_Surface* disp =
              SDL_Resize(color, viewport_zoom_factor * surf->w / 128,
                         viewport_zoom_factor * surf->h / 128);
      ...
      if (id.flag & TEXTURE_FULLID_FLAG_CONVERT) {
        ...
        if (should_do_soft_resize) {
          SDL_Surface* disp = SDL_Resize(color, dispx_z * surf->w / dispx,
                                         dispy_z * surf->h / dispy);
          tile_cache[id] = SDL_CreateTextureFromSurface(sdl_renderer, disp);
          SDL_FreeSurface(disp);
          return tile_cache[id];
        }
  ...
  virtual bool init_video(int w, int h) {
    ...
    if (!window) {
      window = SDL_CreateWindow(
          GAME_TITLE_STRING, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
          init.display.desired_windowed_width,
          init.display.desired_windowed_height, window_flags);
    ...
    clean_tile_cache();
    screen_tex = SDL_CreateTexture(sdl_renderer, SDL_PIXELFORMAT_ABGR8888,
                                   SDL_TEXTUREACCESS_TARGET, w, h);
    SDL_SetHint(SDL_HINT_MOUSE_RELATIVE_SCALING, "1");
    SDL_RenderSetLogicalSize(sdl_renderer, w, h);
    if (!window || !sdl_renderer || !screen_tex)
...
public:
  ...
  void update_anchor_tile(int x, int y) {
    ...
    if (init.display.flag.has_flag(INIT_DISPLAY_FLAG_USE_GRAPHICS)) {
      ...
      if (atp != 0) {
        if (!(gps.screentexpos_flag[x * gps.dimy + y] &
  ...
  void update_top_anchor_tile(int x, int y) {
    ...
    if (init.display.flag.has_flag(INIT_DISPLAY_FLAG_USE_GRAPHICS)) {
      ...
      if (atp != 0) {
        if (!(gps.screentexpos_top_flag[x * gps.dimy + y] &
              SCREENTEXPOS_FLAG_ANCHOR_SUBORDINATE)) {
  ...
  void update_tile(int x, int y) {
    ...
    if (init.display.flag.has_flag(INIT_DISPLAY_FLAG_USE_GRAPHICS)) {
      ...
      if (ltp != 0) {
        ...
        SDL_Texture* tex = tile_cache_lookup(background_tex);
        if (tex != NULL) {
          // SDL_SetSurfaceAlphaMod(tex, 0);//this appears to stop the ghosts
          // from appearing! (first 0 defaults to SDL_SRCALPHA)
          SDL_RenderCopy(sdl_renderer, tex, NULL, &dst);
        }
      }
    ...
    if (s[0] == 0 && gps.screentexpos[x * gps.dimy + y] == 0)
      return;

    // Read tiles from gps, create cached texture
    Either<texture_fullid, int32_t /*texture_ttfid*/> id =
        screen_to_texid(x, y);
    SDL_Texture* tex;
    if (id.isL) { // Ordinary tile, cached here
      id.left.flag = TEXTURE_FULLID_FLAG_DO_RECOLOR;
      id.left.flag |= TEXTURE_FULLID_FLAG_CONVERT;
      if (ltp != 0 && s[4] == 0 && s[5] == 0 && s[6] == 0)
        id.left.flag |= TEXTURE_FULLID_FLAG_TRANSPARENT_BACKGROUND;
  ...
  void update_top_tile(int x, int y) {
    ...
    if (s[0] == 0 && gps.screentexpos_top[x * gps.dimy + y] == 0)
      return;

    // Read tiles from gps, create cached texture
    Either<texture_fullid, int32_t /*texture_ttfid*/> id =
        screen_top_to_texid(x, y);
    SDL_Texture* tex;
    if (id.isL) { // Ordinary tile, cached here
      id.left.flag = TEXTURE_FULLID_FLAG_DO_RECOLOR;
      id.left.flag |= TEXTURE_FULLID_FLAG_CONVERT;
    ...
    */
    ...
    /*if (id.isL)*/ { // Ordinary tile, cached here
      if (init.display.flag.has_flag(INIT_DISPLAY_FLAG_USE_GRAPHICS)) {
        ...
        {
          //************************** TEXTURE SIZE DEPENDENCE
          // in particular, the dy having a +2 is to get rid of some tearing on
          // the border, need to figure that one out
          ...
          for (gx = sx; gx < sx + dx; ++gx) {
            for (gy = sy; gy < sy + dy; ++gy) {
              if (gx >= 0 && gx < gps.dimx && gy >= 0 && gy < gps.dimy) {
                gps.screentexpos_refresh_buffer[gx * gps.dimy + gy] =
                ...
                if (gps.screentexpos_anchored[gx * gps.dimy + gy] != 0) {
                  ...
                  if (tf & SCREENTEXPOS_FLAG_ANCHOR_SUBORDINATE) {
                    int32_t off_x = ((tf & SCREENTEXPOS_FLAG_ANCHOR_X_COORD) >>
                                     SCREENTEXPOS_FLAG_ANCHOR_X_COORD_SHIFT);
                    int32_t off_y = ((tf & SCREENTEXPOS_FLAG_ANCHOR_Y_COORD) >>
                                     SCREENTEXPOS_FLAG_ANCHOR_Y_COORD_SHIFT);
                    if (gx - off_x >= 0 && gx - off_x < gps.dimx &&
                        gy - off_y >= 0 && gy - off_y < gps.dimy) {
                      gps.screentexpos_refresh_buffer[(gx - off_x) * gps.dimy +
                                                      gy - off_y] =
                          gps.refresh_buffer_val;
                    }
                  }
  ...
  void update_viewport_tile(graphic_viewportst* vp, int32_t x, int32_t y) {
    ...
    if (viewport_zoom_factor != 128) {
      ...
      if (cinematic_mode) {
        dst.x =
            (viewport_zoom_factor * (32 * x + cinematic_shift_x + 32)) / 128 +
            origin_x;
        dst.y =
            (viewport_zoom_factor * (32 * y + cinematic_shift_y + 32)) / 128 +
            origin_y;
    ...
    /*if (id.isL)*/ { // Ordinary tile, cached here
      if (init.display.flag.has_flag(INIT_DISPLAY_FLAG_USE_GRAPHICS)) {
        ...
        if (spfl & VIEWPORT_SPATTER_FLAG_FIRE_BITS) {
          ...
          if (fire_bit == VIEWPORT_SPATTER_FLAG_FIRE_FRAME_1)
            fire_frame = 0;
          if (fire_bit == VIEWPORT_SPATTER_FLAG_FIRE_FRAME_2)
            fire_frame = 1;
          if (fire_bit == VIEWPORT_SPATTER_FLAG_FIRE_FRAME_3)
            fire_frame = 2;
          if (fire_bit == VIEWPORT_SPATTER_FLAG_FIRE_FRAME_4)
            fire_frame = 3;
        }
        ...
        if (spectex != 0) {
          int32_t tp = 0;
          switch (spectex) {
          case VIEWPORT_FLOOR_FLAG_SOIL_BACKGROUND_1:
            tp = gps.dirt_floor_texpos[0][4];
            break;
          case VIEWPORT_FLOOR_FLAG_SOIL_BACKGROUND_2:
            tp = gps.dirt_floor_texpos[1][4];
            break;
          case VIEWPORT_FLOOR_FLAG_SOIL_BACKGROUND_3:
            tp = gps.dirt_floor_texpos[2][4];
            break;
          case VIEWPORT_FLOOR_FLAG_SOIL_BACKGROUND_4:
            tp = gps.dirt_floor_texpos[3][4];
            break;
          }
          if (tp != 0) {
            texture_fullid background_tex;
          ...
          if (rf & VIEWPORT_RAMP_FLAG_SHOW_DOWN_ARROW) {
            ...
            if ((rf & VIEWPORT_RAMP_FLAG_WALL_N) &&
              ...
              if (!(rf & VIEWPORT_RAMP_FLAG_S_IS_OPEN_AIR)) {
                ...
                background_tex.bb = 0;
                background_tex.flag =
                    TEXTURE_FULLID_FLAG_TRANSPARENT_BACKGROUND;
                tex = tile_cache_lookup(background_tex);
                if (tex != NULL) {
                  // SDL_SetSurfaceAlphaMod(tex, 0);
                  SDL_RenderCopy(sdl_renderer, tex, NULL, &dst);
              ...
              if (!(rf & VIEWPORT_RAMP_FLAG_S_IS_OPEN_AIR)) {
                {
                  texture_fullid background_tex;
                  if (rf & VIEWPORT_RAMP_FLAG_S_IS_DARK_CORNER)
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_se_s_tri_heavy;
                  else
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_se_s_tri_light;
                  background_tex.r = 1;
                  background_tex.g = 1;
                  background_tex.b = 1;
                  background_tex.br = 0;
              ...
              if (!(rf & VIEWPORT_RAMP_FLAG_E_IS_OPEN_AIR)) {
                {
                  texture_fullid background_tex;
                  if (rf & VIEWPORT_RAMP_FLAG_E_IS_DARK_CORNER)
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_se_e_tri_heavy;
                  else
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_se_e_tri_light;
                  background_tex.r = 1;
                  background_tex.g = 1;
                  background_tex.b = 1;
                  background_tex.br = 0;
              ...
              if (!(rf & VIEWPORT_RAMP_FLAG_N_IS_OPEN_AIR)) {
                {
                  texture_fullid background_tex;
                  if (rf & VIEWPORT_RAMP_FLAG_N_IS_DARK_CORNER)
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_ne_n_tri_heavy;
                  else
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_ne_n_tri_light;
                  background_tex.r = 1;
                  background_tex.g = 1;
                  background_tex.b = 1;
                  background_tex.br = 0;
              ...
              if (!(rf & VIEWPORT_RAMP_FLAG_E_IS_OPEN_AIR)) {
                {
                  ...
                  if (rf & VIEWPORT_RAMP_FLAG_E_IS_DARK_CORNER)
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_ne_e_tri_heavy;
                  else
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_ne_e_tri_light;
                  background_tex.r = 1;
                  background_tex.g = 1;
                  background_tex.b = 1;
                  background_tex.br = 0;
              ...
              if (!(rf & VIEWPORT_RAMP_FLAG_N_IS_OPEN_AIR)) {
                {
                  ...
                  if (rf & VIEWPORT_RAMP_FLAG_N_IS_DARK_CORNER)
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_nw_n_tri_heavy;
                  else
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_nw_n_tri_light;
                  background_tex.r = 1;
                  background_tex.g = 1;
                  background_tex.b = 1;
                  background_tex.br = 0;
              ...
              if (!(rf & VIEWPORT_RAMP_FLAG_W_IS_OPEN_AIR)) {
                {
                  texture_fullid background_tex;
                  if (rf & VIEWPORT_RAMP_FLAG_W_IS_DARK_CORNER)
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_nw_w_tri_heavy;
                  else
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_nw_w_tri_light;
                  background_tex.r = 1;
                  background_tex.g = 1;
                  background_tex.b = 1;
                  background_tex.br = 0;
              ...
              if (!(rf & VIEWPORT_RAMP_FLAG_S_IS_OPEN_AIR)) {
                {
                  ...
                  if (rf & VIEWPORT_RAMP_FLAG_S_IS_DARK_CORNER)
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_sw_s_tri_heavy;
                  else
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_sw_s_tri_light;
                  background_tex.r = 1;
                  background_tex.g = 1;
                  background_tex.b = 1;
                  background_tex.br = 0;
              ...
              if (!(rf & VIEWPORT_RAMP_FLAG_W_IS_OPEN_AIR)) {
                {
                  ...
                  if (rf & VIEWPORT_RAMP_FLAG_W_IS_DARK_CORNER)
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_sw_w_tri_heavy;
                  else
                    background_tex.texpos =
                        gps.ramp_shadow_on_ramp_corner_sw_w_tri_light;
                  background_tex.r = 1;
                  background_tex.g = 1;
                  background_tex.b = 1;
                  background_tex.br = 0;
        ...
        if (ff != 0) {
          if (ff & VIEWPORT_FLOOR_FLAG_S_EDGING) {
            Edging type = (ff & VIEWPORT_FLOOR_FLAG_S_EDGING) >>
                          VIEWPORT_FLOOR_FLAG_S_EDGING_SHIFT;

            texture_fullid background_tex;
            if (type == EDGING_STONE)
              background_tex.texpos = gps.stone_floor_texpos[0][1];
            else if (type == EDGING_SOIL)
              background_tex.texpos = gps.dirt_floor_texpos[0][1];
            else if (type == EDGING_SOIL_SAND)
              background_tex.texpos = gps.texpos_floor_sand[0][1];
            else if (type == EDGING_SOIL_SAND_YELLOW)
              background_tex.texpos = gps.texpos_floor_sand_yellow[0][1];
            else if (type == EDGING_SOIL_SAND_WHITE)
              background_tex.texpos = gps.texpos_floor_sand_white[0][1];
            else if (type == EDGING_SOIL_SAND_BLACK)
              background_tex.texpos = gps.texpos_floor_sand_black[0][1];
            else if (type == EDGING_SOIL_SAND_RED)
              background_tex.texpos = gps.texpos_floor_sand_red[0][1];
              ...
              background_tex.texpos = gps.grass_texpos[0][1];
            background_tex.r = 1;
            background_tex.g = 1;
            background_tex.b = 1;
            background_tex.br = 0;
          ...
          if (ff & VIEWPORT_FLOOR_FLAG_W_EDGING) {
            Edging type = (ff & VIEWPORT_FLOOR_FLAG_W_EDGING) >>
                          VIEWPORT_FLOOR_FLAG_W_EDGING_SHIFT;

            texture_fullid background_tex;
            if (type == EDGING_STONE)
              background_tex.texpos = gps.stone_floor_texpos[0][5];
            else if (type == EDGING_SOIL)
              background_tex.texpos = gps.dirt_floor_texpos[0][5];
            else if (type == EDGING_SOIL_SAND)
              background_tex.texpos = gps.texpos_floor_sand[0][5];
            else if (type == EDGING_SOIL_SAND_YELLOW)
              background_tex.texpos = gps.texpos_floor_sand_yellow[0][5];
            else if (type == EDGING_SOIL_SAND_WHITE)
              background_tex.texpos = gps.texpos_floor_sand_white[0][5];
            else if (type == EDGING_SOIL_SAND_BLACK)
              background_tex.texpos = gps.texpos_floor_sand_black[0][5];
            else if (type == EDGING_SOIL_SAND_RED)
              background_tex.texpos = gps.texpos_floor_sand_red[0][5];
            else if (type >= EDGING_CUSTOM_GRASS_1 &&
                     type < EDGING_CUSTOM_GRASS_32) {
              type -= EDGING_CUSTOM_GRASS_1;
              background_tex.texpos = gps.custom_grass_edge_texpos[type][5];
            } else
              background_tex.texpos = gps.grass_texpos[0][5];
            background_tex.r = 1;
            background_tex.g = 1;
            background_tex.b = 1;
            background_tex.br = 0;
          ...
          if (ff & VIEWPORT_FLOOR_FLAG_E_EDGING) {
            Edging type = (ff & VIEWPORT_FLOOR_FLAG_E_EDGING) >>
                          VIEWPORT_FLOOR_FLAG_E_EDGING_SHIFT;

            texture_fullid background_tex;
            if (type == EDGING_STONE)
              background_tex.texpos = gps.stone_floor_texpos[0][3];
            else if (type == EDGING_SOIL)
              background_tex.texpos = gps.dirt_floor_texpos[0][3];
            else if (type == EDGING_SOIL_SAND)
              background_tex.texpos = gps.texpos_floor_sand[0][3];
            else if (type == EDGING_SOIL_SAND_YELLOW)
              background_tex.texpos = gps.texpos_floor_sand_yellow[0][3];
            else if (type == EDGING_SOIL_SAND_WHITE)
              background_tex.texpos = gps.texpos_floor_sand_white[0][3];
            else if (type == EDGING_SOIL_SAND_BLACK)
              background_tex.texpos = gps.texpos_floor_sand_black[0][3];
            else if (type == EDGING_SOIL_SAND_RED)
              background_tex.texpos = gps.texpos_floor_sand_red[0][3];
            else if (type >= EDGING_CUSTOM_GRASS_1 &&
                     type < EDGING_CUSTOM_GRASS_32) {
              type -= EDGING_CUSTOM_GRASS_1;
              background_tex.texpos = gps.custom_grass_edge_texpos[type][3];
            } else
              background_tex.texpos = gps.grass_texpos[0][3];
            background_tex.r = 1;
            background_tex.g = 1;
            background_tex.b = 1;
            background_tex.br = 0;
          ...
          if (ff & VIEWPORT_FLOOR_FLAG_N_EDGING) {
            Edging type = (ff & VIEWPORT_FLOOR_FLAG_N_EDGING) >>
                          VIEWPORT_FLOOR_FLAG_N_EDGING_SHIFT;

            texture_fullid background_tex;
            if (type == EDGING_STONE)
              background_tex.texpos = gps.stone_floor_texpos[0][7];
            else if (type == EDGING_SOIL)
              background_tex.texpos = gps.dirt_floor_texpos[0][7];
            else if (type == EDGING_SOIL_SAND)
              background_tex.texpos = gps.texpos_floor_sand[0][7];
            else if (type == EDGING_SOIL_SAND_YELLOW)
              background_tex.texpos = gps.texpos_floor_sand_yellow[0][7];
            else if (type == EDGING_SOIL_SAND_WHITE)
              background_tex.texpos = gps.texpos_floor_sand_white[0][7];
            else if (type == EDGING_SOIL_SAND_BLACK)
              background_tex.texpos = gps.texpos_floor_sand_black[0][7];
            else if (type == EDGING_SOIL_SAND_RED)
              background_tex.texpos = gps.texpos_floor_sand_red[0][7];
            else if (type >= EDGING_CUSTOM_GRASS_1 &&
                     type < EDGING_CUSTOM_GRASS_32) {
              type -= EDGING_CUSTOM_GRASS_1;
              background_tex.texpos = gps.custom_grass_edge_texpos[type][7];
            } else
              background_tex.texpos = gps.grass_texpos[0][7];
            background_tex.r = 1;
            background_tex.g = 1;
            background_tex.b = 1;
            background_tex.br = 0;
          ...
          if ((ff & VIEWPORT_FLOOR_FLAG_S_EDGING) &&
            ...
            Edging type_s = (ff & VIEWPORT_FLOOR_FLAG_S_EDGING) >>
                            VIEWPORT_FLOOR_FLAG_S_EDGING_SHIFT;
            Edging type_w = (ff & VIEWPORT_FLOOR_FLAG_W_EDGING) >>
                            VIEWPORT_FLOOR_FLAG_W_EDGING_SHIFT;

            texture_fullid background_tex;
            background_tex.texpos = 0;
            if (type_s == EDGING_STONE && type_w == EDGING_STONE)
              background_tex.texpos = gps.stone_floor_texpos[0][2];
            else if (type_s == EDGING_SOIL && type_w == EDGING_SOIL)
              background_tex.texpos = gps.dirt_floor_texpos[0][2];
            else if (type_s == EDGING_SOIL_SAND && type_w == EDGING_SOIL_SAND)
              background_tex.texpos = gps.texpos_floor_sand[0][2];
            else if (type_s == EDGING_SOIL_SAND_YELLOW &&
                     type_w == EDGING_SOIL_SAND_YELLOW)
              background_tex.texpos = gps.texpos_floor_sand_yellow[0][2];
            else if (type_s == EDGING_SOIL_SAND_WHITE &&
                     type_w == EDGING_SOIL_SAND_WHITE)
              background_tex.texpos = gps.texpos_floor_sand_white[0][2];
            else if (type_s == EDGING_SOIL_SAND_BLACK &&
                     type_w == EDGING_SOIL_SAND_BLACK)
              background_tex.texpos = gps.texpos_floor_sand_black[0][2];
            else if (type_s == EDGING_SOIL_SAND_RED &&
                     type_w == EDGING_SOIL_SAND_RED)
              background_tex.texpos = gps.texpos_floor_sand_red[0][2];
            else if (type_s == EDGING_GRASS && type_w == EDGING_GRASS)
              background_tex.texpos = gps.grass_texpos[0][2];
          ...
          if ((ff & VIEWPORT_FLOOR_FLAG_S_EDGING) &&
            ...
            Edging type_s = (ff & VIEWPORT_FLOOR_FLAG_S_EDGING) >>
                            VIEWPORT_FLOOR_FLAG_S_EDGING_SHIFT;
            Edging type_e = (ff & VIEWPORT_FLOOR_FLAG_E_EDGING) >>
                            VIEWPORT_FLOOR_FLAG_E_EDGING_SHIFT;

            texture_fullid background_tex;
            background_tex.texpos = 0;
            if (type_s == EDGING_STONE && type_e == EDGING_STONE)
              background_tex.texpos = gps.stone_floor_texpos[0][0];
            else if (type_s == EDGING_SOIL && type_e == EDGING_SOIL)
              background_tex.texpos = gps.dirt_floor_texpos[0][0];
            else if (type_s == EDGING_SOIL_SAND && type_e == EDGING_SOIL_SAND)
              background_tex.texpos = gps.texpos_floor_sand[0][0];
            else if (type_s == EDGING_SOIL_SAND_YELLOW &&
                     type_e == EDGING_SOIL_SAND_YELLOW)
              background_tex.texpos = gps.texpos_floor_sand_yellow[0][0];
            else if (type_s == EDGING_SOIL_SAND_WHITE &&
                     type_e == EDGING_SOIL_SAND_WHITE)
              background_tex.texpos = gps.texpos_floor_sand_white[0][0];
            else if (type_s == EDGING_SOIL_SAND_BLACK &&
                     type_e == EDGING_SOIL_SAND_BLACK)
              background_tex.texpos = gps.texpos_floor_sand_black[0][0];
            else if (type_s == EDGING_SOIL_SAND_RED &&
                     type_e == EDGING_SOIL_SAND_RED)
              background_tex.texpos = gps.texpos_floor_sand_red[0][0];
            else if (type_s == EDGING_GRASS && type_e == EDGING_GRASS)
              background_tex.texpos = gps.grass_texpos[0][0];
            else if ((type_s >= EDGING_CUSTOM_GRASS_1 &&
                      type_s < EDGING_CUSTOM_GRASS_32) &&
          ...
          if ((ff & VIEWPORT_FLOOR_FLAG_N_EDGING) &&
              (ff & VIEWPORT_FLOOR_FLAG_W_EDGING)) {
            Edging type_n = (ff & VIEWPORT_FLOOR_FLAG_N_EDGING) >>
                            VIEWPORT_FLOOR_FLAG_N_EDGING_SHIFT;
            Edging type_w = (ff & VIEWPORT_FLOOR_FLAG_W_EDGING) >>
                            VIEWPORT_FLOOR_FLAG_W_EDGING_SHIFT;
            ...
            texture_fullid background_tex;
            background_tex.texpos = 0;
            if (type_n == EDGING_STONE && type_w == EDGING_STONE)
              background_tex.texpos = gps.stone_floor_texpos[0][8];
            else if (type_n == EDGING_SOIL && type_w == EDGING_SOIL)
              background_tex.texpos = gps.dirt_floor_texpos[0][8];
            else if (type_n == EDGING_SOIL_SAND && type_w == EDGING_SOIL_SAND)
              background_tex.texpos = gps.texpos_floor_sand[0][8];
            else if (type_n == EDGING_SOIL_SAND_YELLOW &&
                     type_w == EDGING_SOIL_SAND_YELLOW)
              background_tex.texpos = gps.texpos_floor_sand_yellow[0][8];
            else if (type_n == EDGING_SOIL_SAND_WHITE &&
                     type_w == EDGING_SOIL_SAND_WHITE)
              background_tex.texpos = gps.texpos_floor_sand_white[0][8];
            else if (type_n == EDGING_SOIL_SAND_BLACK &&
                     type_w == EDGING_SOIL_SAND_BLACK)
              background_tex.texpos = gps.texpos_floor_sand_black[0][8];
            else if (type_n == EDGING_SOIL_SAND_RED &&
                     type_w == EDGING_SOIL_SAND_RED)
              background_tex.texpos = gps.texpos_floor_sand_red[0][8];
            else if (type_n == EDGING_GRASS && type_w == EDGING_GRASS)
              background_tex.texpos = gps.grass_texpos[0][8];
          ...
          if ((ff & VIEWPORT_FLOOR_FLAG_N_EDGING) &&
            ...
            texture_fullid background_tex;
            background_tex.texpos = 0;
            if (type_n == EDGING_STONE && type_e == EDGING_STONE)
              background_tex.texpos = gps.stone_floor_texpos[0][6];
            else if (type_n == EDGING_SOIL && type_e == EDGING_SOIL)
              background_tex.texpos = gps.dirt_floor_texpos[0][6];
            else if (type_n == EDGING_SOIL_SAND && type_e == EDGING_SOIL_SAND)
              background_tex.texpos = gps.texpos_floor_sand[0][6];
            else if (type_n == EDGING_SOIL_SAND_YELLOW &&
                     type_e == EDGING_SOIL_SAND_YELLOW)
              background_tex.texpos = gps.texpos_floor_sand_yellow[0][6];
            else if (type_n == EDGING_SOIL_SAND_WHITE &&
                     type_e == EDGING_SOIL_SAND_WHITE)
              background_tex.texpos = gps.texpos_floor_sand_white[0][6];
            else if (type_n == EDGING_SOIL_SAND_BLACK &&
                     type_e == EDGING_SOIL_SAND_BLACK)
              background_tex.texpos = gps.texpos_floor_sand_black[0][6];
            else if (type_n == EDGING_SOIL_SAND_RED &&
                     type_e == EDGING_SOIL_SAND_RED)
              background_tex.texpos = gps.texpos_floor_sand_red[0][6];
            else if (type_n == EDGING_GRASS && type_e == EDGING_GRASS)
              background_tex.texpos = gps.grass_texpos[0][6];
            else if ((type_n >= EDGING_CUSTOM_GRASS_1 &&
                      type_n < EDGING_CUSTOM_GRASS_32) &&
                     (type_e >= EDGING_CUSTOM_GRASS_1 &&
                      type_e < EDGING_CUSTOM_GRASS_32) &&
        ...
        if (vp->screentexpos_liquid_flag[x * vp->dim_y + y] != 0) {
          ...
          uint32_t center_type = (lf & VIEWPORT_LIQUID_FLAG_CENTER_TYPE) >>
                                 VIEWPORT_LIQUID_FLAG_CENTER_SHIFT;
          uint32_t center_level = (lf & VIEWPORT_LIQUID_FLAG_CENTER_LEVEL) >>
                                  VIEWPORT_LIQUID_FLAG_CENTER_LEVEL_SHIFT;
          if (center_type > 0 && center_level > 0) {
            texture_fullid background_tex;
            switch (center_type) {
            case VIEWPORT_LIQUID_TYPE_WATER:
              if (vp->flag & GRAPHIC_VIEWPORT_FLAG_SHOW_LIQUID_NUMBERS)
                background_tex.texpos =
                    gps.underwater_label_texpos[center_level];
              else
                background_tex.texpos = gps.underwater_texpos[center_level];
              break;
            case VIEWPORT_LIQUID_TYPE_MAGMA:
              if (vp->flag & GRAPHIC_VIEWPORT_FLAG_SHOW_LIQUID_NUMBERS)
                background_tex.texpos =
                    gps.undermagma_label_texpos[center_level];
              else
                background_tex.texpos = gps.undermagma_texpos[center_level];
              break;
            }
            background_tex.r = 1;
            background_tex.g = 1;
            ...
            uint32_t n_type = (lf & VIEWPORT_LIQUID_FLAG_N_EDGE_TYPE) >>
                              VIEWPORT_LIQUID_FLAG_N_EDGE_SHIFT;
            uint32_t s_type = (lf & VIEWPORT_LIQUID_FLAG_S_EDGE_TYPE) >>
                              VIEWPORT_LIQUID_FLAG_S_EDGE_SHIFT;
            uint32_t w_type = (lf & VIEWPORT_LIQUID_FLAG_W_EDGE_TYPE) >>
                              VIEWPORT_LIQUID_FLAG_W_EDGE_SHIFT;
            uint32_t e_type = (lf & VIEWPORT_LIQUID_FLAG_E_EDGE_TYPE) >>
                              VIEWPORT_LIQUID_FLAG_E_EDGE_SHIFT;
            if (s_type != 0) {
              texture_fullid background_tex;
              switch (s_type) {
              case VIEWPORT_LIQUID_TYPE_WATER:
                background_tex.texpos = gps.underwater_edge_texpos[1][0];
                break;
              case VIEWPORT_LIQUID_TYPE_MAGMA:
                background_tex.texpos = gps.undermagma_edge_texpos[1][0];
                break;
              }
              background_tex.r = 1;
              background_tex.g = 1;
            ...
            if (w_type != 0) {
              ...
              switch (w_type) {
              case VIEWPORT_LIQUID_TYPE_WATER:
                background_tex.texpos = gps.underwater_edge_texpos[2][1];
                break;
              case VIEWPORT_LIQUID_TYPE_MAGMA:
                background_tex.texpos = gps.undermagma_edge_texpos[2][1];
                break;
              }
              background_tex.r = 1;
              background_tex.g = 1;
            ...
            if (e_type != 0) {
              ...
              switch (e_type) {
              case VIEWPORT_LIQUID_TYPE_WATER:
                background_tex.texpos = gps.underwater_edge_texpos[0][1];
                break;
              case VIEWPORT_LIQUID_TYPE_MAGMA:
                background_tex.texpos = gps.undermagma_edge_texpos[0][1];
                break;
              }
              background_tex.r = 1;
              background_tex.g = 1;
            ...
            if (n_type != 0) {
              ...
              switch (n_type) {
              case VIEWPORT_LIQUID_TYPE_WATER:
                background_tex.texpos = gps.underwater_edge_texpos[1][2];
                break;
              case VIEWPORT_LIQUID_TYPE_MAGMA:
                background_tex.texpos = gps.undermagma_edge_texpos[1][2];
                break;
              }
              background_tex.r = 1;
              background_tex.g = 1;
            ...
            if (s_type != 0 && w_type != 0 && s_type == w_type) {
              ...
              switch (s_type) {
              case VIEWPORT_LIQUID_TYPE_WATER:
                background_tex.texpos = gps.underwater_edge_texpos[2][0];
                break;
              case VIEWPORT_LIQUID_TYPE_MAGMA:
                background_tex.texpos = gps.undermagma_edge_texpos[2][0];
                break;
              }
              if (background_tex.texpos != 0) {
                background_tex.r = 1;
            ...
            if (s_type != 0 && e_type != 0 && s_type == e_type) {
              ...
              switch (s_type) {
              case VIEWPORT_LIQUID_TYPE_WATER:
                background_tex.texpos = gps.underwater_edge_texpos[0][0];
                break;
              case VIEWPORT_LIQUID_TYPE_MAGMA:
                background_tex.texpos = gps.undermagma_edge_texpos[0][0];
                break;
              }
              if (background_tex.texpos != 0) {
                background_tex.r = 1;
            ...
            if (n_type != 0 && w_type != 0 && n_type == w_type) {
              ...
              switch (n_type) {
              case VIEWPORT_LIQUID_TYPE_WATER:
                background_tex.texpos = gps.underwater_edge_texpos[2][2];
                break;
              case VIEWPORT_LIQUID_TYPE_MAGMA:
                background_tex.texpos = gps.undermagma_edge_texpos[2][2];
                break;
              }
              if (background_tex.texpos != 0) {
                background_tex.r = 1;
            ...
            if (n_type != 0 && e_type != 0 && n_type == e_type) {
              ...
              switch (n_type) {
              case VIEWPORT_LIQUID_TYPE_WATER:
                background_tex.texpos = gps.underwater_edge_texpos[0][2];
                break;
              case VIEWPORT_LIQUID_TYPE_MAGMA:
                background_tex.texpos = gps.undermagma_edge_texpos[0][2];
                break;
              }
              if (background_tex.texpos != 0) {
                background_tex.r = 1;
        ...
        if (vp->screentexpos_background_two[x * vp->dim_y + y] != 0) {
          texture_fullid background_tex;
          background_tex.texpos =
              vp->screentexpos_background_two[x * vp->dim_y + y];
          background_tex.r = 1.0f;
          background_tex.g = 1.0f;
          background_tex.b = 1.0f;
          background_tex.br = 0;
        ...
        if (vp->screentexpos_building_one[x * vp->dim_y + y] != 0) {
          texture_fullid background_tex;
          background_tex.texpos =
              vp->screentexpos_building_one[x * vp->dim_y + y];
          background_tex.r = 1;
          background_tex.g = 1;
          background_tex.b = 1;
          background_tex.br = 0;
        ...
        if (vp->screentexpos_right_creature[x * vp->dim_y + y] != 0) {
          texture_fullid background_tex;
          background_tex.texpos =
              vp->screentexpos_right_creature[x * vp->dim_y + y];
          background_tex.r = 1;
          background_tex.g = 1;
          background_tex.b = 1;
          background_tex.br = 0;
        ...
        if (vp->screentexpos_left_creature[x * vp->dim_y + y] != 0) {
          texture_fullid background_tex;
          background_tex.texpos =
              vp->screentexpos_left_creature[x * vp->dim_y + y];
          background_tex.r = 1;
          background_tex.g = 1;
          background_tex.b = 1;
          background_tex.br = 0;
        ...
        if (vp->screentexpos_building_two[x * vp->dim_y + y] != 0) {
          texture_fullid background_tex;
          background_tex.texpos =
              vp->screentexpos_building_two[x * vp->dim_y + y];
          background_tex.r = 1;
          background_tex.g = 1;
          background_tex.b = 1;
          background_tex.br = 0;
        ...
        if (vp->screentexpos_projectile[x * vp->dim_y + y] != 0) {
          texture_fullid background_tex;
          background_tex.texpos =
              vp->screentexpos_projectile[x * vp->dim_y + y];
          background_tex.r = 1;
          background_tex.g = 1;
          background_tex.b = 1;
          background_tex.br = 0;
        ...
        if (vp->screentexpos_upright_creature[x * vp->dim_y + y] != 0) {
          texture_fullid background_tex;
          background_tex.texpos =
              vp->screentexpos_upright_creature[x * vp->dim_y + y];
          background_tex.r = 1;
          background_tex.g = 1;
          background_tex.b = 1;
          background_tex.br = 0;
        ...
        if (vp->screentexpos_up_creature[x * vp->dim_y + y] != 0) {
          texture_fullid background_tex;
          background_tex.texpos =
              vp->screentexpos_up_creature[x * vp->dim_y + y];
          background_tex.r = 1;
          background_tex.g = 1;
          background_tex.b = 1;
          background_tex.br = 0;
        ...
        if (vp->screentexpos_upleft_creature[x * vp->dim_y + y] != 0) {
          texture_fullid background_tex;
          background_tex.texpos =
              vp->screentexpos_upleft_creature[x * vp->dim_y + y];
          background_tex.r = 1;
          background_tex.g = 1;
          background_tex.b = 1;
          background_tex.br = 0;
        ...
        if (vp->screentexpos_designation[x * vp->dim_y + y] != 0) {
          texture_fullid background_tex;
          background_tex.texpos =
              vp->screentexpos_designation[x * vp->dim_y + y];
          background_tex.r = 1;
          background_tex.g = 1;
          background_tex.b = 1;
          background_tex.br = 0;
        ...
        if (x >= 0 && y >= 0 && dispx_z > 0 && dispy_z > 0) // just in case
        {
          ...
          int32_t tx = (viewport_zoom_factor * 32) / 128,
                  ty = (viewport_zoom_factor * 32) / 128;
          int32_t sx = x * tx / dispx_z;
          int32_t sy = y * ty / dispy_z;
          int32_t dx = tx / dispx_z + 1;
          int32_t dy = ty / dispy_z + 2;
  ...
  void update_all() {
    ...
    if (gps.display_background) {
      ...
      SDL_RenderCopy(sdl_renderer,
                     gps.tex[TEXTURE_TITLE_BACKGROUND].get_texture(), NULL,
                     &dst);
    }
    if (gps.display_title) {
      ...
      if (window_h >= 800)
        dst.y += (window_h - 800) / 3;
      // SDL_SetSurfaceAlphaMod(tex, 0);
      SDL_RenderCopy(sdl_renderer, gps.tex[TEXTURE_TITLE].get_texture(), NULL,
                     &dst);
    }
  ...
  void clean_cached_tile(int32_t texpos, float r, float g, float b, float br,
                         float bg, float bb, uint32_t flag) {
  ...
  virtual SDL_Renderer* get_renderer() { return sdl_renderer; }

  virtual SDL_Window* get_window() { return window; }

  virtual void render() {
    // Render the TTFs, which we left for last
    /*    for (auto it = ttfs_to_render.begin(); it != ttfs_to_render.end();
       ++it) { SDL_BlitSurface(it->first, NULL, screen, &it->second);
        }
  ...
  void compute_forced_zoom() {
    ...
    const int dispx =
        (enabler.flag & ENABLERFLAG_BASIC_TEXT)
            ...
            : (enabler.is_fullscreen() ? init.font.large_font_dispx
                                       : init.font.small_font_dispx);
    const int dispy =
        (enabler.flag & ENABLERFLAG_BASIC_TEXT)
            ? init.font.basic_font_dispy
            : (enabler.is_fullscreen() ? init.font.large_font_dispy
                                       : init.font.small_font_dispy);

    if (init.display.flag.has_flag(
      ...
      if (dispx < dispy) {
        int32_t desired_dim =
            natural_w * 100 / init.display.interface_scaling_percentage;
        ...
        int32_t desired_dim =
            natural_h * 100 / init.display.interface_scaling_percentage;
  ...
  void resize(int w, int h) {
    if (w < MINIMUM_WINDOW_WIDTH)
      w = MINIMUM_WINDOW_WIDTH;
    if (h < MINIMUM_WINDOW_HEIGHT)
      h = MINIMUM_WINDOW_HEIGHT;
  ...
  void reshape(pair<int, int> max_grid) {
    ...
    // Compute the largest tile size that will fit this grid into the window,
    // roughly maintaining aspect ratio
    double try_x = dispx, try_y = dispy;
    try_x = cur_w / w;
    try_y = MIN(try_x / dispx * dispy, cur_h / h);
    try_x = MIN(try_x, try_y / dispy * dispx);
    dispx_z = (int)(MAX(1, try_x));
    dispy_z = (int)(MAX(try_y, 1));
    cout << "Resizing font to " << dispx_z << "x" << dispy_z << endl;
    // Recompute grid based on the new tile size
    //*********************************** SCREEN SIZE 1
    // where does the mouse wheel stuff enter into this?  still need to be able
    // to zoom out the 32x32 stuff too
    //************************** WIDE SCREENS
...
private:
  void set_fullscreen() {
    ...
    if (fullscreen & FULLSCREEN) {
      if (init.display.desired_fullscreen_width == 0 ||
        ...
        w = init.display.actual_fullscreen_width =
            init.display.desired_fullscreen_width;
        h = init.display.actual_fullscreen_height =
  ...
  bool get_precise_mouse_coords(int& px, int& py, int& x, int& y) {
    ...
    mouse_x -= origin_x;
    mouse_y -= origin_y;
    if (mouse_x < 0 || mouse_x >= dispx_z * dimx || mouse_y < 0 ||
        mouse_y >= dispy_z * dimy)
      return false;
    px = mouse_x;
    py = mouse_y;
    x = mouse_x / dispx_z;
...
public:
  renderer_2d() {
    ...
    bool worked = init_video(
        enabler.is_fullscreen() ? init.display.actual_fullscreen_width
                                : init.display.actual_windowed_width,
        enabler.is_fullscreen() ? init.display.actual_fullscreen_height
