...
void draw_nineslice(int32_t* slice, int sy, int sx, int ey, int ex,
                    bool override = false);
...
struct scrollbarst {
  ...
  void init(int32_t n_sel, int32_t n_smn, int32_t n_smx, int32_t npnm,
            int32_t psy, int32_t pey) {
    sel = n_sel;
    sel_min = n_smn;
    sel_max = n_smx;
    resize(npnm, psy, pey);
    ...
    if (scroller_sy == scroller_ey) {
      if (scroller_sy < print_ey) {
        ...
        if (scroller_ey >= print_ey && sel < sel_max - page_size + 1) {
          ...
          --scroller_ey;
          if (scroller_sy < print_sy)
            scroller_sy = print_sy; // only happens if scrollbar is height <=2
          scroller_ey = scroller_sy + width;
        }
      } else {
        ...
        if (scroller_sy < print_sy)
          scroller_sy =
              print_sy; // only happens if scrollbar is height 1...  or less
        scroller_ey = scroller_sy + width;
      }
    }
  ...
  void render(int32_t mouse_x, int32_t mouse_y, int32_t bx, bool active,
              uint32_t display_flag = 0);
  void handle_initial_click(int32_t mouse_y, int32_t& scroll_position,
                            bool& scrolling);
  bool handle_events(std::set<InterfaceKey>& events, int32_t& scroll_position,
                     bool& scrolling);
};
...
protected:
  abstract_interfacest*
      parent; // will be a widget unless it's the top-level viewscreen widget
...
// Generic container widget, no custom arrangement etc logic. Every viewscreen
// has one of these to make it easier to add widgets to them
...
// Keeps things in columns--not sure anything in vanilla is like this, no
// arrangement from the right or anything right now
...
// Just displays a string where it's put, nothing special--only here to allow
// text to conform to the widget system
...
// Give it one of the nineslice texboxes and it'll automagically draw it in its
// extents. Science?
...
public:
  ...
  nineslice(int32_t* which) { set_texpos(which); }

  void set_texpos(int32_t* which) { selected_texpos = which; }
};

// INPUT WIDGETS

// BUTTON
// Actually checking if the mouse button and capturing such clicks is up to the
// callback, as is behavior
...
public:
  ...
  virtual void render() {
  } // this is an "abstract button", it doesn't render--up to whatever's using
    // it to do that, this just have the click behavior

  virtual void logic() {}
};

// BETTER BUTTON
// The button's cooler younger sibling. Learned to render and display text on
// its own. Requires a lot of functions, though.
...
public:
  ...
  void set_callback(std::function<void()> f) { callback = f; }

  void set_check(std::function<bool()> f) { check_truth = f; }

  void set_textures(std::function<int32_t*()> f) { texpos = f; }

  void set_strings(std::function<string()> f) { display_string = f; }
...
public:
  ...
  virtual void arrange() {
    toggle.set_extents(rect.y, rect.y + rect.h, rect.x + rect.w - 4,
                       rect.x + rect.w);
  }
};
...
template <typename T> class menu {
  ...
  int first_line(int total, int sel, int space) {
    // There is no doubt some clever math to do this, but I'm tired and don't
    // care.
...
public:
  ...
  void set(int line, string text, T token) { lines[line] = mp(text, token); }

  // Set the color of a line
  void set_color(int line, int fg, int bg) { colors[line] = make_pair(fg, bg); }
