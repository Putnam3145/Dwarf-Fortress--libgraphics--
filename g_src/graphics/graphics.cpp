...
void process_object_lines(textlinesst& lines, const string& chktype,
                          const string& graphics_dir);
...
void graphicst::reshape_viewports(int32_t zoom_factor) {
  if (main_viewport != NULL) {
    main_viewport->shape_viewport_according_by_pixel_size(
        screen_pixel_x, screen_pixel_y, zoom_factor);
  }
  int32_t lvp = LOWER_VIEWPORT_MAX - 1;
  while (lvp >= 0) {
    if (lower_viewport[lvp] != NULL)
      lower_viewport[lvp]->shape_viewport_according_by_pixel_size(
          screen_pixel_x, screen_pixel_y, zoom_factor);
    --lvp;
  }
  ...
  if (main_map_port != NULL) {
    ...
    // also: need to be more careful about this cutoff -- perc introduces some
    // error so at certain zooms it is way off (the -16 is a cludge around this)
    main_map_port->top_left_corner_x =
        gps.screen_pixel_x - (px + 15) / 16 * 16 - 16;
    main_map_port->top_left_corner_y =
        32; //*********************** TEXTURE SIZE DEPENDENCE
    if (main_map_port->pixel_perc_x == 100)
      main_map_port->top_left_corner_x = 0;
    if (main_map_port->pixel_perc_y == 100)
      main_map_port->top_left_corner_y = 0;

    main_map_port->shape_map_port_according_by_pixel_size(px, py);
  }
}

void graphic_viewportst::shape_viewport_according_by_pixel_size(
    int32_t px, int32_t py, int32_t zoom_factor) {
  if (zoom_factor != 128) {
...
void graphic_map_portst::shape_map_port_according_by_pixel_size(int32_t px,
                                                                int32_t py) {
  //*************************** TEXTURE SIZE DEPENDENCE
  int32_t ndx = px / 16;
  int32_t ndy = py / 16;
...
void graphicst::addcoloredst(const char* str, const char* colorstr) {
  ...
  for (s = 0; s < slen && screenx < init.display.grid_x; s++) {
    ...
    changecolor((colorstr[s] & 7), ((colorstr[s] & 56)) >> 3,
                ((colorstr[s] & 64)) >> 6);
    addchar(str[s]);
  }
}
...
static void abbreviate_string_helper_hackaroundmissingcode(string& str,
                                                           int len) {
  if (str.length() >= 2) {
  ...
  for (l = (int32_t)str.length() - 1; l >= 1; l--) {
    ...
    if (str[l] == 'a' || str[l] == 'e' || str[l] == 'i' || str[l] == 'o' ||
        str[l] == 'u' || str[l] == 'A' || str[l] == 'E' || str[l] == 'I' ||
        str[l] == 'O' || str[l] == 'U') {
...
void graphicst::addst_flag(const string& str_orig, justification just,
                           int space, uint32_t sflag) {
  if (!str_orig.size())
    return;
  string str = str_orig;
  if (space)
...
void graphicst::addst(const string& str_orig, justification just, int space) {
  ...
  /*
    ...
    if (str.size() > 2 && str[0] == ':' && str[1] == ' ')
      str[1] = '\t'; // EVIL HACK
    struct ttf_id id = {str, (unsigned char)screenf, (unsigned char)screenb,
  (unsigned char)screenbright}; ttfstr.push_back(id);
    // if (str.size() == 80) {
    //   cout << "(" << int(str[0]) << ") ";
    // }
...
void graphicst::top_addst_flag(const string& str_orig, justification just,
                               int space, uint32_t sflag) {
  if (!str_orig.size())
    return;
  string str = str_orig;
...
void graphicst::top_addst(const string& str_orig, justification just,
                          int space) {
  if (!str_orig.size())
    return;
  string str = str_orig;
  ...
  if (x >= clipx[0] && x <= clipx[1] && y >= clipy[0] && y <= clipy[1]) {
    ...
    switch(dim)
      ...
      case 4:
        ...
        if(screen[x*dimy*4 + y*4 + 1]==screen[x*dimy*4 + y*4 +
    2])screen[x*dimy*4 + y*4 + 1]=0; screen[x*dimy*4 + y*4 + 3]=0;
        if(screen[x*dimy*4 + y*4 + 1]==0&&screen[x*dimy*4 + y*4 +
    2]==0&&screen[x*dimy*4 + y*4 + 3]==0)screen[x*dimy*4 + y*4 + 3]=1; break;
      case 3:
        switch(screen[x*dimy*4 + y*4 + 2])
          {
          case 4:
...
void graphicst::color_square(long x, long y, unsigned char f, unsigned char b,
                             unsigned char br) {
  if (x >= clipx[0] && x <= clipx[1] && y >= clipy[0] && y <= clipy[1]) {
    //******************************** DIM COLORS
    // beware -- *4 should be *8 in most places now
...
void graphicst::prepare_graphics(const string& src_dir) {
  ...
  // MUST INSURE THAT THEY ARE LOADED IN THE PROPER ORDER, IN CASE THEY REFER TO
  // EACH OTHER
  {
    string chk = src_dir;
  ...
  // reset_textures called later, as there are still universe-specific textures
  // to derive
}

void graphicst::add_lower_tile(long texp) {
  if (screenx >= clipx[0] && screenx <= clipx[1] && screeny >= clipy[0] &&
      screeny <= clipy[1]) {
    if (!init.display.flag.has_flag(INIT_DISPLAY_FLAG_USE_GRAPHICS)) {
      ...
      if (s < screen_limit) {
        unsigned char c = (unsigned char)(texp & (BIT1 | BIT2 | BIT3 | BIT4 |
                                                  BIT5 | BIT6 | BIT7 | BIT8));
...
void graphicst::add_anchored_tile(long texp, long offset_x, long offset_y,
                                  long tdx, long tdy, bool use_color) {
  int32_t sx, sy;
  uint32_t ox, oy;
  for (sx = screenx, ox = 0; sx < screenx + tdx; ++sx, ++ox) {
    for (sy = screeny, oy = 0; sy < screeny + tdy; ++sy, ++oy) {
      if (sx >= clipx[0] && sx <= clipx[1] && sy >= clipy[0] &&
          sy <= clipy[1]) {
        ...
        if (sx != screenx || sy != screeny) {
          ...
          *stp_f |=
              ((ox > 63 ? 0 : ox) << SCREENTEXPOS_FLAG_ANCHOR_X_COORD_SHIFT);
          *stp_f |=
              ((oy > 63 ? 0 : oy) << SCREENTEXPOS_FLAG_ANCHOR_Y_COORD_SHIFT);
        } else
          screentexpos_flag[sx * dimy + sy] &=
              ~SCREENTEXPOS_FLAG_ANCHOR_SUBORDINATE;
        if (use_color) {
          ...
          screentexpos_flag[sx * dimy + sy] |=
              SCREENTEXPOS_FLAG_ANCHOR_USE_SCREEN_COLOR;
        } else
          screentexpos_flag[sx * dimy + sy] &=
              ~SCREENTEXPOS_FLAG_ANCHOR_USE_SCREEN_COLOR;
      }
...
void graphicst::add_top_anchored_tile(long texp, long offset_x, long offset_y,
                                      long tdx, long tdy, bool use_color) {
  int32_t sx, sy;
  uint32_t ox, oy;
  for (sx = screenx, ox = 0; sx < screenx + tdx; ++sx, ++ox) {
      ...
      if (sx >= clipx[0] && sx <= clipx[1] && sy >= clipy[0] &&
        ...
        if (sx != screenx || sy != screeny) {
          ...
          *stp_f |=
          ...
          screentexpos_top_flag[sx * dimy + sy] &=
              ~SCREENTEXPOS_FLAG_ANCHOR_SUBORDINATE;
        if (use_color) {
          int32_t ind = (sx * dimy + sy) * 8;
          if (use_old_16_colors) {
...
void graphicst::add_tile(long texp, char addcolor) {
  if (screenx >= clipx[0] && screenx <= clipx[1] && screeny >= clipy[0] &&
    ...
    if (addcolor)
      screentexpos_flag[screenx * dimy + screeny] |= SCREENTEXPOS_FLAG_ADDCOLOR;
  }
}
...
void graphicst::add_top_tile(long texp, char addcolor) {
  if (screenx >= clipx[0] && screenx <= clipx[1] && screeny >= clipy[0] &&
    ...
    if (addcolor)
      screentexpos_top_flag[screenx * dimy + screeny] |=
          SCREENTEXPOS_FLAG_ADDCOLOR;
  }
}
...
void render_things() {
  ...
  // Render REC when recording macros. Definitely want this screen-specific. Or
  // do we?
  ...
  if (gps.display_frames) {
    ...
    fps_stream << "FPS: " << setw(3) << enabler.calculate_fps() << " ("
               << enabler.calculate_gfps() << ")";
...
void graphic_viewportst::add_building_one_tile(long texp) {
  if (screen_x >= clipx[0] && screen_x <= clipx[1] && screen_y >= clipy[0] &&
      screen_y <= clipy[1]) {
...
void graphic_viewportst::add_spatter_flag_fire(uint32_t nspatter_flag) {
  if (screen_x >= clipx[0] && screen_x <= clipx[1] && screen_y >= clipy[0] &&
      screen_y <= clipy[1]) {
    screentexpos_spatter_flag[screen_x * dim_y + screen_y] &=
        ~VIEWPORT_SPATTER_FLAG_FIRE_BITS;
...
void graphicst::copy_pixel_data_with_alpha_mask(SDL_Surface* src,
                                                SDL_Surface* alpha_src,
                                                SDL_Surface* dst) {
  ...
  for (y = 0; y < src->h; y++) {
    ...
    for (x = 0; x < src->w;
         ++x, pixel_src += 4, pixel_alpha_src += 4, pixel_dst += 4) {
      pixel_dst[0] = pixel_src[0];
      pixel_dst[1] = pixel_src[1];
      pixel_dst[2] = pixel_src[2];
      pixel_dst[3] =
          (Uint8)((int32_t)pixel_src[3] * (int32_t)pixel_alpha_src[3] / 255);
    }
  }
}

void graphicst::copy_transformed_pixel_data(SDL_Surface* src, SDL_Surface* dst,
                                            int32_t cw_rotate, bool flip_vert) {
    ...
    if (cw_rotate == 0) {
      ...
      for (y = 0; y < src->h; y++) {
        ...
        for (x = 0; x < src->w; ++x, pixel_src += 4) {
          pixel_dst = ((Uint8*)dst->pixels) + ((src->w - 1 - x) * dst->pitch) +
                      (src->h - 1 - y) * 4;
  ...
  if (cw_rotate == 0) {
    ...
    for (y = 0; y < src->h; y++) {
      ...
      for (x = 0; x < src->w; ++x, pixel_src += 4) {
        pixel_dst =
            ((Uint8*)dst->pixels) + (x * dst->pitch) + (src->h - 1 - y) * 4;
    ...
    for (y = 0; y < src->h; y++) {
      ...
      for (x = 0; x < src->w; ++x, pixel_src += 4) {
        pixel_dst =
            ((Uint8*)dst->pixels) + ((src->w - 1 - x) * dst->pitch) + y * 4;
...
void graphicst::create_derived_textures() {
  ...
  if(wall_shadow_near_n_open_ne!=0)
    {
    SDL_Surface
  *tex=enabler.textures.get_texture_data(wall_shadow_near_n_open_ne);
    if(tex!=NULL)
      {
      SDL_LockSurface(tex);
  ...
  if (wall_shadow_end_wall_w != 0) {
    SDL_Surface* tex =
        enabler.textures.get_texture_data(wall_shadow_end_wall_w);
    if (tex != NULL) {
      SDL_LockSurface(tex);
      ...
      for (i = 0; i < 3; ++i) {
        switch (i) {
        case 0:
          tpp = &wall_shadow_end_wall_n;
          break;
        case 1:
          tpp = &wall_shadow_end_wall_s;
          break;
        case 2:
          tpp = &wall_shadow_end_wall_e;
          break;
        }
        *tpp = enabler.textures.create_texture(tex->w, tex->h);
        ...
        if (color != NULL) {
          ...
          switch (i) {
          case 0:
            copy_transformed_pixel_data(tex, color, 90, false);
            break;
          case 1:
            copy_transformed_pixel_data(tex, color, 270, false);
            break;
          case 2:
            copy_transformed_pixel_data(tex, color, 180, false);
            break;
          }
  ...
  if (multilevel_depth_texpos[0] != 0) {
    SDL_Surface* src =
        enabler.textures.get_texture_data(multilevel_depth_texpos[0]);
    if (src != NULL) {
      SDL_LockSurface(src);

      int32_t i;
      for (i = 1; i < LOWER_VIEWPORT_MAX; ++i) {
        multilevel_depth_texpos[i] =
            enabler.textures.create_texture(src->w, src->h);
        SDL_Surface* dst =
            enabler.textures.get_texture_data(multilevel_depth_texpos[i]);
        if (dst != NULL) {
          SDL_LockSurface(dst);
  ...
  if (gps.texpos_item_cage_metal != 0)
    gps.texpos_planned_bld_cage =
        create_alpha_texture(gps.texpos_item_cage_metal, alpha_level);
  if (gps.texpos_item_bag != 0)
    gps.texpos_planned_bld_bag =
        create_alpha_texture(gps.texpos_item_bag, alpha_level);
  if (gps.texpos_item_box != 0)
    gps.texpos_planned_bld_box =
        create_alpha_texture(gps.texpos_item_box, alpha_level);
  if (gps.texpos_item_cabinet != 0)
    gps.texpos_planned_bld_cabinet =
        create_alpha_texture(gps.texpos_item_cabinet, alpha_level);
  if (gps.texpos_item_table != 0)
    gps.texpos_planned_bld_table =
        create_alpha_texture(gps.texpos_item_table, alpha_level);
  if (gps.texpos_item_animal_trap_metal != 0)
    gps.texpos_planned_bld_animal_trap =
        create_alpha_texture(gps.texpos_item_animal_trap_metal, alpha_level);
  if (gps.texpos_item_chair != 0)
    gps.texpos_planned_bld_chair =
        create_alpha_texture(gps.texpos_item_chair, alpha_level);
  if (gps.texpos_item_bed != 0)
    gps.texpos_planned_bld_bed =
        create_alpha_texture(gps.texpos_item_bed, alpha_level);
  if (gps.texpos_item_statue != 0)
    gps.texpos_planned_bld_statue =
        create_alpha_texture(gps.texpos_item_statue, alpha_level);
  if (gps.texpos_item_slab_blank != 0)
    gps.texpos_planned_bld_slab =
        create_alpha_texture(gps.texpos_item_slab_blank, alpha_level);
  if (gps.texpos_item_hatch_cover_stone != 0)
    gps.texpos_planned_bld_hatch_cover =
        create_alpha_texture(gps.texpos_item_hatch_cover_stone, alpha_level);
  if (gps.texpos_item_hatch_cover_wooden != 0)
    gps.texpos_planned_bld_hatch_cover =
        create_alpha_texture(gps.texpos_item_hatch_cover_wooden, alpha_level);
  if (gps.texpos_item_hatch_cover_metal != 0)
    gps.texpos_planned_bld_hatch_cover =
        create_alpha_texture(gps.texpos_item_hatch_cover_metal, alpha_level);
  if (gps.texpos_item_hatch_cover_glass != 0)
    gps.texpos_planned_bld_hatch_cover =
        create_alpha_texture(gps.texpos_item_hatch_cover_glass, alpha_level);
  if (gps.texpos_item_coffin_wood != 0)
    texpos_planned_bld_coffin_wooden =
        create_alpha_texture(gps.texpos_item_coffin_wood, alpha_level);
  if (gps.texpos_item_coffin_stone != 0)
    texpos_planned_bld_coffin_stone =
        create_alpha_texture(gps.texpos_item_coffin_stone, alpha_level);
  if (gps.texpos_item_coffin_metal != 0)
    texpos_planned_bld_coffin_metal =
        create_alpha_texture(gps.texpos_item_coffin_metal, alpha_level);
  if (gps.texpos_item_coffin_glass != 0)
    texpos_planned_bld_coffin_glass =
        create_alpha_texture(gps.texpos_item_coffin_glass, alpha_level);
  if (gps.texpos_item_traction_bench_stone_chain != 0)
    texpos_planned_bld_traction_bench_stone_chain = create_alpha_texture(
        gps.texpos_item_traction_bench_stone_chain, alpha_level);
  if (gps.texpos_item_traction_bench_stone_rope != 0)
    texpos_planned_bld_traction_bench_stone_rope = create_alpha_texture(
        gps.texpos_item_traction_bench_stone_rope, alpha_level);
  if (gps.texpos_item_traction_bench_wooden_chain != 0)
    texpos_planned_bld_traction_bench_wooden_chain = create_alpha_texture(
        gps.texpos_item_traction_bench_wooden_chain, alpha_level);
  if (gps.texpos_item_traction_bench_wooden_rope != 0)
    texpos_planned_bld_traction_bench_wooden_rope = create_alpha_texture(
        gps.texpos_item_traction_bench_wooden_rope, alpha_level);
  if (gps.texpos_item_traction_bench_metal_chain != 0)
    texpos_planned_bld_traction_bench_metal_chain = create_alpha_texture(
        gps.texpos_item_traction_bench_metal_chain, alpha_level);
  if (gps.texpos_item_traction_bench_metal_rope != 0)
    texpos_planned_bld_traction_bench_metal_rope = create_alpha_texture(
        gps.texpos_item_traction_bench_metal_rope, alpha_level);
  if (gps.texpos_item_traction_bench_glass_chain != 0)
    texpos_planned_bld_traction_bench_glass_chain = create_alpha_texture(
        gps.texpos_item_traction_bench_glass_chain, alpha_level);
  if (gps.texpos_item_traction_bench_glass_rope != 0)
    texpos_planned_bld_traction_bench_glass_rope = create_alpha_texture(
        gps.texpos_item_traction_bench_glass_rope, alpha_level);
  if (gps.texpos_item_door_stone != 0)
    gps.texpos_planned_bld_door_stone =
        create_alpha_texture(gps.texpos_item_door_stone, alpha_level);
  if (gps.texpos_item_door_wooden != 0)
    gps.texpos_planned_bld_door_wooden =
        create_alpha_texture(gps.texpos_item_door_wooden, alpha_level);
  if (gps.texpos_item_door_metal != 0)
    gps.texpos_planned_bld_door_metal =
        create_alpha_texture(gps.texpos_item_door_metal, alpha_level);
  if (gps.texpos_item_door_glass != 0)
    gps.texpos_planned_bld_door_glass =
        create_alpha_texture(gps.texpos_item_door_glass, alpha_level);

  if (gps.workshop_quern_texpos[1][0] != 0 &&
      gps.workshop_quern_texpos[0][0] == 0)
    gps.workshop_quern_texpos[0][0] =
        create_alpha_texture(gps.workshop_quern_texpos[1][0], alpha_level);
  if (gps.workshop_quern_texpos[1][1] != 0 &&
      gps.workshop_quern_texpos[0][1] == 0)
    gps.workshop_quern_texpos[0][1] =
        create_alpha_texture(gps.workshop_quern_texpos[1][1], alpha_level);
  if (gps.workshop_millstone_texpos[1][0] != 0 &&
      gps.workshop_millstone_texpos[0][0] == 0)
    gps.workshop_millstone_texpos[0][0] =
        create_alpha_texture(gps.workshop_millstone_texpos[1][0], alpha_level);
  if (gps.workshop_millstone_texpos[1][1] != 0 &&
      gps.workshop_millstone_texpos[0][1] == 0)
    gps.workshop_millstone_texpos[0][1] =
        create_alpha_texture(gps.workshop_millstone_texpos[1][1], alpha_level);

  if (gps.texpos_bld_chain_metal[0][0] != 0)
    gps.texpos_planned_bld_chain_metal =
        create_alpha_texture(gps.texpos_bld_chain_metal[0][0], alpha_level);
  if (gps.texpos_bld_chain_rope[0][0] != 0)
    gps.texpos_planned_bld_chain_rope =
        create_alpha_texture(gps.texpos_bld_chain_rope[0][0], alpha_level);
  if (gps.texpos_bld_archery_target_stone != 0)
    gps.texpos_planned_bld_archery_target_stone =
        create_alpha_texture(gps.texpos_bld_archery_target_stone, alpha_level);
  if (gps.texpos_bld_archery_target_metal != 0)
    gps.texpos_planned_bld_archery_target_metal =
        create_alpha_texture(gps.texpos_bld_archery_target_metal, alpha_level);
  if (gps.texpos_bld_archery_target_wood != 0)
    gps.texpos_planned_bld_archery_target_wood =
        create_alpha_texture(gps.texpos_bld_archery_target_wood, alpha_level);
  if (gps.texpos_bld_well != 0)
    gps.texpos_planned_bld_well =
        create_alpha_texture(gps.texpos_bld_well, alpha_level);
  if (gps.texpos_item_grate_metal_floor_closed != 0)
    gps.texpos_planned_bld_floor_grate = create_alpha_texture(
        gps.texpos_item_grate_metal_floor_closed, alpha_level);
  if (gps.texpos_item_grate_metal_wall_closed != 0)
    gps.texpos_planned_bld_wall_grate = create_alpha_texture(
        gps.texpos_item_grate_metal_wall_closed, alpha_level);
  if (gps.texpos_item_floodgate_stone != 0)
    gps.texpos_planned_bld_floodgate_stone =
        create_alpha_texture(gps.texpos_item_floodgate_stone, alpha_level);
  if (gps.texpos_item_floodgate_wooden != 0)
    gps.texpos_planned_bld_floodgate_wooden =
        create_alpha_texture(gps.texpos_item_floodgate_wooden, alpha_level);
  if (gps.texpos_item_floodgate_metal != 0)
    gps.texpos_planned_bld_floodgate_metal =
        create_alpha_texture(gps.texpos_item_floodgate_metal, alpha_level);
  if (gps.texpos_item_floodgate_glass != 0)
    gps.texpos_planned_bld_floodgate_glass =
        create_alpha_texture(gps.texpos_item_floodgate_glass, alpha_level);
  if (gps.texpos_bld_paved_road != 0)
    gps.texpos_planned_bld_paved_road =
        create_alpha_texture(gps.texpos_bld_paved_road, alpha_level);
  if (gps.texpos_bld_dirt_road != 0)
    gps.texpos_planned_bld_dirt_road =
        create_alpha_texture(gps.texpos_bld_dirt_road, alpha_level);
      ...
      for (i = 0; i < 10; ++i) {
        ...
        *dst = enabler.textures.create_texture(tex->w, tex->h);
        ...
        for (si = 1; si < 3; ++si) {
          ...
          // NOTE: a blit did not work here, it overwrote instead of doing alpha
          // blending
          SDL_LockSurface(tex);
          SDL_LockSurface(dsts);
          ...
          for (y = 0; y < tex->h; y++) {
            ...
            for (x = 0; x < tex->w; ++x, pixel_src += 4, pixel_dst += 4) {
              //******************************** BROOKBED
              // instead of using the most alpha pixel, can we get the blend
              // formula? note that for bars here, we have if(pixel_src[3]!=0)
              // instead of the dst<src
              if (pixel_src[3] != 0) {
                pixel_dst[0] = pixel_src[0];
                pixel_dst[1] = pixel_src[1];
                pixel_dst[2] = pixel_src[2];
  ...
  if (gps.texpos_farmplot != 0)
    gps.texpos_planned_farmplot =
        create_alpha_texture(gps.texpos_farmplot, alpha_level);

  if (gps.texpos_lever_1 != 0)
    gps.texpos_planned_lever =
        create_alpha_texture(gps.texpos_lever_1, alpha_level);
  if (gps.texpos_trap_cage != 0)
    gps.texpos_planned_trap_cage =
        create_alpha_texture(gps.texpos_trap_cage, alpha_level);
  if (gps.texpos_trap_stone != 0)
    gps.texpos_planned_trap_stone =
        create_alpha_texture(gps.texpos_trap_stone, alpha_level);
  if (gps.texpos_trap_weapon != 0)
    gps.texpos_planned_trap_weapon =
        create_alpha_texture(gps.texpos_trap_weapon, alpha_level);
  if (gps.texpos_trap_plate != 0)
    gps.texpos_planned_trap_plate =
        create_alpha_texture(gps.texpos_trap_plate, alpha_level);
  if (gps.texpos_upright_weapon_retracted[9] != 0)
    gps.texpos_planned_upright_weapon = create_alpha_texture(
        gps.texpos_upright_weapon_retracted[9], alpha_level);

  if (gps.texpos_gear_assembly[0] != 0)
    gps.texpos_planned_gear_assembly =
        create_alpha_texture(gps.texpos_gear_assembly[0], alpha_level);

  {
    ...
    for (sp = 0; sp < 21; ++sp) {
      ...
      switch (sp) {
      case 0:
        spp = &gps.stockpile_nw_up_texpos;
        n_up = true;
        w_up = true;
        break;
      case 1:
        spp = &gps.stockpile_nw_texpos;
        n = true;
        w = true;
        has_floor = true;
        break;
      case 2:
        spp = &gps.stockpile_ne_up_texpos;
        n_up = true;
        e_up = true;
        break;
      case 3:
        spp = &gps.stockpile_ne_texpos;
        n = true;
        e = true;
        has_floor = true;
        break;
      case 4:
        spp = &gps.stockpile_sw_texpos;
        s = true;
        w = true;
        has_floor = true;
        break;
      case 5:
        spp = &gps.stockpile_se_texpos;
        s = true;
        e = true;
        has_floor = true;
        break;
      case 6:
        spp = &gps.stockpile_ns_up_texpos;
        n_up = true;
        break;
      case 7:
        spp = &gps.stockpile_ns_texpos;
        n = true;
        s = true;
        has_floor = true;
        break;
      case 8:
        spp = &gps.stockpile_we_texpos;
        w = true;
        e = true;
        has_floor = true;
        break;
      case 9:
        spp = &gps.stockpile_nsw_up_texpos;
        n_up = true;
        w_up = true;
        break;
      case 10:
        spp = &gps.stockpile_nsw_texpos;
        n = true;
        s = true;
        w = true;
        has_floor = true;
        break;
      case 11:
        spp = &gps.stockpile_nse_up_texpos;
        n_up = true;
        e_up = true;
        break;
      case 12:
        spp = &gps.stockpile_nse_texpos;
        n = true;
        s = true;
        e = true;
        has_floor = true;
        break;
      case 13:
        spp = &gps.stockpile_nwe_up_texpos;
        n_up = true;
        w_up = true;
        e_up = true;
        break;
      case 14:
        spp = &gps.stockpile_nwe_texpos;
        n = true;
        w = true;
        e = true;
        has_floor = true;
      ...
      case 15:
        ...
        break;
      case 16:
        spp = &gps.stockpile_nswe_up_texpos;
        n_up = true;
        w_up = true;
        e_up = true;
        break;
      case 17:
        spp = &gps.stockpile_nswe_texpos;
        n = true;
        s = true;
        w = true;
        e = true;
        ...
        break;
      case 18:
        spp = &gps.stockpile_n_texpos;
        has_floor = true;
        already_exists = true;
        break;
      case 19:
        spp = &gps.stockpile_w_texpos;
        has_floor = true;
        already_exists = true;
        break;
      case 20:
        spp = &gps.stockpile_e_texpos;
        has_floor = true;
        already_exists = true;
        break;
      }
      ...
      for (i = 0; i < 8; ++i) {
        ...
        case 0:
          chkb = &s;
          src_pos = &stockpile_s_texpos;
          break;
        case 1:
          chkb = &w_up;
          src_pos = &stockpile_w_up_texpos;
          break;
        case 2:
          chkb = &w;
          src_pos = &stockpile_w_texpos;
          break;
        case 3:
          chkb = &e_up;
          src_pos = &stockpile_e_up_texpos;
          break;
        case 4:
          chkb = &e;
          src_pos = &stockpile_e_texpos;
          break;
        case 5:
          chkb = &n_up;
          src_pos = &stockpile_n_up_texpos;
          break;
        case 6:
          chkb = &n;
          src_pos = &stockpile_n_texpos;
          break;
        case 7:
          chkb = &has_floor;
          src_pos = &stockpile_floor_texpos;
          break;
        }
        if (*chkb) {
          ...
          if (src != NULL) {
            if (dst == NULL) {
              if (!already_exists)
                *spp = enabler.textures.create_texture(src->w, src->h);
  ...
  {
    SDL_Surface* src =
        enabler.textures.get_texture_data(stockpile_ne_up_texpos);
    if (src != NULL) {
      SDL_LockSurface(src);
      int32_t sp;
      for (sp = 0; sp < 19; ++sp) {
        ...
        switch (sp) {
        case 0:
          tp = stockpile_blank_texpos;
          break;
        case 1:
          tp = stockpile_ammo_texpos;
          break;
        case 2:
          tp = stockpile_animals_texpos;
          break;
        case 3:
          tp = stockpile_armor_texpos;
          break;
        case 4:
          tp = stockpile_bars_texpos;
          break;
        case 5:
          tp = stockpile_cloth_texpos;
          break;
        case 6:
          tp = stockpile_coins_texpos;
          break;
        case 7:
          tp = stockpile_corpses_texpos;
          break;
        case 8:
          tp = stockpile_finished_goods_texpos;
          break;
        case 9:
          tp = stockpile_food_texpos;
          break;
        case 10:
          tp = stockpile_furniture_texpos;
          break;
        case 11:
          tp = stockpile_gems_texpos;
          break;
        case 12:
          tp = stockpile_leather_texpos;
          break;
        case 13:
          tp = stockpile_refuse_texpos;
          break;
        case 14:
          tp = stockpile_sheets_texpos;
          break;
        case 15:
          tp = stockpile_stone_texpos;
          break;
        case 16:
          tp = stockpile_weapons_texpos;
          break;
        case 17:
          tp = stockpile_wood_texpos;
          break;
        case 18:
          tp = stockpile_custom_texpos;
          break;
        }
  ...
  // debris -- uses EGA colors, based on 15 (which is white debris), two
  // variants
  {
    int32_t d, v;
    for (v = 0; v < 2; ++v) {
      ...
      for (d = 0; d < 15; ++d) {
        gps.texpos_debris[d][v] =
            enabler.textures.create_texture(src->w, src->h);
        SDL_Surface* dst =
            enabler.textures.get_texture_data(gps.texpos_debris[d][v]);
        if (dst == NULL)
          continue;
  ...
  // small world map corner cutups
  {
    ...
    for (i = 0; i < 55; ++i) {
      if (i < 25)
        var_num = REGION_SQUARE_MOUNTAIN_NUM;
      else
        var_num = REGION_SQUARE_FOREST_NUM;
      int32_t v, c;
      for (v = 0; v < var_num; ++v) {
        int32_t* src_tx = NULL;
        switch (i) {
        case 0:
          src_tx = &texpos_mountain_peak[v];
          break;
        case 1:
          src_tx = &texpos_mountain_peak_evil[v];
          break;
        case 2:
          src_tx = &texpos_mountain_peak_evilsav[v];
          break;
        case 3:
          src_tx = &texpos_mountain_peak_good[v];
          break;
        case 4:
          src_tx = &texpos_mountain_peak_goodsav[v];
          break;
        case 5:
          src_tx = &texpos_volcano[v];
          break;
        case 6:
          src_tx = &texpos_volcano_evil[v];
          break;
        case 7:
          src_tx = &texpos_volcano_evilsav[v];
          break;
        case 8:
          src_tx = &texpos_volcano_good[v];
          break;
        case 9:
          src_tx = &texpos_volcano_goodsav[v];
          break;
        case 10:
          src_tx = &texpos_mountain_high[v];
          break;
        case 11:
          src_tx = &texpos_mountain_high_evil[v];
          break;
        case 12:
          src_tx = &texpos_mountain_high_evilsav[v];
          break;
        case 13:
          src_tx = &texpos_mountain_high_good[v];
          break;
        case 14:
          src_tx = &texpos_mountain_high_goodsav[v];
          break;
        case 15:
          src_tx = &texpos_mountain_mid[v];
          break;
        case 16:
          src_tx = &texpos_mountain_mid_evil[v];
          break;
        case 17:
          src_tx = &texpos_mountain_mid_evilsav[v];
          break;
        case 18:
          src_tx = &texpos_mountain_mid_good[v];
          break;
        case 19:
          src_tx = &texpos_mountain_mid_goodsav[v];
          break;
        case 20:
          src_tx = &texpos_mountain_low[v];
          break;
        case 21:
          src_tx = &texpos_mountain_low_evil[v];
          break;
        case 22:
          src_tx = &texpos_mountain_low_evilsav[v];
          break;
        case 23:
          src_tx = &texpos_mountain_low_good[v];
          break;
        case 24:
          src_tx = &texpos_mountain_low_goodsav[v];
          break;
        case 25:
          src_tx = &texpos_forest_broadleaf_trop_moist[v];
          break;
        case 26:
          src_tx = &texpos_forest_broadleaf_trop_moist_evil[v];
          break;
        case 27:
          src_tx = &texpos_forest_broadleaf_trop_moist_evilsav[v];
          break;
        case 28:
          src_tx = &texpos_forest_broadleaf_trop_moist_good[v];
          break;
        case 29:
          src_tx = &texpos_forest_broadleaf_trop_moist_goodsav[v];
          break;
        case 30:
          src_tx = &texpos_forest_broadleaf_trop_dry[v];
          break;
        case 31:
          src_tx = &texpos_forest_broadleaf_trop_dry_evil[v];
          break;
        case 32:
          src_tx = &texpos_forest_broadleaf_trop_dry_evilsav[v];
          break;
        case 33:
          src_tx = &texpos_forest_broadleaf_trop_dry_good[v];
          break;
        case 34:
          src_tx = &texpos_forest_broadleaf_trop_dry_goodsav[v];
          break;
        case 35:
          src_tx = &texpos_forest_broadleaf_temp[v];
          break;
        case 36:
          src_tx = &texpos_forest_broadleaf_temp_evil[v];
          break;
        case 37:
          src_tx = &texpos_forest_broadleaf_temp_evilsav[v];
          break;
        case 38:
          src_tx = &texpos_forest_broadleaf_temp_good[v];
          break;
        case 39:
          src_tx = &texpos_forest_broadleaf_temp_goodsav[v];
          break;
        case 40:
          src_tx = &texpos_forest_conifer_trop[v];
          break;
        case 41:
          src_tx = &texpos_forest_conifer_trop_evil[v];
          break;
        case 42:
          src_tx = &texpos_forest_conifer_trop_evilsav[v];
          break;
        case 43:
          src_tx = &texpos_forest_conifer_trop_good[v];
          break;
        case 44:
          src_tx = &texpos_forest_conifer_trop_goodsav[v];
          break;
        case 45:
          src_tx = &texpos_forest_conifer_temp[v];
          break;
        case 46:
          src_tx = &texpos_forest_conifer_temp_evil[v];
          break;
        case 47:
          src_tx = &texpos_forest_conifer_temp_evilsav[v];
          break;
        case 48:
          src_tx = &texpos_forest_conifer_temp_good[v];
          break;
        case 49:
          src_tx = &texpos_forest_conifer_temp_goodsav[v];
          break;
        case 50:
          src_tx = &texpos_forest_taiga[v];
          break;
        case 51:
          src_tx = &texpos_forest_taiga_evil[v];
          break;
        case 52:
          src_tx = &texpos_forest_taiga_evilsav[v];
          break;
        case 53:
          src_tx = &texpos_forest_taiga_good[v];
          break;
        case 54:
          src_tx = &texpos_forest_taiga_goodsav[v];
          break;
        }
        if (src_tx == NULL)
          continue;
        ...
        for (c = 0; c < 9; ++c) {
          ...
          switch (i) {
          case 0:
            dst_tx = &texpos_mountain_peak_c[v][c];
            break;
          case 1:
            dst_tx = &texpos_mountain_peak_evil_c[v][c];
            break;
          case 2:
            dst_tx = &texpos_mountain_peak_evilsav_c[v][c];
            break;
          case 3:
            dst_tx = &texpos_mountain_peak_good_c[v][c];
            break;
          case 4:
            dst_tx = &texpos_mountain_peak_goodsav_c[v][c];
            break;
          case 5:
            dst_tx = &texpos_volcano_c[v][c];
            break;
          case 6:
            dst_tx = &texpos_volcano_evil_c[v][c];
            break;
          case 7:
            dst_tx = &texpos_volcano_evilsav_c[v][c];
            break;
          case 8:
            dst_tx = &texpos_volcano_good_c[v][c];
            break;
          case 9:
            dst_tx = &texpos_volcano_goodsav_c[v][c];
            break;
          case 10:
            dst_tx = &texpos_mountain_high_c[v][c];
            break;
          case 11:
            dst_tx = &texpos_mountain_high_evil_c[v][c];
            break;
          case 12:
            dst_tx = &texpos_mountain_high_evilsav_c[v][c];
            break;
          case 13:
            dst_tx = &texpos_mountain_high_good_c[v][c];
            break;
          case 14:
            dst_tx = &texpos_mountain_high_goodsav_c[v][c];
            break;
          case 15:
            dst_tx = &texpos_mountain_mid_c[v][c];
            break;
          case 16:
            dst_tx = &texpos_mountain_mid_evil_c[v][c];
            break;
          case 17:
            dst_tx = &texpos_mountain_mid_evilsav_c[v][c];
            break;
          case 18:
            dst_tx = &texpos_mountain_mid_good_c[v][c];
            break;
          case 19:
            dst_tx = &texpos_mountain_mid_goodsav_c[v][c];
            break;
          case 20:
            dst_tx = &texpos_mountain_low_c[v][c];
            break;
          case 21:
            dst_tx = &texpos_mountain_low_evil_c[v][c];
            break;
          case 22:
            dst_tx = &texpos_mountain_low_evilsav_c[v][c];
            break;
          case 23:
            dst_tx = &texpos_mountain_low_good_c[v][c];
            break;
          case 24:
            dst_tx = &texpos_mountain_low_goodsav_c[v][c];
            break;
          case 25:
            dst_tx = &texpos_forest_broadleaf_trop_moist_c[v][c];
            break;
          case 26:
            dst_tx = &texpos_forest_broadleaf_trop_moist_evil_c[v][c];
            break;
          case 27:
            dst_tx = &texpos_forest_broadleaf_trop_moist_evilsav_c[v][c];
            break;
          case 28:
            dst_tx = &texpos_forest_broadleaf_trop_moist_good_c[v][c];
            break;
          case 29:
            dst_tx = &texpos_forest_broadleaf_trop_moist_goodsav_c[v][c];
            break;
          case 30:
            dst_tx = &texpos_forest_broadleaf_trop_dry_c[v][c];
            break;
          case 31:
            dst_tx = &texpos_forest_broadleaf_trop_dry_evil_c[v][c];
            break;
          case 32:
            dst_tx = &texpos_forest_broadleaf_trop_dry_evilsav_c[v][c];
            break;
          case 33:
            dst_tx = &texpos_forest_broadleaf_trop_dry_good_c[v][c];
            break;
          case 34:
            dst_tx = &texpos_forest_broadleaf_trop_dry_goodsav_c[v][c];
            break;
          case 35:
            dst_tx = &texpos_forest_broadleaf_temp_c[v][c];
            break;
          case 36:
            dst_tx = &texpos_forest_broadleaf_temp_evil_c[v][c];
            break;
          case 37:
            dst_tx = &texpos_forest_broadleaf_temp_evilsav_c[v][c];
            break;
          case 38:
            dst_tx = &texpos_forest_broadleaf_temp_good_c[v][c];
            break;
          case 39:
            dst_tx = &texpos_forest_broadleaf_temp_goodsav_c[v][c];
            break;
          case 40:
            dst_tx = &texpos_forest_conifer_trop_c[v][c];
            break;
          case 41:
            dst_tx = &texpos_forest_conifer_trop_evil_c[v][c];
            break;
          case 42:
            dst_tx = &texpos_forest_conifer_trop_evilsav_c[v][c];
            break;
          case 43:
            dst_tx = &texpos_forest_conifer_trop_good_c[v][c];
            break;
          case 44:
            dst_tx = &texpos_forest_conifer_trop_goodsav_c[v][c];
            break;
          case 45:
            dst_tx = &texpos_forest_conifer_temp_c[v][c];
            break;
          case 46:
            dst_tx = &texpos_forest_conifer_temp_evil_c[v][c];
            break;
          case 47:
            dst_tx = &texpos_forest_conifer_temp_evilsav_c[v][c];
            break;
          case 48:
            dst_tx = &texpos_forest_conifer_temp_good_c[v][c];
            break;
          case 49:
            dst_tx = &texpos_forest_conifer_temp_goodsav_c[v][c];
            break;
          case 50:
            dst_tx = &texpos_forest_taiga_c[v][c];
            break;
          case 51:
            dst_tx = &texpos_forest_taiga_evil_c[v][c];
            break;
          case 52:
            dst_tx = &texpos_forest_taiga_evilsav_c[v][c];
            break;
          case 53:
            dst_tx = &texpos_forest_taiga_good_c[v][c];
            break;
          case 54:
            dst_tx = &texpos_forest_taiga_goodsav_c[v][c];
            break;
          }
          if (dst_tx == NULL)
            continue;
            ...
            for (x = sx; x <= ex; ++x, pixel_src += 4) {
              if (pixel_src[3] != 0) {
                if (x >= sx && x <= ex && y >= sy && y <= ey) {
                  ((Uint8*)dst->pixels)[(x - dst_off_x) * 4 +
                                        (y - dst_off_y) * dst->pitch] =
                      pixel_src[0];
                  ((Uint8*)dst->pixels)[(x - dst_off_x) * 4 +
                                        (y - dst_off_y) * dst->pitch + 1] =
                      pixel_src[1];
                  ((Uint8*)dst->pixels)[(x - dst_off_x) * 4 +
                                        (y - dst_off_y) * dst->pitch + 2] =
                      pixel_src[2];
                  ((Uint8*)dst->pixels)[(x - dst_off_x) * 4 +
                                        (y - dst_off_y) * dst->pitch + 3] =
                      pixel_src[3];
                }
              }
            }
  ...
  for (sv = 0; sv < REGION_SQUARE_VARIANT_NUM; ++sv) {
    for (bm = 0; bm < 33; ++bm) {
      ...
      switch (bm) {
      case 0:
        tp = &texpos_grassland_temp[0];
        break;
      case 1:
        tp = &texpos_grassland_temp_evil[0];
        break;
      case 2:
        tp = &texpos_grassland_temp_good[0];
        break;
      case 3:
        tp = &texpos_grassland_trop[0];
        break;
      case 4:
        tp = &texpos_grassland_trop_evil[0];
        break;
      case 5:
        tp = &texpos_grassland_trop_good[0];
        break;
      case 6:
        tp = &texpos_marsh[0];
        break;
      case 7:
        tp = &texpos_rocky_plains[0];
        break;
      case 8:
        tp = &texpos_rocky_plains_evil[0];
        break;
      case 9:
        tp = &texpos_rocky_plains_good[0];
        break;
      case 10:
        tp = &texpos_tundra[0];
        break;
      case 11:
        tp = &texpos_tundra_evil[0];
        break;
      case 12:
        tp = &texpos_tundra_good[0];
        break;
      case 13:
        tp = &texpos_glacier[0];
        break;
      case 14:
        tp = &texpos_glacier_evil[0];
        break;
      case 15:
        tp = &texpos_glacier_good[0];
        break;
      case 16:
        tp = &texpos_ocean[0];
        break;
      case 17:
        tp = &texpos_ocean_evil[0];
        break;
      case 18:
        tp = &texpos_ocean_good[0];
        break;
      case 19:
        tp = &texpos_sand_desert[0];
        break;
      case 20:
        tp = &texpos_sand_desert_yellow[0];
        break;
      case 21:
        tp = &texpos_sand_desert_white[0];
        break;
      case 22:
        tp = &texpos_sand_desert_black[0];
        break;
      case 23:
        tp = &texpos_sand_desert_red[0];
        break;
      case 24:
        tp = &texpos_sand_desert_evil[0];
        break;
      case 25:
        tp = &texpos_sand_desert_good[0];
        break;
      case 26:
        tp = &texpos_marsh_evil[0];
        break;
      case 27:
        tp = &texpos_marsh_good[0];
        break;
      case 28:
        tp = &texpos_beach[0];
        break;
      case 29:
        tp = &texpos_beach_evil[0];
        break;
      case 30:
        tp = &texpos_beach_good[0];
        break;
      case 31:
        tp = &texpos_glacier_evilsav[0];
        break;
      case 32:
        tp = &texpos_glacier_goodsav[0];
        break;
      }
      if (*tp == 0)
        continue;
      ...
      for (siv = 0; siv < 37; ++siv) {
        ...
        switch (siv) {
        case 0:
          alpha_tp = &texpos_world_edge_shape_straight_n[sv];
          break;
        case 1:
          alpha_tp = &texpos_world_edge_shape_straight_s[sv];
          break;
        case 2:
          alpha_tp = &texpos_world_edge_shape_straight_w[sv];
          break;
        case 3:
          alpha_tp = &texpos_world_edge_shape_straight_e[sv];
          break;
        case 4:
          alpha_tp = &texpos_world_edge_shape_thick_corner_nw[sv];
          break;
        case 5:
          alpha_tp = &texpos_world_edge_shape_thick_corner_ne[sv];
          break;
        case 6:
          alpha_tp = &texpos_world_edge_shape_thick_corner_sw[sv];
          break;
        case 7:
          alpha_tp = &texpos_world_edge_shape_thick_corner_se[sv];
          break;
        case 8:
          alpha_tp = &texpos_world_edge_shape_thin_corner_nw[sv];
          break;
        case 9:
          alpha_tp = &texpos_world_edge_shape_thin_corner_ne[sv];
          break;
        case 10:
          alpha_tp = &texpos_world_edge_shape_thin_corner_sw[sv];
          break;
        case 11:
          alpha_tp = &texpos_world_edge_shape_thin_corner_se[sv];
          break;
        case 12:
          alpha_tp = &texpos_world_edge_shape_tiny_corner_nw[sv];
          break;
        case 13:
          alpha_tp = &texpos_world_edge_shape_tiny_corner_ne[sv];
          break;
        case 14:
          alpha_tp = &texpos_world_edge_shape_tiny_corner_sw[sv];
          break;
        case 15:
          alpha_tp = &texpos_world_edge_shape_tiny_corner_se[sv];
          break;
        case 16:
          alpha_tp = &texpos_world_edge_shape_inlet_nsw[sv];
          break;
        case 17:
          alpha_tp = &texpos_world_edge_shape_inlet_nse[sv];
          break;
        case 18:
          alpha_tp = &texpos_world_edge_shape_inlet_nwe[sv];
          break;
        case 19:
          alpha_tp = &texpos_world_edge_shape_inlet_swe[sv];
          break;
        case 20:
          alpha_tp = &texpos_world_edge_shape_surround[sv];
          break;
        case 21:
          alpha_tp = &texpos_world_edge_shape_thinning_n_w[sv];
          break;
        case 22:
          alpha_tp = &texpos_world_edge_shape_thinning_n_e[sv];
          break;
        case 23:
          alpha_tp = &texpos_world_edge_shape_thinning_s_w[sv];
          break;
        case 24:
          alpha_tp = &texpos_world_edge_shape_thinning_s_e[sv];
          break;
        case 25:
          alpha_tp = &texpos_world_edge_shape_thinning_w_n[sv];
          break;
        case 26:
          alpha_tp = &texpos_world_edge_shape_thinning_w_s[sv];
          break;
        case 27:
          alpha_tp = &texpos_world_edge_shape_thinning_e_n[sv];
          break;
        case 28:
          alpha_tp = &texpos_world_edge_shape_thinning_e_s[sv];
          break;
        case 29:
          alpha_tp = &texpos_world_edge_shape_thin_corner_transition_nw_n[sv];
          break;
        case 30:
          alpha_tp = &texpos_world_edge_shape_thin_corner_transition_nw_w[sv];
          break;
        case 31:
          alpha_tp = &texpos_world_edge_shape_thin_corner_transition_ne_n[sv];
          break;
        case 32:
          alpha_tp = &texpos_world_edge_shape_thin_corner_transition_ne_e[sv];
          break;
        case 33:
          alpha_tp = &texpos_world_edge_shape_thin_corner_transition_sw_s[sv];
          break;
        case 34:
          alpha_tp = &texpos_world_edge_shape_thin_corner_transition_sw_w[sv];
          break;
        case 35:
          alpha_tp = &texpos_world_edge_shape_thin_corner_transition_se_s[sv];
          break;
        case 36:
          alpha_tp = &texpos_world_edge_shape_thin_corner_transition_se_e[sv];
          break;
        }
        if (*alpha_tp == 0) {
          switch (siv) {
          case 0:
            alpha_tp = &texpos_world_edge_shape_straight_n[0];
            break;
          case 1:
            alpha_tp = &texpos_world_edge_shape_straight_s[0];
            break;
          case 2:
            alpha_tp = &texpos_world_edge_shape_straight_w[0];
            break;
          case 3:
            alpha_tp = &texpos_world_edge_shape_straight_e[0];
            break;
          case 4:
            alpha_tp = &texpos_world_edge_shape_thick_corner_nw[0];
            break;
          case 5:
            alpha_tp = &texpos_world_edge_shape_thick_corner_ne[0];
            break;
          case 6:
            alpha_tp = &texpos_world_edge_shape_thick_corner_sw[0];
            break;
          case 7:
            alpha_tp = &texpos_world_edge_shape_thick_corner_se[0];
            break;
          case 8:
            alpha_tp = &texpos_world_edge_shape_thin_corner_nw[0];
            break;
          case 9:
            alpha_tp = &texpos_world_edge_shape_thin_corner_ne[0];
            break;
          case 10:
            alpha_tp = &texpos_world_edge_shape_thin_corner_sw[0];
            break;
          case 11:
            alpha_tp = &texpos_world_edge_shape_thin_corner_se[0];
            break;
          case 12:
            alpha_tp = &texpos_world_edge_shape_tiny_corner_nw[0];
            break;
          case 13:
            alpha_tp = &texpos_world_edge_shape_tiny_corner_ne[0];
            break;
          case 14:
            alpha_tp = &texpos_world_edge_shape_tiny_corner_sw[0];
            break;
          case 15:
            alpha_tp = &texpos_world_edge_shape_tiny_corner_se[0];
            break;
          case 16:
            alpha_tp = &texpos_world_edge_shape_inlet_nsw[0];
            break;
          case 17:
            alpha_tp = &texpos_world_edge_shape_inlet_nse[0];
            break;
          case 18:
            alpha_tp = &texpos_world_edge_shape_inlet_nwe[0];
            break;
          case 19:
            alpha_tp = &texpos_world_edge_shape_inlet_swe[0];
            break;
          case 20:
            alpha_tp = &texpos_world_edge_shape_surround[0];
            break;
          case 21:
            alpha_tp = &texpos_world_edge_shape_thinning_n_w[0];
            break;
          case 22:
            alpha_tp = &texpos_world_edge_shape_thinning_n_e[0];
            break;
          case 23:
            alpha_tp = &texpos_world_edge_shape_thinning_s_w[0];
            break;
          case 24:
            alpha_tp = &texpos_world_edge_shape_thinning_s_e[0];
            break;
          case 25:
            alpha_tp = &texpos_world_edge_shape_thinning_w_n[0];
            break;
          case 26:
            alpha_tp = &texpos_world_edge_shape_thinning_w_s[0];
            break;
          case 27:
            alpha_tp = &texpos_world_edge_shape_thinning_e_n[0];
            break;
          case 28:
            alpha_tp = &texpos_world_edge_shape_thinning_e_s[0];
            break;
          case 29:
            alpha_tp = &texpos_world_edge_shape_thin_corner_transition_nw_n[0];
            break;
          case 30:
            alpha_tp = &texpos_world_edge_shape_thin_corner_transition_nw_w[0];
            break;
          case 31:
            alpha_tp = &texpos_world_edge_shape_thin_corner_transition_ne_n[0];
            break;
          case 32:
            alpha_tp = &texpos_world_edge_shape_thin_corner_transition_ne_e[0];
            break;
          case 33:
            alpha_tp = &texpos_world_edge_shape_thin_corner_transition_sw_s[0];
            break;
          case 34:
            alpha_tp = &texpos_world_edge_shape_thin_corner_transition_sw_w[0];
            break;
          case 35:
            alpha_tp = &texpos_world_edge_shape_thin_corner_transition_se_s[0];
            break;
          case 36:
            alpha_tp = &texpos_world_edge_shape_thin_corner_transition_se_e[0];
            break;
          }
          if (*alpha_tp == 0)
            continue;
        ...
        case 0:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_grass_temp_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_grass_temp_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_grass_temp_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_grass_temp_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_grass_temp_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_grass_temp_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_grass_temp_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_grass_temp_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_grass_temp_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_grass_temp_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_grass_temp_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_grass_temp_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_grass_temp_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_grass_temp_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_grass_temp_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_grass_temp_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_grass_temp_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_grass_temp_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_grass_temp_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_grass_temp_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_grass_temp_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_grass_temp_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_grass_temp_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_grass_temp_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_grass_temp_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_grass_temp_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_grass_temp_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_grass_temp_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_grass_temp_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_grass_temp_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_grass_temp_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_grass_temp_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_grass_temp_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_grass_temp_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_grass_temp_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_grass_temp_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_grass_temp_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 1:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_grass_temp_evil_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_grass_temp_evil_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_grass_temp_evil_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_grass_temp_evil_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_grass_temp_evil_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_grass_temp_evil_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_grass_temp_evil_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_grass_temp_evil_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_grass_temp_evil_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_grass_temp_evil_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_grass_temp_evil_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_grass_temp_evil_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_grass_temp_evil_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_grass_temp_evil_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_grass_temp_evil_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_grass_temp_evil_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_grass_temp_evil_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_grass_temp_evil_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_grass_temp_evil_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_grass_temp_evil_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_grass_temp_evil_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_grass_temp_evil_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_grass_temp_evil_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_grass_temp_evil_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_grass_temp_evil_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_grass_temp_evil_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 2:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_grass_temp_good_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_grass_temp_good_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_grass_temp_good_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_grass_temp_good_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_grass_temp_good_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_grass_temp_good_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_grass_temp_good_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_grass_temp_good_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_grass_temp_good_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_grass_temp_good_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_grass_temp_good_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_grass_temp_good_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_grass_temp_good_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_grass_temp_good_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_grass_temp_good_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_grass_temp_good_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_grass_temp_good_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_grass_temp_good_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_grass_temp_good_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_grass_temp_good_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_grass_temp_good_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_grass_temp_good_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_grass_temp_good_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_grass_temp_good_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_grass_temp_good_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_grass_temp_good_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_grass_temp_good_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_grass_temp_good_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_grass_temp_good_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_grass_temp_good_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_grass_temp_good_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_grass_temp_good_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_grass_temp_good_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_grass_temp_good_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_grass_temp_good_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_grass_temp_good_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_grass_temp_good_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 3:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_grass_trop_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_grass_trop_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_grass_trop_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_grass_trop_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_grass_trop_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_grass_trop_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_grass_trop_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_grass_trop_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_grass_trop_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_grass_trop_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_grass_trop_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_grass_trop_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_grass_trop_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_grass_trop_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_grass_trop_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_grass_trop_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_grass_trop_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_grass_trop_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_grass_trop_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_grass_trop_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_grass_trop_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_grass_trop_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_grass_trop_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_grass_trop_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_grass_trop_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_grass_trop_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_grass_trop_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_grass_trop_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_grass_trop_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_grass_trop_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_grass_trop_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_grass_trop_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_grass_trop_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_grass_trop_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_grass_trop_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_grass_trop_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_grass_trop_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 4:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_grass_trop_evil_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_grass_trop_evil_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_grass_trop_evil_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_grass_trop_evil_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_grass_trop_evil_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_grass_trop_evil_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_grass_trop_evil_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_grass_trop_evil_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_grass_trop_evil_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_grass_trop_evil_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_grass_trop_evil_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_grass_trop_evil_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_grass_trop_evil_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_grass_trop_evil_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_grass_trop_evil_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_grass_trop_evil_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_grass_trop_evil_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_grass_trop_evil_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_grass_trop_evil_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_grass_trop_evil_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_grass_trop_evil_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_grass_trop_evil_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_grass_trop_evil_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_grass_trop_evil_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_grass_trop_evil_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_grass_trop_evil_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 5:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_grass_trop_good_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_grass_trop_good_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_grass_trop_good_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_grass_trop_good_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_grass_trop_good_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_grass_trop_good_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_grass_trop_good_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_grass_trop_good_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_grass_trop_good_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_grass_trop_good_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_grass_trop_good_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_grass_trop_good_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_grass_trop_good_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_grass_trop_good_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_grass_trop_good_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_grass_trop_good_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_grass_trop_good_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_grass_trop_good_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_grass_trop_good_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_grass_trop_good_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_grass_trop_good_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_grass_trop_good_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_grass_trop_good_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_grass_trop_good_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_grass_trop_good_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_grass_trop_good_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_grass_trop_good_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_grass_trop_good_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_grass_trop_good_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_grass_trop_good_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_grass_trop_good_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_grass_trop_good_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_grass_trop_good_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_grass_trop_good_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_grass_trop_good_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_grass_trop_good_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_grass_trop_good_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 6:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_wetland_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_wetland_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_wetland_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_wetland_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_wetland_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_wetland_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_wetland_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_wetland_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_wetland_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_wetland_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_wetland_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_wetland_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_wetland_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_wetland_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_wetland_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_wetland_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_wetland_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_wetland_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_wetland_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_wetland_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_wetland_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_wetland_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_wetland_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_wetland_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_wetland_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_wetland_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_wetland_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_wetland_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_wetland_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_wetland_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_wetland_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_wetland_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_wetland_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_wetland_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_wetland_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_wetland_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_wetland_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 7:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_desert_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_desert_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_desert_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_desert_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_desert_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_desert_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_desert_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_desert_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_desert_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_desert_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_desert_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_desert_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_desert_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_desert_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_desert_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_desert_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_desert_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_desert_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_desert_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_desert_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_desert_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_desert_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_desert_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_desert_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_desert_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_desert_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_desert_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_desert_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_desert_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_desert_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_desert_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_desert_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_desert_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_desert_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_desert_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_desert_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_desert_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 8:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_desert_evil_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_desert_evil_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_desert_evil_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_desert_evil_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_desert_evil_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_desert_evil_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_desert_evil_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_desert_evil_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_desert_evil_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_desert_evil_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_desert_evil_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_desert_evil_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_desert_evil_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_desert_evil_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_desert_evil_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_desert_evil_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_desert_evil_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_desert_evil_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_desert_evil_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_desert_evil_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_desert_evil_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_desert_evil_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_desert_evil_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_desert_evil_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_desert_evil_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_desert_evil_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_desert_evil_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_desert_evil_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_desert_evil_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_desert_evil_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_desert_evil_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_desert_evil_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_desert_evil_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_desert_evil_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_desert_evil_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_desert_evil_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_desert_evil_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 9:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_desert_good_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_desert_good_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_desert_good_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_desert_good_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_desert_good_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_desert_good_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_desert_good_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_desert_good_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_desert_good_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_desert_good_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_desert_good_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_desert_good_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_desert_good_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_desert_good_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_desert_good_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_desert_good_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_desert_good_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_desert_good_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_desert_good_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_desert_good_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_desert_good_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_desert_good_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_desert_good_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_desert_good_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_desert_good_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_desert_good_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_desert_good_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_desert_good_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_desert_good_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_desert_good_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_desert_good_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_desert_good_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_desert_good_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_desert_good_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_desert_good_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_desert_good_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_desert_good_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 10:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_tundra_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_tundra_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_tundra_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_tundra_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_tundra_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_tundra_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_tundra_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_tundra_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_tundra_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_tundra_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_tundra_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_tundra_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_tundra_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_tundra_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_tundra_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_tundra_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_tundra_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_tundra_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_tundra_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_tundra_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_tundra_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_tundra_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_tundra_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_tundra_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_tundra_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_tundra_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_tundra_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_tundra_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_tundra_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_tundra_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_tundra_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_tundra_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_tundra_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_tundra_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_tundra_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_tundra_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_tundra_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 11:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_tundra_evil_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_tundra_evil_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_tundra_evil_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_tundra_evil_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_tundra_evil_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_tundra_evil_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_tundra_evil_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_tundra_evil_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_tundra_evil_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_tundra_evil_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_tundra_evil_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_tundra_evil_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_tundra_evil_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_tundra_evil_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_tundra_evil_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_tundra_evil_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_tundra_evil_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_tundra_evil_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_tundra_evil_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_tundra_evil_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_tundra_evil_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_tundra_evil_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_tundra_evil_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_tundra_evil_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_tundra_evil_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_tundra_evil_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_tundra_evil_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_tundra_evil_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_tundra_evil_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_tundra_evil_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_tundra_evil_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_tundra_evil_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_tundra_evil_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_tundra_evil_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_tundra_evil_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_tundra_evil_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_tundra_evil_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 12:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_tundra_good_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_tundra_good_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_tundra_good_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_tundra_good_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_tundra_good_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_tundra_good_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_tundra_good_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_tundra_good_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_tundra_good_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_tundra_good_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_tundra_good_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_tundra_good_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_tundra_good_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_tundra_good_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_tundra_good_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_tundra_good_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_tundra_good_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_tundra_good_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_tundra_good_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_tundra_good_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_tundra_good_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_tundra_good_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_tundra_good_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_tundra_good_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_tundra_good_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_tundra_good_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_tundra_good_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_tundra_good_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_tundra_good_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_tundra_good_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_tundra_good_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_tundra_good_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_tundra_good_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_tundra_good_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_tundra_good_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_tundra_good_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_tundra_good_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 13:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_glacier_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_glacier_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_glacier_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_glacier_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_glacier_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_glacier_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_glacier_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_glacier_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_glacier_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_glacier_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_glacier_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_glacier_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_glacier_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_glacier_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_glacier_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_glacier_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_glacier_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_glacier_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_glacier_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_glacier_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_glacier_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_glacier_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_glacier_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_glacier_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_glacier_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_glacier_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_glacier_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_glacier_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_glacier_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_glacier_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_glacier_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_glacier_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_glacier_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_glacier_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_glacier_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_glacier_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_glacier_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 14:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_glacier_evil_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_glacier_evil_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_glacier_evil_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_glacier_evil_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_glacier_evil_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_glacier_evil_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_glacier_evil_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_glacier_evil_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_glacier_evil_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_glacier_evil_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_glacier_evil_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_glacier_evil_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_glacier_evil_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_glacier_evil_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_glacier_evil_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_glacier_evil_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_glacier_evil_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_glacier_evil_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_glacier_evil_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_glacier_evil_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_glacier_evil_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_glacier_evil_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_glacier_evil_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_glacier_evil_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_glacier_evil_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_glacier_evil_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_glacier_evil_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_glacier_evil_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_glacier_evil_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_glacier_evil_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_glacier_evil_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_glacier_evil_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_glacier_evil_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_glacier_evil_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_glacier_evil_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_glacier_evil_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_glacier_evil_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 15:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_glacier_good_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_glacier_good_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_glacier_good_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_glacier_good_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_glacier_good_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_glacier_good_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_glacier_good_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_glacier_good_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_glacier_good_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_glacier_good_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_glacier_good_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_glacier_good_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_glacier_good_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_glacier_good_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_glacier_good_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_glacier_good_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_glacier_good_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_glacier_good_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_glacier_good_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_glacier_good_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_glacier_good_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_glacier_good_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_glacier_good_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_glacier_good_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_glacier_good_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_glacier_good_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_glacier_good_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_glacier_good_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_glacier_good_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_glacier_good_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_glacier_good_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_glacier_good_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_glacier_good_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_glacier_good_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_glacier_good_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_glacier_good_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_glacier_good_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 16:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_water_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_water_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_water_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_water_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_water_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_water_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_water_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_water_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_water_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_water_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_water_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_water_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_water_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_water_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_water_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_water_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_water_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_water_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_water_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_water_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_water_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_water_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_water_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_water_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_water_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_water_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_water_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_water_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_water_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_water_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_water_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_water_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_water_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_water_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_water_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_water_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_water_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 17:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_water_evil_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_water_evil_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_water_evil_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_water_evil_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_water_evil_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_water_evil_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_water_evil_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_water_evil_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_water_evil_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_water_evil_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_water_evil_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_water_evil_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_water_evil_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_water_evil_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_water_evil_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_water_evil_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_water_evil_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_water_evil_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_water_evil_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_water_evil_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_water_evil_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_water_evil_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_water_evil_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_water_evil_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_water_evil_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_water_evil_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_water_evil_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_water_evil_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_water_evil_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_water_evil_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_water_evil_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_water_evil_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_water_evil_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_water_evil_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_water_evil_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_water_evil_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_water_evil_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 18:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_water_good_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_water_good_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_water_good_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_water_good_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_water_good_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_water_good_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_water_good_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_water_good_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_water_good_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_water_good_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_water_good_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_water_good_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_water_good_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_water_good_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_water_good_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_water_good_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_water_good_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_water_good_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_water_good_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_water_good_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_water_good_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_water_good_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_water_good_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_water_good_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_water_good_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_water_good_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_water_good_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_water_good_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_water_good_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_water_good_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_water_good_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_water_good_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_water_good_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_water_good_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_water_good_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_water_good_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_water_good_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 19:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_sand_desert_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_sand_desert_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_sand_desert_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_sand_desert_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_sand_desert_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_sand_desert_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_sand_desert_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_sand_desert_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_sand_desert_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_sand_desert_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_sand_desert_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_sand_desert_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_sand_desert_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_sand_desert_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_sand_desert_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_sand_desert_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_sand_desert_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_sand_desert_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_sand_desert_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_sand_desert_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_sand_desert_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_sand_desert_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_sand_desert_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_sand_desert_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_sand_desert_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_sand_desert_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_sand_desert_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_sand_desert_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_sand_desert_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_sand_desert_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_sand_desert_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_sand_desert_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_sand_desert_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_sand_desert_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_sand_desert_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_sand_desert_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_sand_desert_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 20:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_sand_desert_yellow_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_sand_desert_yellow_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_sand_desert_yellow_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_sand_desert_yellow_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_sand_desert_yellow_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_sand_desert_yellow_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_sand_desert_yellow_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_sand_desert_yellow_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_sand_desert_yellow_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_sand_desert_yellow_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_sand_desert_yellow_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_sand_desert_yellow_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_sand_desert_yellow_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_sand_desert_yellow_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_sand_desert_yellow_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_sand_desert_yellow_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_sand_desert_yellow_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_sand_desert_yellow_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_sand_desert_yellow_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_sand_desert_yellow_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_sand_desert_yellow_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_sand_desert_yellow_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_sand_desert_yellow_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_sand_desert_yellow_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_sand_desert_yellow_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_sand_desert_yellow_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_sand_desert_yellow_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_sand_desert_yellow_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_sand_desert_yellow_thinning_e_s[sv];
            break;
          case 29:
            tpp =
                &texpos_edge_sand_desert_yellow_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp =
                &texpos_edge_sand_desert_yellow_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp =
                &texpos_edge_sand_desert_yellow_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp =
                &texpos_edge_sand_desert_yellow_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp =
                &texpos_edge_sand_desert_yellow_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp =
                &texpos_edge_sand_desert_yellow_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp =
                &texpos_edge_sand_desert_yellow_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp =
                &texpos_edge_sand_desert_yellow_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 21:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_sand_desert_white_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_sand_desert_white_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_sand_desert_white_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_sand_desert_white_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_sand_desert_white_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_sand_desert_white_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_sand_desert_white_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_sand_desert_white_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_sand_desert_white_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_sand_desert_white_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_sand_desert_white_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_sand_desert_white_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_sand_desert_white_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_sand_desert_white_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_sand_desert_white_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_sand_desert_white_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_sand_desert_white_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_sand_desert_white_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_sand_desert_white_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_sand_desert_white_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_sand_desert_white_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_sand_desert_white_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_sand_desert_white_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_sand_desert_white_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_sand_desert_white_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_sand_desert_white_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_sand_desert_white_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_sand_desert_white_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_sand_desert_white_thinning_e_s[sv];
            break;
          case 29:
            tpp =
                &texpos_edge_sand_desert_white_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp =
                &texpos_edge_sand_desert_white_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp =
                &texpos_edge_sand_desert_white_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp =
                &texpos_edge_sand_desert_white_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp =
                &texpos_edge_sand_desert_white_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp =
                &texpos_edge_sand_desert_white_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp =
                &texpos_edge_sand_desert_white_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp =
                &texpos_edge_sand_desert_white_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 22:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_sand_desert_black_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_sand_desert_black_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_sand_desert_black_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_sand_desert_black_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_sand_desert_black_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_sand_desert_black_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_sand_desert_black_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_sand_desert_black_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_sand_desert_black_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_sand_desert_black_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_sand_desert_black_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_sand_desert_black_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_sand_desert_black_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_sand_desert_black_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_sand_desert_black_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_sand_desert_black_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_sand_desert_black_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_sand_desert_black_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_sand_desert_black_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_sand_desert_black_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_sand_desert_black_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_sand_desert_black_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_sand_desert_black_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_sand_desert_black_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_sand_desert_black_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_sand_desert_black_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_sand_desert_black_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_sand_desert_black_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_sand_desert_black_thinning_e_s[sv];
            break;
          case 29:
            tpp =
                &texpos_edge_sand_desert_black_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp =
                &texpos_edge_sand_desert_black_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp =
                &texpos_edge_sand_desert_black_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp =
                &texpos_edge_sand_desert_black_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp =
                &texpos_edge_sand_desert_black_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp =
                &texpos_edge_sand_desert_black_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp =
                &texpos_edge_sand_desert_black_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp =
                &texpos_edge_sand_desert_black_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 23:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_sand_desert_red_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_sand_desert_red_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_sand_desert_red_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_sand_desert_red_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_sand_desert_red_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_sand_desert_red_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_sand_desert_red_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_sand_desert_red_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_sand_desert_red_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_sand_desert_red_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_sand_desert_red_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_sand_desert_red_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_sand_desert_red_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_sand_desert_red_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_sand_desert_red_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_sand_desert_red_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_sand_desert_red_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_sand_desert_red_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_sand_desert_red_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_sand_desert_red_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_sand_desert_red_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_sand_desert_red_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_sand_desert_red_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_sand_desert_red_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_sand_desert_red_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_sand_desert_red_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_sand_desert_red_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_sand_desert_red_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_sand_desert_red_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_sand_desert_red_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_sand_desert_red_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_sand_desert_red_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_sand_desert_red_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_sand_desert_red_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_sand_desert_red_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_sand_desert_red_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_sand_desert_red_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 24:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_sand_desert_evil_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_sand_desert_evil_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_sand_desert_evil_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_sand_desert_evil_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_sand_desert_evil_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_sand_desert_evil_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_sand_desert_evil_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_sand_desert_evil_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_sand_desert_evil_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_sand_desert_evil_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_sand_desert_evil_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_sand_desert_evil_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_sand_desert_evil_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_sand_desert_evil_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_sand_desert_evil_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_sand_desert_evil_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_sand_desert_evil_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_sand_desert_evil_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_sand_desert_evil_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_sand_desert_evil_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_sand_desert_evil_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_sand_desert_evil_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_sand_desert_evil_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_sand_desert_evil_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_sand_desert_evil_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_sand_desert_evil_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 25:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_sand_desert_good_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_sand_desert_good_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_sand_desert_good_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_sand_desert_good_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_sand_desert_good_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_sand_desert_good_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_sand_desert_good_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_sand_desert_good_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_sand_desert_good_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_sand_desert_good_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_sand_desert_good_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_sand_desert_good_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_sand_desert_good_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_sand_desert_good_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_sand_desert_good_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_sand_desert_good_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_sand_desert_good_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_sand_desert_good_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_sand_desert_good_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_sand_desert_good_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_sand_desert_good_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_sand_desert_good_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_sand_desert_good_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_sand_desert_good_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_sand_desert_good_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_sand_desert_good_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_sand_desert_good_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_sand_desert_good_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_sand_desert_good_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_sand_desert_good_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_sand_desert_good_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_sand_desert_good_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_sand_desert_good_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_sand_desert_good_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_sand_desert_good_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_sand_desert_good_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_sand_desert_good_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 26:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_wetland_evil_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_wetland_evil_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_wetland_evil_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_wetland_evil_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_wetland_evil_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_wetland_evil_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_wetland_evil_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_wetland_evil_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_wetland_evil_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_wetland_evil_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_wetland_evil_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_wetland_evil_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_wetland_evil_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_wetland_evil_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_wetland_evil_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_wetland_evil_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_wetland_evil_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_wetland_evil_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_wetland_evil_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_wetland_evil_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_wetland_evil_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_wetland_evil_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_wetland_evil_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_wetland_evil_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_wetland_evil_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_wetland_evil_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_wetland_evil_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_wetland_evil_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_wetland_evil_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_wetland_evil_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_wetland_evil_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_wetland_evil_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_wetland_evil_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_wetland_evil_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_wetland_evil_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_wetland_evil_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_wetland_evil_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 27:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_wetland_good_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_wetland_good_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_wetland_good_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_wetland_good_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_wetland_good_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_wetland_good_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_wetland_good_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_wetland_good_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_wetland_good_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_wetland_good_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_wetland_good_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_wetland_good_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_wetland_good_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_wetland_good_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_wetland_good_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_wetland_good_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_wetland_good_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_wetland_good_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_wetland_good_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_wetland_good_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_wetland_good_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_wetland_good_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_wetland_good_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_wetland_good_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_wetland_good_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_wetland_good_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_wetland_good_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_wetland_good_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_wetland_good_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_wetland_good_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_wetland_good_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_wetland_good_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_wetland_good_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_wetland_good_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_wetland_good_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_wetland_good_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_wetland_good_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 28:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_beach_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_beach_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_beach_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_beach_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_beach_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_beach_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_beach_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_beach_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_beach_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_beach_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_beach_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_beach_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_beach_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_beach_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_beach_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_beach_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_beach_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_beach_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_beach_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_beach_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_beach_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_beach_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_beach_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_beach_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_beach_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_beach_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_beach_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_beach_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_beach_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_beach_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_beach_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_beach_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_beach_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_beach_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_beach_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_beach_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_beach_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 29:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_beach_evil_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_beach_evil_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_beach_evil_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_beach_evil_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_beach_evil_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_beach_evil_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_beach_evil_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_beach_evil_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_beach_evil_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_beach_evil_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_beach_evil_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_beach_evil_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_beach_evil_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_beach_evil_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_beach_evil_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_beach_evil_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_beach_evil_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_beach_evil_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_beach_evil_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_beach_evil_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_beach_evil_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_beach_evil_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_beach_evil_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_beach_evil_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_beach_evil_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_beach_evil_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_beach_evil_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_beach_evil_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_beach_evil_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_beach_evil_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_beach_evil_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_beach_evil_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_beach_evil_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_beach_evil_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_beach_evil_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_beach_evil_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_beach_evil_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 30:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_beach_good_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_beach_good_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_beach_good_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_beach_good_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_beach_good_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_beach_good_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_beach_good_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_beach_good_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_beach_good_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_beach_good_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_beach_good_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_beach_good_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_beach_good_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_beach_good_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_beach_good_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_beach_good_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_beach_good_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_beach_good_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_beach_good_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_beach_good_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_beach_good_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_beach_good_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_beach_good_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_beach_good_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_beach_good_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_beach_good_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_beach_good_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_beach_good_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_beach_good_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_beach_good_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_beach_good_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_beach_good_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_beach_good_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_beach_good_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_beach_good_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_beach_good_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_beach_good_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 31:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_glacier_evilsav_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_glacier_evilsav_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_glacier_evilsav_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_glacier_evilsav_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_glacier_evilsav_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_glacier_evilsav_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_glacier_evilsav_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_glacier_evilsav_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_glacier_evilsav_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_glacier_evilsav_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_glacier_evilsav_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_glacier_evilsav_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_glacier_evilsav_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_glacier_evilsav_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_glacier_evilsav_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_glacier_evilsav_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_glacier_evilsav_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_glacier_evilsav_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_glacier_evilsav_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_glacier_evilsav_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_glacier_evilsav_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_glacier_evilsav_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_glacier_evilsav_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_glacier_evilsav_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_glacier_evilsav_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_glacier_evilsav_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        case 32:
          switch (siv) {
          case 0:
            tpp = &texpos_edge_glacier_goodsav_straight_n[sv];
            break;
          case 1:
            tpp = &texpos_edge_glacier_goodsav_straight_s[sv];
            break;
          case 2:
            tpp = &texpos_edge_glacier_goodsav_straight_w[sv];
            break;
          case 3:
            tpp = &texpos_edge_glacier_goodsav_straight_e[sv];
            break;
          case 4:
            tpp = &texpos_edge_glacier_goodsav_thick_corner_nw[sv];
            break;
          case 5:
            tpp = &texpos_edge_glacier_goodsav_thick_corner_ne[sv];
            break;
          case 6:
            tpp = &texpos_edge_glacier_goodsav_thick_corner_sw[sv];
            break;
          case 7:
            tpp = &texpos_edge_glacier_goodsav_thick_corner_se[sv];
            break;
          case 8:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_nw[sv];
            break;
          case 9:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_ne[sv];
            break;
          case 10:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_sw[sv];
            break;
          case 11:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_se[sv];
            break;
          case 12:
            tpp = &texpos_edge_glacier_goodsav_tiny_corner_nw[sv];
            break;
          case 13:
            tpp = &texpos_edge_glacier_goodsav_tiny_corner_ne[sv];
            break;
          case 14:
            tpp = &texpos_edge_glacier_goodsav_tiny_corner_sw[sv];
            break;
          case 15:
            tpp = &texpos_edge_glacier_goodsav_tiny_corner_se[sv];
            break;
          case 16:
            tpp = &texpos_edge_glacier_goodsav_inlet_nsw[sv];
            break;
          case 17:
            tpp = &texpos_edge_glacier_goodsav_inlet_nse[sv];
            break;
          case 18:
            tpp = &texpos_edge_glacier_goodsav_inlet_nwe[sv];
            break;
          case 19:
            tpp = &texpos_edge_glacier_goodsav_inlet_swe[sv];
            break;
          case 20:
            tpp = &texpos_edge_glacier_goodsav_surround[sv];
            break;
          case 21:
            tpp = &texpos_edge_glacier_goodsav_thinning_n_w[sv];
            break;
          case 22:
            tpp = &texpos_edge_glacier_goodsav_thinning_n_e[sv];
            break;
          case 23:
            tpp = &texpos_edge_glacier_goodsav_thinning_s_w[sv];
            break;
          case 24:
            tpp = &texpos_edge_glacier_goodsav_thinning_s_e[sv];
            break;
          case 25:
            tpp = &texpos_edge_glacier_goodsav_thinning_w_n[sv];
            break;
          case 26:
            tpp = &texpos_edge_glacier_goodsav_thinning_w_s[sv];
            break;
          case 27:
            tpp = &texpos_edge_glacier_goodsav_thinning_e_n[sv];
            break;
          case 28:
            tpp = &texpos_edge_glacier_goodsav_thinning_e_s[sv];
            break;
          case 29:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_transition_nw_n[sv];
            break;
          case 30:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_transition_nw_w[sv];
            break;
          case 31:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_transition_ne_n[sv];
            break;
          case 32:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_transition_ne_e[sv];
            break;
          case 33:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_transition_sw_s[sv];
            break;
          case 34:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_transition_sw_w[sv];
            break;
          case 35:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_transition_se_s[sv];
            break;
          case 36:
            tpp = &texpos_edge_glacier_goodsav_thin_corner_transition_se_e[sv];
            break;
          }
          break;
        }
        ...
        for (f = 0; f < BROOK_FLOW_ANIMATION_NUM; ++f) {
          ...
          *dst = enabler.textures.create_texture(tex->w, tex->h);
          ...
          for (si = 1; si < 4; ++si) {
            ...
            // NOTE: a blit did not work here, it overwrote instead of doing
            // alpha blending
            SDL_LockSurface(tex);
            SDL_LockSurface(dsts);
...

  {
    ...
    for (dig_type = 0; dig_type < 7; ++dig_type) {
      ...
      switch (dig_type) {
      case 0:
        src_tp = texpos_designation_dig_stair_updown[0][0];
        break;
      case 1:
        src_tp = texpos_designation_dig_stair_up[0][0];
        break;
      case 2:
        src_tp = texpos_designation_dig_stair_down[0][0];
        break;
      case 3:
        src_tp = texpos_designation_dig_ramp[0][0];
        break;
      case 4:
        src_tp = texpos_designation_dig_channel[0][0];
        break;
      case 5:
        src_tp = texpos_designation_dig_remove_construction[0][0];
        break;
      case 6:
        src_tp = texpos_designation_dig_standard[0][0];
        break;
      }
      if (src_tp == 0)
        continue;
      ...
      for (p = 0; p < 8; ++p) {
        ...
        int32_t psrc = 0;
        if (p >= 1)
          psrc = texpos_designation_priority[p - 1];
        if (psrc != 0) {
        ...
        for (m = 0; m < 3; ++m) {
          ...
          switch (dig_type) {
          case 0:
            dst = &texpos_designation_dig_stair_updown[p][m];
            break;
          case 1:
            dst = &texpos_designation_dig_stair_up[p][m];
            break;
          case 2:
            dst = &texpos_designation_dig_stair_down[p][m];
            break;
          case 3:
            dst = &texpos_designation_dig_ramp[p][m];
            break;
          case 4:
            dst = &texpos_designation_dig_channel[p][m];
            break;
          case 5:
            dst = &texpos_designation_dig_remove_construction[p][m];
            break;
          case 6:
            dst = &texpos_designation_dig_standard[p][m];
            break;
          }
          *dst = enabler.textures.create_texture(tex->w, tex->h);
          SDL_Surface* dsts = enabler.textures.get_texture_data(*dst);
          ...
          for (y = 0; y < tex->h; y++) {
            ...
            pixel_dst = ((Uint8*)dsts->pixels) + (y * dsts->pitch);
            if (ptex != NULL)
              pixel_psrc = ((Uint8*)ptex->pixels) + (y * ptex->pitch);
            else
              pixel_psrc = 0; // doesn't matter
            for (x = 0; x < tex->w;
                 ++x, pixel_src += 4, pixel_dst += 4, pixel_psrc += 4) {
              pixel_dst[3] = pixel_src[3];
  ...
  }
  {
    ...
    for (des_type = 0; des_type < 20; ++des_type) {
      ...
      switch (des_type) {
      case 0:
        src_tp = texpos_designation_chop[0][0];
        break;
      case 1:
        src_tp = texpos_designation_gather[0][0];
        break;
      case 2:
        src_tp = texpos_designation_smooth[0][0];
        break;
      case 3:
        src_tp = texpos_designation_engrave[0][0];
        break;
      case 4:
        src_tp = texpos_designation_fortify[0][0];
        break;
      case 5:
        src_tp = texpos_designation_track_n[0][0];
        break;
      case 6:
        src_tp = texpos_designation_track_s[0][0];
        break;
      case 7:
        src_tp = texpos_designation_track_w[0][0];
        break;
      case 8:
        src_tp = texpos_designation_track_e[0][0];
        break;
      case 9:
        src_tp = texpos_designation_track_ns[0][0];
        break;
      case 10:
        src_tp = texpos_designation_track_nw[0][0];
        break;
      case 11:
        src_tp = texpos_designation_track_ne[0][0];
        break;
      case 12:
        src_tp = texpos_designation_track_sw[0][0];
        break;
      case 13:
        src_tp = texpos_designation_track_se[0][0];
        break;
      case 14:
        src_tp = texpos_designation_track_we[0][0];
        break;
      case 15:
        src_tp = texpos_designation_track_nsw[0][0];
        break;
      case 16:
        src_tp = texpos_designation_track_nse[0][0];
        break;
      case 17:
        src_tp = texpos_designation_track_nwe[0][0];
        break;
      case 18:
        src_tp = texpos_designation_track_swe[0][0];
        break;
      case 19:
        src_tp = texpos_designation_track_nswe[0][0];
        break;
      }
      if (src_tp == 0)
        continue;
      ...
      for (p = 0; p < 8; ++p) {
        ...
        for (m = 0; m < 2; ++m) {
          ...
          switch (des_type) {
          case 0:
            dst = &texpos_designation_chop[p][m];
            break;
          case 1:
            dst = &texpos_designation_gather[p][m];
            break;
          case 2:
            dst = &texpos_designation_smooth[p][m];
            break;
          case 3:
            dst = &texpos_designation_engrave[p][m];
            break;
          case 4:
            dst = &texpos_designation_fortify[p][m];
            break;
          case 5:
            dst = &texpos_designation_track_n[p][m];
            break;
          case 6:
            dst = &texpos_designation_track_s[p][m];
            break;
          case 7:
            dst = &texpos_designation_track_w[p][m];
            break;
          case 8:
            dst = &texpos_designation_track_e[p][m];
            break;
          case 9:
            dst = &texpos_designation_track_ns[p][m];
            break;
          case 10:
            dst = &texpos_designation_track_nw[p][m];
            break;
          case 11:
            dst = &texpos_designation_track_ne[p][m];
            break;
          case 12:
            dst = &texpos_designation_track_sw[p][m];
            break;
          case 13:
            dst = &texpos_designation_track_se[p][m];
            break;
          case 14:
            dst = &texpos_designation_track_we[p][m];
            break;
          case 15:
            dst = &texpos_designation_track_nsw[p][m];
            break;
          case 16:
            dst = &texpos_designation_track_nse[p][m];
            break;
          case 17:
            dst = &texpos_designation_track_nwe[p][m];
            break;
          case 18:
            dst = &texpos_designation_track_swe[p][m];
            break;
          case 19:
            dst = &texpos_designation_track_nswe[p][m];
            break;
          }
          *dst = enabler.textures.create_texture(tex->w, tex->h);
          SDL_Surface* dsts = enabler.textures.get_texture_data(*dst);
  ...
  if(texpos_item_barrel_wood!=0)
    ...
    if(tex!=NULL)
      ...
      SDL_Surface
  *color=enabler.textures.get_texture_data(texpos_item_barrel_wood_lockable);
  ...
  if(texpos_item_barrel_metal!=0)
    {
    SDL_Surface
  *tex=enabler.textures.get_texture_data(texpos_item_barrel_metal);
    if(tex!=NULL)
      {
      texpos_item_barrel_metal_lockable=enabler.textures.create_texture(tex->w,tex->h);
      SDL_Surface
  *color=enabler.textures.get_texture_data(texpos_item_barrel_metal_lockable);
...
void graphicst::set_force_full_display_count(short new_count) {
  if (force_full_display_count < new_count)
    force_full_display_count = new_count;
}
...
void graphicst::zero_and_clear_all_texpos() {
  ...
  texpos_building_icon_wagon = 0;
  {
    int32_t c;
    for (c = 0; c < 37; ++c)
      texpos_planned_construction[c] = 0;
  }
  ...
  memset(texpos_button_category_rectangle_on_selected, 0,
         sizeof(int32_t) * 3 * 3);
  memset(texpos_button_category_rectangle_off, 0, sizeof(int32_t) * 3 * 3);
  memset(texpos_button_category_rectangle_off_selected, 0,
         sizeof(int32_t) * 3 * 3);
  ...
  memset(texpos_button_main, 0,
         sizeof(int32_t) * INTERFACE_BUTTON_MAINNUM * 4 * 3);
  memset(texpos_button_small, 0,
         sizeof(int32_t) * INTERFACE_BUTTON_SMALLNUM * 2 * 2);
  ...
  memset(texpos_button_horizontal_option_left_ornament, 0,
         sizeof(int32_t) * 4 * 3);
  ...
  memset(texpos_button_horizontal_option_right_ornament, 0,
         sizeof(int32_t) * 4 * 3);
  ...
  memset(texpos_button_assign_trade, 0,
         sizeof(int32_t) * 3 * 3 * INTERFACE_BUTTON_ASSIGN_TRADENUM);
  memset(texpos_button_building_info, 0,
         sizeof(int32_t) * 3 * 3 * INTERFACE_BUTTON_BUILDING_INFONUM);
  memset(texpos_button_building_sheet, 0,
         sizeof(int32_t) * 4 * 3 * INTERFACE_BUTTON_BUILDING_SHEETNUM);
  memset(texpos_button_unit_sheet, 0,
         sizeof(int32_t) * 3 * 3 * INTERFACE_BUTTON_UNIT_SHEETNUM);
  memset(texpos_button_large_unit_sheet, 0,
         sizeof(int32_t) * 4 * 3 * INTERFACE_BUTTON_LARGE_UNIT_SHEETNUM);
  memset(texpos_button_pets_livestock, 0,
         sizeof(int32_t) * 3 * 3 * INTERFACE_BUTTON_PETS_LIVESTOCKNUM);
...

  {
    ...
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_carpenter_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_farmer_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_mason_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_craftsman_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_jeweler_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_metalsmith_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_lavamill_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_bowyer_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_mechanic_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 5; ++b) {
        for (c = 0; c < 6; ++c)
          workshop_siege_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_butcher_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_leather_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_tanner_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_clothes_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_fishery_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_still_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_loom_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 5; ++b) {
        for (c = 0; c < 6; ++c)
          workshop_kennel_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_kitchen_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_ashery_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_dyer_texpos[a][b][c] = 0;
      }
    }
    ...
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_carpenter_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_farmer_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_mason_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_craftsman_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_jeweler_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_metalsmith_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_lavamill_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_bowyer_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_mechanic_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_butcher_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_leather_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_tanner_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_clothes_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_fishery_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_still_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_loom_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_kitchen_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_ashery_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 3; ++b) {
        for (c = 0; c < 4; ++c)
          workshop_dyer_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 5; ++b) {
        for (c = 0; c < 6; ++c)
          workshop_siege_texpos_overlay[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 5; ++b) {
        for (c = 0; c < 6; ++c)
          workshop_kennel_texpos_overlay[a][b][c] = 0;
      }
    }
    ...
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 5; ++b) {
        for (c = 0; c < 6; ++c)
          trade_depot_texpos[a][b][c] = 0;
      }
    }
    for (a = 0; a < 4; ++a) {
      for (b = 0; b < 5; ++b) {
        for (c = 0; c < 6; ++c)
          trade_depot_texpos_overlay[a][b][c] = 0;
      }
    }
    int32_t find;
    for (find = 0; find < 7; ++find) {
      for (a = 0; a < 4; ++a) {
        for (b = 0; b < 3; ++b) {
          for (c = 0; c < 4; ++c)
            furnace_texpos[find][a][b][c] = 0;
          furnace_texpos_overlay[find][a][b][c] = 0;
        }
      }
    }
    for (a = 0; a < 2; ++a) {
      for (b = 0; b < 2; ++b)
        workshop_quern_texpos[a][b] = 0;
    }
    for (a = 0; a < 2; ++a) {
      for (b = 0; b < 2; ++b)
        workshop_millstone_texpos[a][b] = 0;
    }
    for (a = 0; a < 2; ++a)
      workshop_millstone_turning_texpos[a] = 0;
  ...
  for (m = 0; m < 32; ++m) {
    int32_t mi;
    for (mi = 0; mi < 9; ++mi)
      custom_grass_edge_texpos[m][mi] = 0;
  }
  for (m = 0; m < 8; ++m) {
  ...
  texpos_shrub = 0;
  texpos_sapling = 0;
  for (m = 0; m < TREE_WOOD_TILENUM; ++m)
    texpos_tree_wood_tile[m] = 0;
  ...
  memset(texpos_tree_overleaves_heavy_branch_nse_autumn, 0,
         sizeof(int32_t) * 3);
  memset(texpos_tree_overleaves_heavy_branch_nsw_autumn, 0,
         sizeof(int32_t) * 3);
  memset(texpos_tree_overleaves_heavy_branch_new_autumn, 0,
         sizeof(int32_t) * 3);
  memset(texpos_tree_overleaves_heavy_branch_sew_autumn, 0,
         sizeof(int32_t) * 3);
  memset(texpos_tree_overleaves_heavy_branch_nsew_autumn, 0,
         sizeof(int32_t) * 3);
  ...
  memset(texpos_tree_overleaves_heavy_branch_nsew_fruit, 0,
         sizeof(int32_t) * 3);
  ...
  memset(texpos_tree_overleaves_heavy_branch_nse_flower, 0,
         sizeof(int32_t) * 3);
  memset(texpos_tree_overleaves_heavy_branch_nsw_flower, 0,
         sizeof(int32_t) * 3);
  memset(texpos_tree_overleaves_heavy_branch_new_flower, 0,
         sizeof(int32_t) * 3);
  memset(texpos_tree_overleaves_heavy_branch_sew_flower, 0,
         sizeof(int32_t) * 3);
  memset(texpos_tree_overleaves_heavy_branch_nsew_flower, 0,
         sizeof(int32_t) * 3);
  ...
  for (m = 0; m < 4; ++m)
    stone_wall_n_s_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    stone_wall_w_e_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    stone_wall_n_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    stone_wall_s_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    stone_wall_w_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    stone_wall_e_texpos[m] = 0;
  ...
  for (m = 0; m < 4; ++m)
    soil_wall_n_s_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    soil_wall_w_e_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    soil_wall_n_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    soil_wall_s_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    soil_wall_w_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    soil_wall_e_texpos[m] = 0;
  ...
  for (m = 0; m < 4; ++m)
    worn1_stone_wall_nswe_texpos[m] = 0;
  ...
  for (m = 0; m < 4; ++m)
    worn1_stone_wall_n_s_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn1_stone_wall_w_e_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn1_stone_wall_n_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn1_stone_wall_s_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn1_stone_wall_w_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn1_stone_wall_e_texpos[m] = 0;
  ...
  for (m = 0; m < 4; ++m)
    worn2_stone_wall_nswe_texpos[m] = 0;
  ...
  for (m = 0; m < 4; ++m)
    worn2_stone_wall_n_s_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn2_stone_wall_w_e_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn2_stone_wall_n_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn2_stone_wall_s_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn2_stone_wall_w_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn2_stone_wall_e_texpos[m] = 0;
  ...
  for (m = 0; m < 4; ++m)
    worn3_stone_wall_nswe_texpos[m] = 0;
  ...
  for (m = 0; m < 4; ++m)
    worn3_stone_wall_n_s_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn3_stone_wall_w_e_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn3_stone_wall_n_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn3_stone_wall_s_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn3_stone_wall_w_texpos[m] = 0;
  for (m = 0; m < 4; ++m)
    worn3_stone_wall_e_texpos[m] = 0;
  ...
  for (m = 0; m < 5; ++m)
    texpos_hidden_rock[m] = 0;

  {
    int32_t p;
    for (p = 0; p < 7; ++p)
      texpos_designation_priority[p] = 0;
    for (p = 0; p < 8; ++p) {
      for (m = 0; m < 3; ++m) {
  ...
  memset(texpos_classic_moon_weather, 0,
         sizeof(int32_t) * INTERFACE_MOON_WEATHERNUM * 4 * 3);
  memset(texpos_classic_announcement_alert, 0,
         sizeof(int32_t) * ANNOUNCEMENT_ALERTNUM * 4 * 3);
  ...
  for (smt = 0; smt < SPATTER_MATERIALNUM; ++smt) {
    ...
    */
    ...
    for (x = 0; x < 4; ++x) {
      for (y = 0; y < 4; ++y)
        texpos_partial_spatter[smt][x][y] = 0;
    }
  }
  ...
  for (s = 0; s < 4; ++s)
    texpos_item_hatch_cover_variant[s] = 0;
  ...
  for (s = 0; s < 3; ++s)
    texpos_item_hatch_cover_damage[s] = 0;
  ...
  for (s = 0; s < 4; ++s)
    texpos_item_floodgate_variant[s] = 0;
  ...
  for (s = 0; s < 3; ++s)
    texpos_item_floodgate_damage[s] = 0;
  ...
  for (s = 0; s < 4; ++s)
    texpos_food_container_top_fish[s] = 0;
  for (s = 0; s < 4; ++s)
    texpos_food_container_top_meat[s] = 0;
  ...
  texpos_item_armor = 0;
  for (s = 0; s < 4; ++s)
    texpos_item_web_harmless[s] = 0;
  for (s = 0; s < 4; ++s)
    texpos_item_web_thick[s] = 0;
  ...
  }
  {
    ...
    for (y = 0; y < 4; ++y) {
      for (z = 0; z < 2; ++z)
        texpos_water_wheel_ns[y][z] = 0;
    }
    for (x = 0; x < 3; ++x) {
      for (y = 0; y < 2; ++y) {
        for (z = 0; z < 2; ++z)
          texpos_water_wheel_we[x][y][z] = 0;
      }
    }
  }
  {
