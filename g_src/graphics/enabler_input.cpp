...
static set<Event>
    timeline; // A timeline of pending key events (for next get_input)
static set<EventMatch> pressed_keys; // Keys we consider "pressed"
static int modState;                 // Modifier state

// These do not change as part of the normal dynamics of DF, only at
// startup/when editing.
static multimap<EventMatch, InterfaceKey> keymap;
static map<InterfaceKey, Repeat> repeatmap;
static map<InterfaceKey, set<string, less_sz>>
    keydisplay; // Used only for display, not for meaning
...
static Time next_serial() { return ++last_serial; }
...
static void update_keydisplay(InterfaceKey binding, string display) {
  ...
  if (display == "\t")
    display = "Tab";
  map<InterfaceKey, set<string, less_sz>>::iterator it =
      keydisplay.find(binding);
  if (it == keydisplay.end()) {
    set<string, less_sz> s;
    s.insert(display);
...
static string display(const EventMatch& match) {
  static const std::map<SDL_Keycode, string> capitals = {
      {SDLK_a, "A"}, {SDLK_b, "B"}, {SDLK_c, "C"}, {SDLK_d, "D"}, {SDLK_e, "E"},
      {SDLK_f, "F"}, {SDLK_g, "G"}, {SDLK_h, "H"}, {SDLK_i, "I"}, {SDLK_h, "J"},
      {SDLK_j, "K"}, {SDLK_k, "L"}, {SDLK_l, "M"}, {SDLK_m, "N"}, {SDLK_n, "O"},
      {SDLK_o, "P"}, {SDLK_p, "Q"}, {SDLK_q, "R"}, {SDLK_r, "S"}, {SDLK_t, "T"},
      {SDLK_u, "U"}, {SDLK_v, "V"}, {SDLK_w, "W"}, {SDLK_x, "X"}, {SDLK_y, "Y"},
      {SDLK_z, "Z"},
  };
  ostringstream ret;
  bool is_letter = false;
  if (match.type == type_key && (match.mod & 1) &&
      capitals.contains(match.key)) {
    is_letter = true;
    if (match.mod == 1) {
...
Uint8 getModState() { return modState; }

// Not sure what to call this, but it ain't using regexes.
static bool parse_line(const string& line, const string& regex,
                       vector<string>& parts) {
  parts.clear();
  parts.push_back(line);
  for (int l = 0, r = 0; r < regex.length();) {
    switch (regex[r]) {
    case '*': // Read until ], : or the end of the line, but at least one
              // character.
    {
      const int start = l;
      for (; l < line.length() &&
             (l == start || (line[l] != ']' && line[l] != ':'));
           l++)
        ;
...
void enabler_inputst::clear_keybindings() { keymap.clear(); }
...
bool enabler_inputst::load_keybindings(const string& file) {
  ...
  while (line != lines.end()) {
    ...
    if (parse_line(*line, bind, match)) {
      map<string, InterfaceKey>::iterator it =
          bindingNames.right.find(match[1]);
      if (it != bindingNames.right.end()) {
        InterfaceKey binding = it->second;

        // bail if these key is already accounted for - prefs is loaded first,
        // default second
        ...
        while (line != lines.end()) {
          ...
          if (parse_line(*line, sym, match)) {
            map<string, SDL_Keycode>::iterator it =
                sdlNames.right.find(match[2]);
            if (it != sdlNames.right.end()) {
              matcher.mod = atoi(string(match[1]).c_str());
              matcher.type = type_key;
              matcher.key = it->second;
              keymap.insert(pair<EventMatch, InterfaceKey>(
                  matcher, (InterfaceKey)binding));
            ...
            if (matcher.button) {
              ...
              cout << "Broken button (should be [BUTTON:#:#]): " << *line
                   << endl;
            }
            ++line;
          }
          // Mousewheel
            ...
            if (str == "UP" || str == "DOWN") {
              ...
              cout << "Broken mousewheel (should be [MOUSEWHEEL:#:UP|DOWN]: "
                   << *line << endl;
            }
            ++line;
          } else {
...
void enabler_inputst::save_keybindings(const string& file) {
  ...
  // Invert keyboard map
  for (multimap<EventMatch, InterfaceKey>::iterator it = keymap.begin();
       it != keymap.end(); ++it)
  ...
  s << "[VERSION:1]" << endl;
  // And write.
  for (multimap<InterfaceKey, EventMatch>::iterator it = map.begin();
       it != map.end(); ++it) {
    if (!s.good()) {
    ...
    case type_key:
      s << "[SYM:" << (int)it->second.mod << ":"
        << sdlNames.left[it->second.key] << "]" << endl;
      break;
    case type_button:
      s << "[BUTTON:" << (int)it->second.mod << ":" << (int)it->second.button
        << "]" << endl;
      break;
    case type_mwheel:
      s << "[MOUSEWHEEL:" << (int)it->second.mod
        << (it->second.y > 0 ? ":UP]" : ":DOWN]") << endl;
      break;
    }
  }
  s.close();
...
void enabler_inputst::add_input(SDL_Event& e, Uint32 now) {
  ...
  if ((e.type == SDL_KEYUP || e.type == SDL_KEYDOWN) &&
      (e.key.keysym.sym == SDLK_RSHIFT || e.key.keysym.sym == SDLK_LSHIFT ||
       e.key.keysym.sym == SDLK_RCTRL || e.key.keysym.sym == SDLK_LCTRL ||
       e.key.keysym.sym == SDLK_RALT || e.key.keysym.sym == SDLK_LALT)) {
    for (pkit = pressed_keys.begin(); pkit != pressed_keys.end(); ++pkit) {
      ...
      // Re-press them, with new modifiers, if they aren't unicode. We can't
    ...
    KeyEvent real;
    real.release =
        (e.type == SDL_KEYUP || e.type == SDL_MOUSEBUTTONUP) ? true : false;
    ...
    case SDL_MOUSEWHEEL:
      ...
      real.match.y =
          e.wheel.y * ((e.wheel.direction == SDL_MOUSEWHEEL_NORMAL) ? 1 : -1);
  ...
  for (lit = synthetics.begin(); lit != synthetics.end(); ++lit) {
    ...
    if (lit->first.release)
      pressed_keys.erase(lit->first.match);
    else
      pressed_keys.insert(lit->first.match);
...
void enabler_inputst::add_input_ncurses(int key, Time now, bool esc) {
  ...
  EventMatch sdl; // Each key may provoke an unicode event, an "SDL-key" event,
                  // or both
  const int serial = next_serial();
  sdl.type = type_key;
  ...
  if (key == -10) {       // Return
    ...
    sdl.key = SDLK_BACKSPACE;
             ...
             key >= -26) { // Control-a through z (but not ctrl-j, or ctrl-i)
    sdl.mod |= DFMOD_CTRL;
    sdl.key = (SDL_KeyCode)(SDLK_a + (-key) - 1);
  } else if (key <= -32 && key >= -126) { // ASCII character set
    sdl.key =
        (SDL_KeyCode)-key; // Most of this maps directly to SDL keys, except..
  ...
  if (sdl.key) {
    ...
    for (set<InterfaceKey>::iterator k = events.begin(); k != events.end();
         ++k) {
      e.serial = serial;
      e.k = *k;
      timeline.insert(e);
...
void enabler_inputst::add_input_refined(KeyEvent& e, Uint32 now, int serial) {
  ...
  if (e.release) {
    ...
    for (set<InterfaceKey>::iterator k = keys.begin(); k != keys.end(); ++k) {
      ...
      Event ev = {is_mousewheel ? REPEAT_MWHEEL : key_repeat(*k),
                  *k,
                  is_mousewheel ? abs(e.match.y) : 0,
                  serial,
                  (int)now,
                  enabler.simticks.load()};
      timeline.insert(ev);
    }
    // if (cancel_ok) {
...
set<InterfaceKey> enabler_inputst::get_input(Time now) {
  ...
  while (ev != timeline.end() && ev->time == first_time &&
         ev->serial == first_serial) {
    // Avoid recording macro-sources events as macro events.
    if (ev->macro)
      event_from_macro = true;
    // To make sure the user had a chance to cancel (by lifting the key), we
    // require there to be at least three simulation ticks before the first
    // repeat.
    ...
    case REPEAT_FAST: {
      ...
      if (ev->repeats >= init.input.repeat_accel_start) {
        ...
        accel = MIN(
            init.input.repeat_accel_limit,
            sqrt(double(next.repeats - init.input.repeat_accel_start) + 16) -
                3);
      }
  ...
  if (input.size() && !init.display.flag.has_flag(INIT_DISPLAY_FLAG_TEXT)) {
    ...
    for (it = input.begin(); it != input.end(); ++it)
      cout << "    " << GetKeyDisplay(*it) << ": " << GetBindingDisplay(*it)
           << endl;
  }
#endif
...
set<InterfaceKey> enabler_inputst::key_translation(EventMatch& match) {
  ...
  pair<multimap<EventMatch, InterfaceKey>::iterator,
       multimap<EventMatch, InterfaceKey>::iterator>
      its;
...
void enabler_inputst::record_stop() { macro_recording = false; }

bool enabler_inputst::is_recording() { return macro_recording; }

void enabler_inputst::play_macro() {
  Time now = SDL_GetTicks();
  for_each(timeline.begin(), timeline.end(),
           [&](Event e) { now = MAX(now, e.time); });
  for (macro::iterator sim = active_macro.begin(); sim != active_macro.end();
       ++sim) {
    Event e;
    e.r = REPEAT_NOT;
    e.repeats = 0;
...
bool enabler_inputst::is_macro_playing() { return SDL_GetTicks() <= macro_end; }
  ...
  for (;;) {
    ...
    if (line == "End of macro") {
      ...
      if (line.substr(1).find("\t") != string::npos) {
        ...
        if (ss >> count) {
          ...
          if (remainingline.size()) {
            for (int i = 0; i < count; i++) {
              map<string, InterfaceKey>::iterator it =
                  bindingNames.right.find(remainingline);
              if (it == bindingNames.right.end()) {
                cout << "Binding name unknown while loading macro: "
                     << line.substr(1) << endl;
              } else {
            ...
            cout << "Binding missing while loading macro: " << line.substr(1)
                 << endl;
          }
        } else {
          cout << "Quantity not numeric or Unexpected tab(s) while loading "
                  "macro: "
               << line.substr(1) << endl;
        }
      } else {
        ...
        map<string, InterfaceKey>::iterator it =
            bindingNames.right.find(line.substr(1));
        if (it == bindingNames.right.end()) {
          cout << "Binding name unknown while loading macro: " << line.substr(1)
               << endl;
        } else {
          group.insert(it->second);
          if (group.size())
      ...
      map<string, InterfaceKey>::iterator it =
          bindingNames.right.find(line.substr(2));
      if (it == bindingNames.right.end())
        cout << "Binding name unknown while loading macro: " << line.substr(2)
             << endl;
      else
        group.insert(it->second);
    }
...
void enabler_inputst::save_macro_to_file(const string& file, const string& name,
                                         const macro& macro) {
  ofstream s(file.c_str());
  s << name << endl;
  for (macro::const_iterator group = macro.begin(); group != macro.end();
       ++group) {
    for (set<InterfaceKey>::const_iterator key = group->begin();
         key != group->end(); ++key)
...
list<string> enabler_inputst::list_macros() {
  ...
  list<string> ret;
  for (map<string, macro>::iterator it = macros.begin(); it != macros.end();
       ++it)
...
void enabler_inputst::save_macro(string name) {
  ...
  save_macro_to_file("prefs/macros/" + filter_filename(name, '_') + ".mak",
                     name, active_macro);
}
...
void enabler_inputst::stop_registering_key() { key_registering = false; }
...
std::list<RegisteredKey> enabler_inputst::getRegisteredKey() {
  ...
  for (list<EventMatch>::iterator it = stored_keys.begin();
...
void enabler_inputst::bindRegisteredKey(MatchType type, InterfaceKey key) {
  // 2022-08-29: this loop fails if there's no type match, so we add something
  // as needed
  bool added = false;
  for (list<EventMatch>::iterator it = stored_keys.begin();
       it != stored_keys.end(); ++it) {
...
bool enabler_inputst::is_registering() { return key_registering; }
...
bool enabler_inputst::prefix_building() { return in_prefix_command; }
...
string enabler_inputst::prefix() { return prefix_command; }
