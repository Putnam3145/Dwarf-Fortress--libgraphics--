...
char file_compressorst::flush_in_buffer() {
  ...
  while (c_stream.total_in != in_buffer_amount_loaded &&
         c_stream.total_out < out_buffersize) {
...
char file_compressorst::load_new_in_buffer() {
  ...
  if (out_buffer_amount_written > 0) {
    ...
    while (d_stream.total_out < in_buffersize &&
           d_stream.total_in < out_buffer_amount_written) {
...
char file_compressorst::open_file(const std::string& filename,
                                  char existing_only) {
  if (filename.empty())
    return 0;
  ...
  */
  ...
  // else
  // h=CreateFile(filename.c_str(),GENERIC_READ|GENERIC_WRITE,0,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
...
void file_compressorst::set_buffer_info(char* new_in_buffer,
...
char file_compressorst::read_file(bool& var) {
  int8_t temp;
  if (!read_file(&temp, sizeof(int8_t)))
    return 0;
  var = (temp != 0);
  return 1;
}
