...
void find_files_by_pattern_with_exception(const char* pattern,
                                          svector<char*>& filenames,
  ...
  if (h != INVALID_HANDLE_VALUE) {
    if (strcmp(finddata.cFileName, ".") && strcmp(finddata.cFileName, "..") &&
        strcmp(finddata.cFileName, exception)) {
      if (!(finddata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
        c = new char[strlen(finddata.cFileName) + 1];
        strcpy(c, finddata.cFileName);
        filenames.push_back(c);
...
void find_files_by_pattern(const char* pattern, stringvectst& filenames) {
  ...
  if (h != INVALID_HANDLE_VALUE) {
    if (strcmp(finddata.cFileName, ".") && strcmp(finddata.cFileName, "..")) {
      if (!(finddata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
        filenames.add_string(finddata.cFileName);
    }
...
void find_directories_by_pattern_with_exception(const char* pattern,
                                                stringvectst& filenames,
                                                const char* exception) {
  HANDLE h;
  WIN32_FIND_DATA finddata;
